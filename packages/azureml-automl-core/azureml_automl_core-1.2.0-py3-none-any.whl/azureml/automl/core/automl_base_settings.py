...
class AutoMLBaseSettings:
    ...
    def _verify_settings(self, logger):
        ...
        if self.is_timeseries:
            if isinstance(self.featurization, dict):
                self.validate_timeseries_featurization_settings(self.featurization, logger)
    ...
    def validate_timeseries_featurization_settings(
            self, featurization_config: Dict[str, Any], logger: logging.Logger) -> None:
        """
        Validate whether the custom featurization is supported.

        :param featurization_config: The customized featurization config.
        ...
        blocked_transformers = featurization_config.get("_blocked_transformers")
        ...
        column_purposes = featurization_config.get("_column_purposes")
        if column_purposes is not None and len(column_purposes) > 0:
            logging.warning(warning_msg_template.format("column_purposes"))
            logger.warning(warning_msg_template.format("column_purposes"))

        drop_columns_error_msg = ("The {} as {} cannot be dropped in featurization config's drop "
                                  "columns for a forecasting tasks.")
        if featurization_config.get("_drop_columns") is not None:
            drop_columns = set(featurization_config.get("_drop_columns"))
        else:
            drop_columns = set()
        ...
        if self.time_column_name in drop_columns:
            raise InvalidValueException(
                ...
                target="drop_columns",
                reference_code=ReferenceCodes._SETTINGS_CONFING_DROP_TIME_COLUMN).with_generic_msg(
                    "Time column name is in the featurization config drop columns.")

        if self.grain_column_names is not None:
            shared_grain_columns = drop_columns.intersection(set(self.grain_column_names))
            if len(shared_grain_columns) > 0:
                raise InvalidValueException(
                    drop_columns_error_msg.format(",".join(shared_grain_columns), "grain columns"),
                    target="drop_columns",
                    reference_code=ReferenceCodes._SETTINGS_CONFING_DROP_GRAIN_COLUMNS).with_generic_msg(
                        "Grain columns are in the featurization config drop columns.")
        ...
        if len(shared_reserve_columns) > 0:
            raise InvalidValueException(
                ...
                target="drop_columns",
                reference_code=ReferenceCodes._SETTINGS_CONFING_DROP_RESEVERD_COLUMNS).with_generic_msg(
                    "Reserved columns are in the featurization config drop columns.")

        if self.drop_column_names is not None:
            shared_drop_columns = drop_columns.intersection(set(self.drop_column_names))
            if len(shared_drop_columns) > 0:
                warning_msg = ("Featurization config drop columns and automl settings "
                               "drop columns contains same columns.")
                logging.warning(warning_msg)
                logger.warning(warning_msg)

        wrong_featurization_msg = ("Forecasting task only support Imputer with constant value now. "
                                   "All other settings will be ignored.")

        for col in self.drop_column_names:
            drop_columns.add(col)
        cannot_customized_reversed_columns = set(
            [col for col in constants.TimeSeriesInternal.RESERVED_COLUMN_NAMES
                if col != constants.TimeSeriesInternal.DUMMY_TARGET_COLUMN])

        transformer_params = featurization_config.get("_transformer_params")
        if transformer_params is not None and len(transformer_params) > 0:
            if transformer_params.get(constants.TimeSeriesInternal.IMPUTER) is not None:
                config_dropped_columns = list()
                config_reserved_columns = list()
                for cols, params in transformer_params[constants.TimeSeriesInternal.IMPUTER]:
                    if params.get(constants.TimeSeriesInternal.IMPUTER_STRATEGY) != \
                            constants.TimeSeriesInternal.IMPUTER_CONSTANT:
                        logging.warning(wrong_featurization_msg)
                        logger.warning(wrong_featurization_msg)
                    elif params.get(constants.TimeSeriesInternal.IMPUTER_FILL_VALUE) is None:
                        raise MissingValueException(
                            "For constant value imputation, the fill value cannot be None.",
                            target=constants.TimeSeriesInternal.IMPUTER_FILL_VALUE,
                            reference_code=ReferenceCodes._SETTINGS_CONFIG_MISSING_IMPUTE_VALUE,
                            has_pii=False)
                    for col in cols:
                        if col in drop_columns:
                            config_dropped_columns.append(col)
                        if col in cannot_customized_reversed_columns:
                ...
                if len(config_dropped_columns) > 0:
                    raise InvalidValueException(
                        ...
                        reference_code=ReferenceCodes._SETTINGS_CONFIG_IMPUTE_COLUMN_DROPPED).with_generic_msg(
                            "Imputation configuration was provided for the column to be dropped.")
                if len(config_reserved_columns) > 0:
                    raise InvalidValueException(
                        ...
                        target="featurization_config",
                        reference_code=ReferenceCodes._SETTINGS_CONFIG_IMPUTE_COLUMN_RESERVED).with_generic_msg(
                            "Imputation configuration was provided for the column to be reserved columns.")
