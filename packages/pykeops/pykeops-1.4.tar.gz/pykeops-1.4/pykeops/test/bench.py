...
class TestGPUDirect(unittest.TestCase):
    def setUp(self):
        ...
        self.num_N = 10
    def test_benchmark_pykeops(self):
        times_GPU = []
        all_N = np.geomspace( self.start_N, self.stop_N, self.num_N).astype('int')
        for N in all_N:
            print(N)
            time_GPU = 0
            for i in range(self.num_iterations):
                print(i)
                points = cp.random.random((N, 3))
                st = time.time()
                pots = direct_pykeops_compute_torch(points)
                tiime = (time.time() - st)
                print(tiime)
                time_GPU += tiime / self.num_iterations
