...
def turn_entity_event_into_attribute(
        ...
        attributeValueType: Optional[Type[ProfileAttributeValue]] = EntityAttributeValue,
        cast_inner_type: bool = True) -> ProfileAttributeType:
    """
    Transforms an attribute into an entity event.
        If type(attribute) == ProfileAttribute[EntityAttributeValue] then the Entity Event captured
            within the attribute is used as is ...
    ...
    if cast_inner_type:
        ...
        attribute_value = cast_ee_into_attr_value_according_to_schema(entityEvent, value_type_to_cast_to)
    else:
        attribute_value = dict(entityEvent).get("properties", {})
