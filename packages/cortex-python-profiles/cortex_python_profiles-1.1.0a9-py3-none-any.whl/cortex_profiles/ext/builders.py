...
class BulkProfilesBuilder(AbstractProfilesBuilder):
    ...
    def build(self,
              ...
              reset_cache: bool = True) -> Tuple[int, List, List]:
        """
        ...
        * This method logs progress as chunks are incrementally saved ...
        ...
        while (next_attr_chunk is not None) or (next_event_chunk is not None):
            ...
            if validated_events_in_chunk:
                ...
                attributes_in_events = [
                    turn_entity_event_into_attribute(
                        ...
                        cast_inner_type=False,  # Already casted previously ...
                        **pydash.defaults(
                ...
                te = time.time()
                time_take = f'%2.2f seconds' % (te - ts)
                log.info(f'Converted Events into Attributes in {time_take}')
            ...
            if reset_cache is True:
                # Flush cache ...
                ts = time.time()
                reset_response = self._profiles_client.delete_cache_for_specific_profiles(list(profile_ids_to_flush))
                te = time.time()
                time_take = f'%2.2f seconds' % (te - ts)
                log.info(f'Flushed cache in {time_take}')
                log.debug(reset_response)
            ...
            te = time.time()
            time_take = f'%2.2f seconds' % (te - ts)
            log.info(f'Generated Next Chunks from Iterator in {time_take}')
            chunk_num += 1
...
class ProfilesBuilder(AbstractProfilesBuilder):
    ...
    def with_events(self, events: List[TProfileBuildingEventAlternatives]) -> 'ProfilesBuilder':  # type:ignore
        ...
        :return:
        """
        casted_events = self.cast_events(iter(events))
        errors = self.stream_errors(iter(casted_events))
        for error in errors:
            log.warning(error)
        valid_events = self.stream_valid_events(iter(casted_events))
