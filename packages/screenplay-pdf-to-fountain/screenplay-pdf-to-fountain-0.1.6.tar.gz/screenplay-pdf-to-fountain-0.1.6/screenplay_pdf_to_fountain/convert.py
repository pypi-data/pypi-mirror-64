...
def generateFountain(scriptJson):
    fountainOutput = ""
    firstPagesEnded = False
    for page in scriptJson:
        if not firstPagesEnded and "scene" in page["content"][0]:
            firstPagesEnded = True
            fountainOutput += "===\n\n"
        ...
        for i, content in enumerate(page["content"]):
            if "scene_number" in content and content["scene_number"] != currentSceneNumber:
                ...
                fountainOutput += "{} {} {}\n\n".format(
                    currentScene["region"], currentScene["location"], time)
            ...
            if "scene" in content:
                for section in content["scene"]:
                    if section["type"] == "ACTION":
                        ...
                        modifier = " ({})".format(
                            section["content"]["modifier"]) if section["content"]["modifier"] != None else ""
                        fountainOutput += "{}{}\n".format(
                            section["content"]["character"], modifier)
                ...
                fountainOutput += "{}\n\n".format(
                    content["text"])
    return fountainOutput.encode("utf-8")
