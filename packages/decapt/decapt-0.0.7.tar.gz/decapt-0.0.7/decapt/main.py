...
def main():
    ...
    com_gen.add_argument("conf", help="Configuration path", default="decapt.luxem")
    ...
    com_sync.add_argument("conf", help="Configuration path", default="decapt.luxem")
    ...
    if command == "generate":
        ...
        for package in conf["installed"]:
            ...
            want.setdefault(plugin, set()).add(package)
        for plugin in plugins:
            print(f"Scanning current package state for {plugin.id()}...")
            installed = set(plugin.installed())
            add[plugin.id()] = want.get(plugin.id(), set()) - installed
            remove[plugin.id()] = installed - want.get(plugin.id(), set())
