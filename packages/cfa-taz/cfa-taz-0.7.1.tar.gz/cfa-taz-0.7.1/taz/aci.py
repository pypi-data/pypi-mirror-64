    ...
    def __init__(
        ...
        Args:
            ...
            - subscription_id (None, optional, str): subscription 
                id if not default, can also be set by env var
                AZURE_SUBSCRIPTION_ID (parameter first)
        """
        ...
        if os.environ.get("AZURE_CLIENT_SECRET"):
            ...
            self.client = get_client_from_cli_profile(
                ContainerInstanceManagementClient, subscription_id=self.subscription_id
            )
        else:
