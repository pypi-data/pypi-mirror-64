...
class PythonPackage:
    ...
    def __init__(self, project, package_fallback=False):
        ...
        version_makers = self._collect_version_makers(package_fallback)
    ...
    def _collect_version_makers(self, package_fallback):
        version_makers = [self._get_version_from_tag, self._get_version_from_version_py]
        if package_fallback:
            version_makers.append(self._get_version_from_package)
        return version_makers
    ...
    def _get_version_from_package(self):
        try:
            ...
            self._logger.debug('Got version from package: %s', version_from_current_package)
            return version_from_current_package
        except DistributionNotFound as exc:
            self._logger.debug('Unable to get version from package %s [%s]', self._project, exc)
            return None
