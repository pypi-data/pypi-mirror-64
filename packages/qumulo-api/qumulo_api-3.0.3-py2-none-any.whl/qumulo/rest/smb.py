...
#       |_|
@request.request
def list_file_handles(conninfo, credentials, limit=None, after=None):
    method = 'GET'
...
@request.request
def close_smb_file(conninfo, credentials, id_):
    method = 'POST'
    uri = str(UriBuilder(path="/v1/smb/files/close")
        .add_query_param("location", id_))
    return request.rest_request(conninfo, credentials, method, uri)
