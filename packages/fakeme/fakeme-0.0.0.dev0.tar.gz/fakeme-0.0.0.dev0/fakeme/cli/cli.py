...
import argparse
import inspect
from fakeme.core import RunGenerator
...
def run_cli():
    ...
    with open(config_path, 'r') as config:
        ...
        RunGenerator(path_prefix=os.path.dirname(config_path),
                     cli_path=os.path.dirname(config_path),
                     **conf).run()


def validate_config(conf):
    possible_config_params = inspect.signature(RunGenerator.__init__).parameters
