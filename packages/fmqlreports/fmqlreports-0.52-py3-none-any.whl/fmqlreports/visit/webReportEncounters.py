...
from fmqlutils.schema.reduceTypeUtils import checkDataPresent, muBVC, muBVCOfSTProp
from fmqlreports.webReportUtils import TOP_MD_TEMPL, SITE_DIR_TEMPL, keyStats, roundFloat, reduce44, filteredResultIterator
...
def gatherEncounterDataByDay(stationNo, onAndAfterDay, upToDay):
    ...
    try:
        ...
        printf("Appt source data not yet gathered by day - organizing relevant 44_003, 409_84, 9000010_06 and 9000010 ...")
    else:
    ...
    def gatherVisitsByDay(stationNo, onAndAfterDay, upToDay):
        ...
        try:
            ...
            printf("9000010 not yet gathered by day for {} to {} ...".format(onAndAfterDay, upToDay))
        else:
            return visitsByDay

        visitStartDate = (datetime.strptime(onAndAfterDay, "%Y-%m-%d") - relativedelta(years=1)).strftime("%Y-%m-%d") # one year back
        printf("Organizing 9000010 starting at {}, one year back from onAndAfter {} ...".format(visitStartDate, onAndAfterDay))
        ...
        resourceIter = FilteredResultIterator(dataLocn, "9000010", isOrdered=False, startAtReply=startAtReply)
        printf("... starting at reply {}".format(startAtReply))
        for i, resource in enumerate(resourceIter, 1):
            ...
            if i % 50000 == 0:
                printf("\tAnother 50K to {:,}, {:,} taken, {:,} credit stop visit's skipped, last resource day {} - 50K resource create date {}".format(i, cntEncountersInDateRange, cntQualCreditStopVisitsSkipped, resourceDay, resource[startProp]["value"] if startProp in resource else "?"))
        ...
        printf("... finished taking {:,} encounters in date range ({}-{}), {:,} are top level, {:,} children, {:,} in range child visits unattached and dropped".format(
    ...
    def gatherVProviderByVisitIEN(stationNo, onAndAfterDay, upToDay, topVisitIENs, childVisitIENs):
        ...
        printf("Organizing 9000010_06 under visits starting at {}, the on and after date ...".format(onAndAfterDay))
    ...
    def gatherOldApptsByDay(stationNo, onAndAfterDay, upToDay):
        ...
        try:
            ...
            printf("44_003 not yet gathered by day for {} to {} ...".format(onAndAfterDay, upToDay))
        else:
            return oldApptsByDay

        printf("Organizing 44_003 (walking all) ...")
        ...
        for i, resource in enumerate(filteredResultIterator(stationNo, "44_003", useRF=True), 1):
            if i % 50000 == 0:
                printf("\tAnother 50K to {:,}, {:,} taken".format(i, taken))
            resourceDay = getResourceDay(resource, createProp, onAndAfterDay, upToDay)
            # filter to window
            if resourceDay == "":
                continue
        ...
        printf("... finished processing {:,} 44_003's, {:,} taken".format(i, taken))
    ...
    def gatherNewApptsByDay(stationNo, onAndAfterDay, upToDay):
        ...
        try:
            ...
            printf("409_84 not yet gathered by day for {} to {} ...".format(onAndAfterDay, upToDay))
        else:
            return newApptsByDay
        
        printf("Organizing 409_84 (walking all, filtering to window) ...")
        ...
        printf("... finished processing {:,} 409_84, {:,} taken".format(i, taken))
    ...
    printf("Returning Organized Data")     
    ...
    def oneAppt(info, apptPrefix="apptN"):
        ...
        if len(apptProps) > 1:
            if apptPrefix == "apptN" or apptPrefix not in info:  
                raise Exception("Ambiguous multiple {} matches".format(apptPrefix))
            apptProp = apptPrefix
        else:
            apptProp = apptProps[0]
        # Strict for apptN - looser - pick bigger for apptL (TODO: tighten)
        if len(info[apptProp]) > 1:
            if apptPrefix == "apptN": # Strict
                raise Exception("Ambiguous multiple {}s at same time".format(apptPrefix))
    ...
    def matchCreationDateTimes(apptN, apptL, visit):
        ...
        def squeezeTimes(dtTimeCollection):
            ...
            for i, dt in enumerate(sorted(dts), 1):
                try:
                    dtDT = datetime.strptime(dt, "%Y-%m-%dT%H:%M:%SZ")
                except:
...
def printf(x, flush=True): # same as print in Python 3
    print(x)
    sys.stdout.flush()
