...
class LogicQuBit(Qubits, Gates, Circuit):
    ...
    def __init__(self, qubits_number = 3, **kwargs):
        ...
        self.__product_first_left = kwargs.get('product_first_left', False)
        self.__qubits_number = qubits_number
        if(self.__symbolic):
            self.__cuda = False
        super().setCuda(self.__cuda)
        super().__init__(qubits_number, self.__symbolic, self.__product_first_left)
        Gates.__init__(self, qubits_number, self.__product_first_left)
    ...
    def Measure(self, target, first_lsb = False):  # ex: medir 3 qubits de 5: 2,1,4 do estado "010" -> M010 = |1><1| x |0><0| x 1 x |0><0| x 1
        target = self.qubitsToList(target)
        if(not first_lsb):  # first_lsb = True -> o primeiro da lista será o menos significativo
            target.reverse()
        self.setMeasuredQubits(target)
        self.addOp("Measure", target)
        density_m = self.DensityMatrix()
        ...
        for i in range(size):
            ...
            if (not self.__product_first_left):
                tlist.reverse()
            M = self.kronProduct(tlist)
            measure = (density_m * M).trace()  # valor esperado
            if(self.__cuda):
