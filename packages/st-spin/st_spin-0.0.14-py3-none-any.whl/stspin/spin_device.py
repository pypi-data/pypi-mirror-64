...
class SpinDevice:
    ...
    def getRegister(self, register: int) -> SpinValue:
        ...
        print(f'Wat: {Command.ParamGet} | {register}')
        print(f'Command val should be {Command.ParamGet | register}')
        self._writeCommand(Command.ParamGet | register)

        response_bytes: List[int] = []

        print('Kwokka')
        for _ in range(RegisterSize):
            response_bytes += self._write(Command.Nop)
            print(f'Growing: {response_bytes}')
    ...
    def move(self, steps: int) -> None:
        ...
        command = (Command.Move | self._direction)
        command.setPayload(steps)
    ...
    def run(self, steps_per_second: float) -> None:
        ...
        command = (Command.Run | self._direction)
        command.setPayload(speed)

        self._writeCommand(command)

    def setDirection(self, direction: SpinValue) -> None:
    ...
    def isBusy(self) -> bool:
        ...
        return False if (status.Value & Status.NotBusy) else True
