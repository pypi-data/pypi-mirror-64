...
def getByteCount(value: int) -> int:
    ...
    :value: value to check. Non-negative :returns: number of bytes to represent the number
...
def getVisualStatus(status: int) -> str:
    ...
    if status & MotorStatus.Stopped:
        Motor += 'Stopped'
    elif status & MotorStatus.ConstantSpeed:
