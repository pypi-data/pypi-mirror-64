    ...
    def process_response(self, request, response):
        ...
        log_entry = dict(
            ...
            body=Utils.obfuscate_body(Utils.adjust_string_length(self._initial_http_body,
                                            getattr(settings, MAX_REQUEST_RESPONSE_SIZE)) if response.status_code < 400 else self._initial_http_body.decode("UTF-8")),
            status_code=response.status_code,
            response=Utils.obfuscate_response(Utils.adjust_string_length(response.content,
                                                settings.MAX_REQUEST_RESPONSE_SIZE) if response.status_code < 400 else response.content.decode("UTF-8")),
            url=Utils.obfuscate_url(request.build_absolute_uri())
        )
