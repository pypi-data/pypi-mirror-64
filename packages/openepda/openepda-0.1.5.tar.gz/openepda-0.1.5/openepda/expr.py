# -*- coding: utf-8 -*-
"""This file contains parser of the analytic expressions

The parser can be used to parse the analytic expressions and thus
check their correctness in a safe way, and also for evaluation of the
expressions.

The grammar is intended to be very similar to the tinyexpr library
grammar (https://github.com/codeplea/tinyexpr). Normally,
...
UNDERSCORE = tuple("_")
VARIABLE_NAME = LETTERS_ALL + UNDERSCORE
SPACE = tuple(" ")
...
# Aliases under which the functions can be imported from math
FCN_NAME_ALIASES = {
    "abs": "fabs",
    "fac": "factorial",
    "ln": "log",
...
class _BaseParser(object):
    ...
    def _check_symbols(self, symbols):
        ...
        The check will not pass if:
            ...
            - any symbol is the same as a python keyword
        ...
        for s in symbols:
            if iskeyword(s):
                # python keywords
                raise OpenEpdaExpressionError(
                    'Symbol "{}" is a reserved keyword in python.'.format(s)
                )
            elif s in self._imported_symbols.keys():
    ...
    def get_parameter_names(self, expr):
        ...
        -------
        list of str
            list of parameter names present in the expression
        """
        ast = self.parse(expr)
    ...
    def eval(self, expr, params=None):
        ...
        params = params or {}
        self.check_missing_params(expr, params.keys())
        ...
        res = eval(expr, self._imported_symbols, params)
        return res

    def check_missing_params(self, expr, params):
        ...
        missing_params = set(actual_param_names) - set(params)
        if missing_params:
            raise OpenEpdaExpressionError(
...
def check_parameter_name(name, reserved=()):
    ...
    if not (name[0] in LETTERS_LOWER or name[0] in LETTERS_UPPER):
        ...
        # python keywords
        raise OpenEpdaExpressionError(
            'Parameter name "{}" is a reserved keyword in python.'.format(name)
        )
...
def check_expr(expr):
    ...
    if any(map(partial(contains, expr), kwlist)):
        raise OpenEpdaExpressionError(
            "Expression contains a reserved python keyword."
        )
    elif not all(map(partial(contains, ALLOWED_SYMBOLS), expr)):
