...
def _check_building_block(bb_name, bb):
    ...
    if has_params:
        bb_params = bb["parameters"].keys()
        for name in bb_params:
            check_parameter_name(
                name, reserved=openepda_parser.reserved_identifiers
            )
    else:
        bb_params = []
    ...
    # check bbox
    for bbox_coord_expr in chain(*bb["bbox"]):
        _check_bb_num_value(bbox_coord_expr, bb_name, params=bb_params)

    # check pins
    for pin_name, pin in bb["pins"].items():
        _check_pin_num_value(pin["width"], pin_name, bb_name, params=bb_params)
        for i in range(3):
            _check_pin_num_value(
                pin["xya"][i], pin_name, bb_name, params=bb_params
            )
...
def _check_pin_num_value(
    v, bb_name, pin_name, params=(), parser=openepda_parser
):
