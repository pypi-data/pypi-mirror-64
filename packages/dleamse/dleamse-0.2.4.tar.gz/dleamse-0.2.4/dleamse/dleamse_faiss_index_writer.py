...
class FaissWriteIndex:
    ...
    def create_index_for_one_file(self, ids_embedded_spectra, ids_save_file, output_path):
        """
    ...
    def create_index_from_embedded_spectra(self, ids_embedded_spectra_path, ids_save_file, output_path):
        """
        Create faiss indexIDMap index
        :param spectra_vectors: spectra embedded data
        :param usi_data: coresponding usi data
        ...
        for i in range(len(embedded_spectra_file_list)):
            if embedded_spectra_file_list[i].endswith("_embedded.txt"):
                embedded_spectra_data = pd.read_csv(embedded_spectra_file_list[i], header=True, index=False)
                ...
                if i == 0:
                    ...
                    if update_id_bool is True:
                        ...
                        store_embed_new_file = dir_path + str(embedded_spectra_file_list[i]).strip("_embedded.txt")+ "_new_ids_embedded.txt"
                        ids_vstack_df.to_csv(store_embed_new_file, header=True, index=None, columns=["ids", "embedded_spectra"])
                        print("Update ids for " + embedded_spectra_file_list[i] + ", and save in new file:" + ids_vstack_df )

                        # add index
                        index.add_with_ids(tmp_spectra_vectors.astype('float32'), np.array(update_new_ids))
                        raw_ids = raw_ids.extend(update_new_ids)
                    else:
                        index.add_with_ids(tmp_spectra_vectors.astype('float32'), ids_data)

                        raw_ids = raw_ids.extend(ids_data.tolist())
        ...
        for i in range(len(embedded_spectra_file_list)):
            if embedded_spectra_file_list[i].endswith("_embedded.txt"):
                ...
                if update_id_bool is True:
                    ...
                    update_ids_df = pd.DataFrame({"ids": update_new_ids})
                    ids_vstack_df = pd.concat([update_ids_df, embedded_spectra_data["embedded_spectra"]], axis=1)
                    store_embed_new_file = dir_path + str(embedded_spectra_file_list[i]).strip(
                        "_embedded.txt") + "_new_ids_embedded.txt"
                    ids_vstack_df.to_csv(store_embed_new_file, header=True, index=None,
                                         columns=["ids", "embedded_spectra"])
                    print("Update ids for " + embedded_spectra_file_list[
                        i] + ", and save in new file:" + ids_vstack_df)
