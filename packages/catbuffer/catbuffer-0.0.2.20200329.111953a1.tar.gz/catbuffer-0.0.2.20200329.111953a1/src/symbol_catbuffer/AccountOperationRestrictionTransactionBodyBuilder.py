...
class AccountOperationRestrictionTransactionBodyBuilder:
    ...
    def __init__(self,  restrictionFlags: int,  restrictionAdditions: List[int],  restrictionDeletions: List[int]):
    ...
    def loadFromBinary(cls, payload: bytes) -> AccountOperationRestrictionTransactionBodyBuilder:
        ...
        restrictionAdditions: List[int] = []
        for i in range(restrictionAdditionsCount):
            item = GeneratorUtils.bufferToUint(GeneratorUtils.getBytes(bytes_, 2))
            restrictionAdditions.append(item)
            bytes_ = bytes_[2:]
        # gen: _load_from_binary_array
        restrictionDeletions: List[int] = []
        for i in range(restrictionDeletionsCount):
            item = GeneratorUtils.bufferToUint(GeneratorUtils.getBytes(bytes_, 2))
    ...
    def getSize(self) -> int:
        ...
        for x in self.restrictionAdditions:
            size += 2
        for x in self.restrictionDeletions:
            size += 2
        return size
    ...
    def serialize(self) -> bytes:
        ...
        for item in self.restrictionAdditions:
            restrictionAdditionsBytes = GeneratorUtils.uintToBuffer(item, 2)
            bytes_ = GeneratorUtils.concatTypedArrays(bytes_, restrictionAdditionsBytes)
        for item in self.restrictionDeletions:
            restrictionDeletionsBytes = GeneratorUtils.uintToBuffer(item, 2)
