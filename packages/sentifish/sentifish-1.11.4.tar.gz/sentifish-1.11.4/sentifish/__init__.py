    ...
    def __wordData__(self):
        ...
        for line in d:
            try:
                ...
                pass
        __word_list__= {'word':word,'score':score,'pos':pos}
...
def remove_stopwords(text):
    ...
    score=__sentiWords__['score']
    word= __sentiWords__['word']
    rb=0
            ...
            if((words_length -i) >=2):
                ...
                if(tagged_word in hv):
                    ...
                    if(word in hv):
                        ...
                        i+=1
                        tag=pos_tags[word_list.index(word)]
                        if(tag == 'a'):
    ...
    def fix_polarity(self,tagged_words_list):
        ...
        for word in tagged_words_list:
            if((word[2][0] == 'JJ') or (word[2][0] == 'RB') or (word[2][0] == 'Nn') or (word[2][0] == 'V')):
                word[2].append(score[data.index(word[0])])
            elif('VB' in word[2][0]):
...
class Sentiment():
    def __init__(self,text):
        ...
        #__score__ =  __sentiWords__['score']
        __word__ =__sentiWords__['word']
        __score__=__sentiWords__['score']
        __pos__ = __sentiWords__['pos']
        obj=Words()
        inv=obj.invert_words()
        inc=obj.increase_words()
        self.__wordList__ = []
        ...
        for w in data:
            if(w in __words_col__):
                self.__wordList__.append((w,["UN",0.0]))
            elif(w in inc):
                if(w ==  "real"):
                    self.__wordList__.append((w,["INC",0.0,1.5]))
                else:
                    self.__wordList__.append((w,["INC",0.0,1.3]))
            elif(w in inv):
                self.__wordList__.append((w,["INV",0.0]))
            elif(w in __word__):
                wi=__word__.index(w)
                tag=__pos__[wi]
                if(tag == 'r'):
                    pol=__score__[wi]
                    self.__wordList__.append((w,["RB",pol]))
                else:
                    pol=__score__[wi]
                    self.__wordList__.append((w,["UN",pol]))
            else:
                self.__wordList__.append((w,["UN",0.0]))
