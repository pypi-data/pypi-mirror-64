...
global __base_form__
global __past_form__
global __past_part__ 
global __s_form__
global __ing_form__
global __polarity_v__
global __pos_tags__
global __pos_score__
global __neg_score__
global __words_col__
__words_col__=[]
class Characters():
...
def load():
    ...
    #obj.__verbs__()
    #obj.__wordData__()
    __base_form__ = __verbData__['base_form']
    __past_form__ = __verbData__['pastForm']
    __past_part__ = __verbData__['pastParticiple']
    __s_form__ = __verbData__['s/es/iesForm']
    __ing_form__ = __verbData__['ingForm']
    __polarity_v__=__verbData__['polarity']
    __pos_tags__= __sentiWords__['pos']
    __pos_score__=  __sentiWords__['posScore']
    __neg_score__=  __sentiWords__['negScore']
    obj=Words()
    char_obj=Characters()
    __words_col__.extend(obj.helping_verbs())
    __words_col__.extend(obj.wh_pronouns())
    __words_col__.extend(obj.prepositions())
    __words_col__.extend(obj.conjunctions())
    __words_col__.extend(obj.cardinal_numbers())
    __words_col__.extend(char_obj.specialChars())
    __words_col__.extend(obj.pronoun_words())
...
class Sentiment():
    def __init__(self,text):
        ...
        for word in data:
            if(word in __words_col__):
                self.__wordList__.append((word,["UN",0.0]))
            elif(word in inc):
                if(word ==  "real"):
                    self.__wordList__.append((word,["INC",0.0,1.5]))
                else:
                    self.__wordList__.append((word,["INC",0.0,1.3]))
            elif(word in inv):
                self.__wordList__.append((word,["INV",0.0]))
            elif(word in __base_form__):
                word_index =__base_form__.index(word)
                pol=float(__polarity_v__[word_index])
                self.__wordList__.append((word,["VB",pol]))
            elif(word in __past_form__):
                word_index =__past_form__.index(word)
                pol=float(__polarity_v__[word_index])
                self.__wordList__.append((word,["VBD",pol]))
            elif(word in __s_form__):
                word_index =__s_form__.index(word)
                pol=float(__polarity_v__[word_index])
                self.__wordList__.append((word,["VBN",pol]))
            elif(word in __past_part__):
                word_index =__past_part__.index(word)
                pol=float(__polarity_v__[word_index])
                self.__wordList__.append((word,["VBN",pol]))
            elif(word in __ing_form__):
                word_index = __ing_form__.index(word)
                pol=float(__polarity_v__[word_index])
                self.__wordList__.append((word,["VBG",pol]))
            elif(word in __pos_tags__):
                tag=__pos_tags__[word]
                if(tag == 'r'):
                    pol=float(__pos_score__[word]) + float(__neg_score__[word])
                    self.__wordList__.append((word,["RB",pol]))
                else:
                    pol=float(__pos_score__[word]) + float(__neg_score__[word])
                    self.__wordList__.append((word,["UN",pol]))
            else:
                self.__wordList__.append((word,["UN",0.0]))
    ...
    def __analyzer__(self,sec_last,last_token,current_token):
        ...
        if(current_token == []):
            if(last_token == ()):
                #print(1)
                return(0)
            elif((last_token[1][0] == "RB") and (last_token[1][1] != 0)):
                if(sec_last == ()):
                    self.__count__ +=1
                    pol=last_token[1][1]
                    #print(2)
                    ...
                    if(sec_last[1][0] == "INV"):
                        pol=(- last_token[1][1])/2
                        self.__count__ +=1
                        return(pol)
                    elif(sec_last[1][0] == "INC"):
                        pol=last_token[1][1]*sec_last[1][2]
                        self.__count__ +=1
                        return(pol)
                    else:
                        self.__count__ += 1
                        return(last_token[1][1])
            elif(last_token[1][0]=='INC'):
                if(sec_last == ()):
                    pol=0.2
                    self.__count__ +=1
                    return(pol)
                elif(sec_last[1][0] == 'INV'):
                    pol=-0.1
                    self.__count__ +=1
                    return(pol)
                elif(sec_last[1][0] == 'INC'):
                    pol=0.2 * sec_last[1][2]
                    self.__count__+=1
                    return(pol)
                else:
                    self.__count__ +=1
            ...
            if(ct[1][1] == 0):
                ...
                if(last_token == ()):
                    ...
                    if(sec_last == ()):
                        ...
                        if(sec_last[1][0] == "INV"):
                            ...
                            pol=last_token[2][1]* sec_last[1][2]
                            if(pol > 1):
                                pol=1
                            if(pol == 0):
                                return(pol + self.__analyzer__(last_token,ct,current_token[1:]))
                ...
                if(last_token == ()):
                    ...
                    if(ct[1][0] == "RB"):
                        ...
                        if(sec_last !=()):
                            if((sec_last[1][0] == 'INV') and (last_token[1][1] == 0)):
                                pol=(- ct[1][1])/2
                                self.__count__ +=1
                                return(pol + self.__analyzer__(last_token,ct,current_token[1:]))
                            else:
