...
def global_torch_device(cuda_if_available: bool = None,
                        override: torch.device = None,
                        verbose: bool = False) -> torch.device:
  """
...
"""
  ...
  if override is not None:
    ...
    d = torch.device("cuda" if torch.cuda.is_available() and cuda_if_available else "cpu")
    if device is None:
      device = d
    return d
  elif device is None:
...
if __name__ == "__main__":
  def stest_override():
    ...
    print(global_torch_device(override=global_torch_device(cuda_if_available=False, verbose=True),
                              verbose=True))
    print(global_torch_device(verbose=True))
    print(global_torch_device(cuda_if_available=True))
    print(global_torch_device())
    print(global_torch_device(override=global_torch_device(cuda_if_available=True, verbose=True)))
    print(global_torch_device())
