...
class Arguments(Context):
    ...
    def froms(cls, *args):
        context = cls()
        context.update({arg.key: arg.value for arg in args})
        context.update({arg.alias: arg.value for arg in args})
        return context
...
class Component(
    ...
    def loadComponentArguments(self, argsDict=None):
        ...
        argsDict = argsDict or self.argsDict
        args = self.loadFormatArguments(copy.deepcopy(self.getArguments(exclude="outputs")), argsDict)
        return Arguments.froms(*args)
    ...
    def arg(self, argument, argtype="args", reverse=True):
