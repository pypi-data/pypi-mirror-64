...
def project_documents(topic_ontology):
    ...
    path_word2vec_model = get_path_topic_model('word2vec','word2vec')
    path_word2vec_vocab = get_path_vocab('word2vec')
    LOGGER.warn("Lok: preprocessing document %s "%documents.first())
    vector = argument_esa_model.esa_top_n_terms.model_topic(path_corpus,path_word2vec_model,path_word2vec_vocab,documents.first(),10)
    LOGGER.warn("Lok: preprocessing document resulted in vector %s "%str(vector))
    array=dict_to_list(vector)
    LOGGER.warn("Lok: preprocessing document resulted in picled vector %s "%str(array))
    ...
    LOGGER.warn("Lok: finished documents"%path_document_vectors)
