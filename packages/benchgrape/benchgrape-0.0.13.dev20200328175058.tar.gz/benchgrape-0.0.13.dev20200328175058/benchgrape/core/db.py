...
class TestDataMapper(Mapper):
    ...
    def available_user_count(self):
        return self.db.cursor().execute(
            "select count(*) FROM users WHERE connected='FALSE'"
        ).fetchone()[0]

    def sync_db(self, organization, users=None):
        c = self.db.cursor()
        self.logger.info(f'inserting {len(users)}')
        try:
