...
def read_info_file(info_file_name):
    ...
    try:
        ...
        dir_script = Path(__file__)
        info_file_name = Path(dir_script, info_file_name)
        with open(info_file_name) as f:
            info_file_content = f.readlines()

    dir_info_file = Path(info_file_name).parent
#    dir_file_parent = Path(__file__).parent
    print("---------------------------------------")
    ...
    for line in info_file_content:
        ...
        ### DIR_SOURCE:
            ...
            if not dir_source.is_absolute():
                dir_source = Path(Path(__file__).resolve().parent, dir_source)
            print('DIR_SOURCE read from file:', dir_source)
            ...
            #                dir_source = Path(dir_info_file, dir_source)
            #            elif dir_source[:3] == '../':
            #                dir_source = dir_source[3:]
            #                dir_source = Path(dir_file_parent, dir_source)
            flag_dir_source = True
        ### DIR_SAVE:
        elif line[:8].lower() == "DIR_SAVE".lower():
            dir_save = line[9:].strip()
            dir_save = Path(dir_save)
            print('DIR_SAVE read from file:', dir_save)
#            assert dir_save.is_dir(), "'DIR_SAVE' not a valid folder path."
    ...
    if not flag_dir_source:
        ...
        # dir_source = Path(dir_parent, 'resources', 'datasets')
        dir_source = Path(dir_info_file, "datasets")
    ...
    if not flag_dir_save:
        ...
        # dir_path = dir_path.resolve()
        # dir_parent = dir_path.parent
        # dir_save_main = Path(dir_parent, 'resources', 'output')
        dir_path = Path.cwd()
        dir_save = Path(dir_path, "output")
