...
def build_model(image_size,
    ...
    conv1 = tf.keras.layers.Conv2D(32, (5, 5), strides=(1, 1), padding="same", kernel_initializer='he_normal',
                                   kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv1')(x1)
    conv1 = tf.keras.layers.BatchNormalization(axis=3, momentum=0.99, name='bn1')(
    ...
    conv2 = tf.keras.layers.Conv2D(48, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal',
                                   kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv2')(pool1)
    conv2 = tf.keras.layers.BatchNormalization(axis=3, momentum=0.99, name='bn2')(conv2)
    conv2 = tf.keras.layers.ELU(name='elu2')(conv2)
    pool2 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), name='pool2')(conv2)
    
    conv3 = tf.keras.layers.Conv2D(64, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal',
                                   kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv3')(pool2)
    conv3 = tf.keras.layers.BatchNormalization(axis=3, momentum=0.99, name='bn3')(conv3)
    conv3 = tf.keras.layers.ELU(name='elu3')(conv3)
    pool3 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), name='pool3')(conv3)
    
    conv4 = tf.keras.layers.Conv2D(64, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal',
                                   kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv4')(pool3)
    conv4 = tf.keras.layers.BatchNormalization(axis=3, momentum=0.99, name='bn4')(conv4)
    conv4 = tf.keras.layers.ELU(name='elu4')(conv4)
    pool4 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), name='pool4')(conv4)
    
    conv5 = tf.keras.layers.Conv2D(48, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal',
                                   kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv5')(pool4)
    conv5 = tf.keras.layers.BatchNormalization(axis=3, momentum=0.99, name='bn5')(conv5)
    conv5 = tf.keras.layers.ELU(name='elu5')(conv5)
    pool5 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), name='pool5')(conv5)
    
    conv6 = tf.keras.layers.Conv2D(48, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal',
                                   kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv6')(pool5)
    conv6 = tf.keras.layers.BatchNormalization(axis=3, momentum=0.99, name='bn6')(conv6)
    conv6 = tf.keras.layers.ELU(name='elu6')(conv6)
    pool6 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), name='pool6')(conv6)
    
    conv7 = tf.keras.layers.Conv2D(32, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal',
                                   kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='conv7')(pool6)
    conv7 = tf.keras.layers.BatchNormalization(axis=3, momentum=0.99, name='bn7')(conv7)
    conv7 = tf.keras.layers.ELU(name='eltf.keras.layers.u7')(conv7)
    ...
    # We precidt `n_classes` confidence values for each box, hence the `classes` predictors have depth `n_boxes *
    # n_classes`
    # We predict 4 box coordinates for each box, hence the `boxes` predictors have depth `n_boxes * 4`
    # Output shape of `classes`: `(batch, height, width, n_boxes * n_classes)`
    classes4 = tf.keras.layers.Conv2D(n_boxes[0] * n_classes, (3, 3), strides=(1, 1), padding="same",
                                      kernel_initializer='he_normal',
                                      kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='classes4')(conv4)
    classes5 = tf.keras.layers.Conv2D(n_boxes[1] * n_classes, (3, 3), strides=(1, 1), padding="same",
                                      kernel_initializer='he_normal',
                                      kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='classes5')(conv5)
    classes6 = tf.keras.layers.Conv2D(n_boxes[2] * n_classes, (3, 3), strides=(1, 1), padding="same",
                                      kernel_initializer='he_normal',
                                      kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='classes6')(conv6)
    classes7 = tf.keras.layers.Conv2D(n_boxes[3] * n_classes, (3, 3), strides=(1, 1), padding="same",
                                      kernel_initializer='he_normal',
                                      kernel_regularizer=tf.keras.regularizers.l2(l2_reg), name='classes7')(conv7)
    # Output shape of `boxes`: `(batch, height, width, n_boxes * 4)`
    boxes4 = tf.keras.layers.Conv2D(n_boxes[0] * 4, (3, 3), strides=(1, 1), padding="same",
                                    kernel_initializer='he_normal', kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                    name='boxes4')(conv4)
    boxes5 = tf.keras.layers.Conv2D(n_boxes[1] * 4, (3, 3), strides=(1, 1), padding="same",
                                    kernel_initializer='he_normal', kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                    name='boxes5')(conv5)
    boxes6 = tf.keras.layers.Conv2D(n_boxes[2] * 4, (3, 3), strides=(1, 1), padding="same",
                                    kernel_initializer='he_normal', kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                    name='boxes6')(conv6)
    boxes7 = tf.keras.layers.Conv2D(n_boxes[3] * 4, (3, 3), strides=(1, 1), padding="same",
                                    kernel_initializer='he_normal', kernel_regularizer=tf.keras.regularizers.l2(l2_reg),
                                    name='boxes7')(conv7)
    
    # Generate the anchor boxes
    # Output shape of `anchors`: `(batch, height, width, n_boxes, 8)`
    anchors4 = keras_layer_AnchorBoxes.AnchorBoxes(img_height, img_width, this_scale=scales[0], next_scale=scales[1],
    ...
    anchors5 = keras_layer_AnchorBoxes.AnchorBoxes(img_height, img_width, this_scale=scales[1], next_scale=scales[2],
    ...
    anchors6 = keras_layer_AnchorBoxes.AnchorBoxes(img_height, img_width, this_scale=scales[2], next_scale=scales[3],
    ...
    anchors7 = keras_layer_AnchorBoxes.AnchorBoxes(img_height, img_width, this_scale=scales[3], next_scale=scales[4],
    ...
    classes4_reshaped = tf.keras.layers.Reshape((-1, n_classes), name='classes4_reshape')(classes4)
    classes5_reshaped = tf.keras.layers.Reshape((-1, n_classes), name='classes5_reshape')(classes5)
    classes6_reshaped = tf.keras.layers.Reshape((-1, n_classes), name='classes6_reshape')(classes6)
    classes7_reshaped = tf.keras.layers.Reshape((-1, n_classes), name='classes7_reshape')(classes7)
    # Reshape the box coordinate predictions, yielding 3D tensors of shape `(batch, height * width * n_boxes, 4)`
    # We want the four box coordinates isolated in the last axis to compute the smooth L1 loss
    boxes4_reshaped = tf.keras.layers.Reshape((-1, 4), name='boxes4_reshape')(boxes4)
    boxes5_reshaped = tf.keras.layers.Reshape((-1, 4), name='boxes5_reshape')(boxes5)
    boxes6_reshaped = tf.keras.layers.Reshape((-1, 4), name='boxes6_reshape')(boxes6)
    boxes7_reshaped = tf.keras.layers.Reshape((-1, 4), name='boxes7_reshape')(boxes7)
    # Reshape the anchor box tensors, yielding 3D tensors of shape `(batch, height * width * n_boxes, 8)`
    anchors4_reshaped = tf.keras.layers.Reshape((-1, 8), name='anchors4_reshape')(anchors4)
    anchors5_reshaped = tf.keras.layers.Reshape((-1, 8), name='anchors5_reshape')(anchors5)
    anchors6_reshaped = tf.keras.layers.Reshape((-1, 8), name='anchors6_reshape')(anchors6)
    anchors7_reshaped = tf.keras.layers.Reshape((-1, 8), name='anchors7_reshape')(anchors7)
    
    # Concatenate the predictions from the different layers and the assosciated anchor box tensors
    # Axis 0 (batch) and axis 2 (n_classes or 4, respectively) are identical for all layer predictions,
    # so we want to concatenate along axis 1
    # Output shape of `classes_concat`: (batch, n_boxes_total, n_classes)
    classes_concat = tf.keras.layers.Concatenate(axis=1, name='classes_concat')([classes4_reshaped,
                                                                                 classes5_reshaped,
                                                                                 classes6_reshaped,
                                                                                 classes7_reshaped])
    
    # Output shape of `boxes_concat`: (batch, n_boxes_total, 4)
    boxes_concat = tf.keras.layers.Concatenate(axis=1, name='boxes_concat')([boxes4_reshaped,
                                                                             boxes5_reshaped,
                                                                             boxes6_reshaped,
                                                                             boxes7_reshaped])
    
    # Output shape of `anchors_concat`: (batch, n_boxes_total, 8)
    anchors_concat = tf.keras.layers.Concatenate(axis=1, name='anchors_concat')([anchors4_reshaped,
                                                                                 anchors5_reshaped,
                                                                                 anchors6_reshaped,
                                                                                 anchors7_reshaped])
    ...
    classes_softmax = tf.keras.layers.Activation('softmax', name='classes_softmax')(classes_concat)
    
    # Concatenate the class and box coordinate predictions and the anchors to one large predictions tensor
    # Output shape of `predictions`: (batch, n_boxes_total, n_classes + 4 + 8)
    predictions = tf.keras.layers.Concatenate(axis=2, name='predictions')(
    ...
    if return_predictor_sizes:
        ...
        predictor_sizes = np.array([classes4.int_shape()[1:3],
                                    classes5.int_shape()[1:3],
                                    classes6.int_shape()[1:3],
                                    classes7.int_shape()[1:3]])
