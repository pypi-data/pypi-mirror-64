...
from twomartens.masterthesis.ssd_keras.data_generator import \
    object_detection_2d_image_boxes_validation_utils as validation_utils
...
class DataGenerator:
    ...
    def __init__(self,
        ...
        self.images_dirs = None
        self.annotations_dirs = None
        self.annotations_filenames = None
        self.image_set_filenames = None
        self.classes = None
        self.cats_to_names = None
        self.classes_to_names = None
        self.cats_to_classes = None
        self.classes_to_cats = None
        ...
        # Note that it does not contain the actual image files themselves. This list is one of the outputs of the
        # parser methods.
        ...
        if image_ids is not None:
            if isinstance(image_ids, str):
                ...
                raise ValueError(
                    "`image_ids` must be either a Python list/tuple or a string representing the path to a pickled "
                    "file containing a list/tuple. The value you passed is neither of the two.")
        else:
            self.image_ids = None
    ...
    def parse_csv(self,
        ...
        with open(self.labels_filename, newline='') as csvfile:
            ...
            for row in csvread:  # For every line (i.e for every bounding box) in the CSV file...
                if self.include_classes == 'all' or int(row[self.input_format.index(
                        'class_id')].strip()) in self.include_classes:  # If the class_id is among the classes that are
                    # to be included in the dataset...
                    box = [row[self.input_format.index(
                        'image_name')].strip()]  # Store the box class and coordinates here
                    # content to `box`
                    for element in self.labels_output_format:  # For each element in the output format (where the
                        # elements are the class ID and the four box coordinates)...
                        box.append(int(row[self.input_format.index(
                            element)].strip()))  # ...select the respective column in the input format and append it
                        # to `box`.
                    data.append(box)
        ...
        current_file = str(data[0][0])  # The current image for which we're collecting the ground truth boxes
        current_image_id = str(data[0][0]).split('.')[
        ...
        for i, box in enumerate(data):
            ...
            if box[0] == current_file:  # If this box (i.e. this line of the CSV file) belongs to the current image file
                ...
                current_file = str(box[0])
                current_image_id = str(box[0]).split('.')[0]
                current_labels.append(box[1:])
                if i == len(data) - 1:  # If this is the last line of the CSV file
                    if random_sample:  # In case we're not using the full dataset, but a random sample of it.
                        p = np.random.uniform(0, 1)
    ...
    def parse_xml(self,
                  images_dirs: Sequence[str],
                  image_set_filenames: Sequence[str],
                  annotations_dirs: Optional[Sequence[str]] = None,
                  ...
                  verbose: bool = True):
        """
        This is an XML parser for the Pascal VOC datasets. It might be applicable to other datasets with minor
        changes to
        ...
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            ...
            for image_id in it:
                ...
                if annotations_dir is not None:
                    ...
                    folder = soup.folder.text  # In case we want to return the folder in addition to the image file
                    # name. Relevant for determining which dataset an image belongs to.
                    ...
                    for obj in objects:
                        ...
                        if (not self.include_classes == 'all') and (class_id not in self.include_classes):
                            continue
    ...
    def parse_json(self,
        ...
        Returns:
            None by default, optionally lists for whichever are available of images, image filenames, labels and
            image IDs.
        """
        self.images_dirs = images_dirs
    ...
    def generate(self,
        ...
        Arguments:
            ...
            returns (set, optional): A set of strings that determines what outputs the generator yields. The
            generator's output
                is always a tuple that contains the outputs specified in this set and only those. If an output is not
                available,
                it will be `None`. The output tuple can contain the following outputs according to the specified
                keyword strings:
                ...
                * 'matched_anchors': Only available if `labels_encoder` is an `SSDInputEncoder` object. The same as
                'encoded_labels',
                    but containing anchor box coordinates for all matched anchor boxes instead of ground truth
                    coordinates.
                ...
                * 'processed_labels': The processed, but not yet encoded labels. This is a list that contains for each
                    batch image a Numpy array with all ground truth boxes for that image. Only available if ground
                    truth is available.
                ...
                * 'evaluation-neutral': A nested list of lists of booleans. Each list contains `True` or `False` for
                every ground truth
                    bounding box of the respective image depending on whether that bounding box is supposed to be
                    evaluation-neutral (`True`)
                    or not (`False`). May return `None` if there exists no such concept for a given dataset. An
                    example for
                    evaluation-neutrality are the ground truth boxes annotated as "difficult" in the Pascal VOC
                    datasets, which are
                    usually treated to be neutral in a model evaluation.
                * 'inverse_transform': A nested list that contains a list of "inverter" functions for each item in
                the batch.
                    These inverter functions take (predicted) labels for an image as input and apply the inverse of
                    the transformations
                    that were applied to the original image to them. This makes it possible to let the model make
                    predictions on a
                    transformed image and then convert these predictions back to the original image. This is mostly
                    relevant for evaluation: If you want to evaluate your model on a dataset with varying image sizes,
                    then you are forced to
                    transform the images somehow (e.g. by resizing or cropping) to make them all the same size. Your
                    model will then
                    predict boxes for those transformed images, but for the evaluation you will need predictions with
                    respect to the
                    original images, not with respect to the transformed images. This means you will have to
                    transform the predicted
                    box coordinates back to the original image sizes. Note that for each image, the inverter
                    functions for that
            ...
            keep_images_without_gt (bool, optional): If `False`, images for which there aren't any ground truth boxes
                before any transformations have been applied will be removed from the batch. If `True`, such images
                will be kept in the batch.
            degenerate_box_handling (str, optional): How to handle degenerate boxes, which are boxes that have `xmax
            <= xmin` and/or
                `ymax <= ymin`. Degenerate boxes can sometimes be in the dataset, or non-degenerate boxes can become
                degenerate after they were processed by transformations. Note that the generator checks for
                degenerate boxes
                after all transformations have been applied (if any), but before the labels were passed to the
                `label_encoder` (if one was given).
                Can be one of 'warn' or 'remove'. If 'warn', the generator will merely print a warning to let you
                know that there are degenerate boxes in a batch. If 'remove', the generator will remove degenerate
                boxes from the batch silently.
        ...
        if self.labels is None:
            ...
            if any([ret in returns for ret in ['encoded_labels', 'matched_anchors']]):
                warnings.warn(
                    "Since no label encoder was given, 'encoded_labels' and 'matched_anchors' aren't possible "
                    "returns, " +
                    "but you set `returns = {}`. The impossible returns will be `None`.".format(returns))
        elif not isinstance(label_encoder, ssd_input_encoder.SSDInputEncoder):
            if 'matched_anchors' in returns:
                warnings.warn(
                    "`label_encoder` is not an `SSDInputEncoder` object, therefore 'matched_anchors' is not a "
                    "possible return, " +
                    "but you set `returns = {}`. The impossible returns will be `None`.".format(returns))
        ...
        if degenerate_box_handling == 'remove':
            box_filter = validation_utils.BoxFilter(check_overlap=False,
        ...
        while True:
            ...
            batch_items_to_remove = []  # In case we need to remove any images from the batch, store their indices in
            # this list.
                ...
                if transformations:
                    ...
                    for transform in transformations:
                        ...
                        if not (self.labels is None):
                            ...
                            if batch_x[i] is None:  # In case the transform failed to produce an output image,
                ...
                if self.labels is not None:
                    ...
                    if np.any(batch_y[i][:, xmax] - batch_y[i][:, xmin] <= 0) or np.any(
                        ...
                        if degenerate_box_handling == 'warn':
                            warnings.warn(
                                "Detected degenerate ground truth bounding boxes for batch item {} with bounding "
                                "boxes {}, ".format(
                                    i, batch_y[i]) +
                                ...
                                "result in degenerate ground truth boxes, or that you are parsing the ground truth in "
                                "the wrong coordinate format." +
            ...
            if label_encoder is not None and self.labels is not None:
                
                if ('matched_anchors' in returns) and isinstance(label_encoder, ssd_input_encoder.SSDInputEncoder):
    ...
    def save_dataset(self,
        ...
        if eval_neutral_path is not None:
