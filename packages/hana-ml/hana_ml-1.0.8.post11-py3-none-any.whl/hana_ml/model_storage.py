    ...
    def save_model(self, model, if_exists='upgrade'):
        ...
        if model_exists:
            ...
            if if_exists == 'upgrade':
                ...
                model_id = {'name': name, 'version': version}
        # Checks the current connection_context is equal to model.connection_context
        if self.connection_context is not model.conn_context:
            raise ModelStorageError('Model Storage must use the same connection than the model')
