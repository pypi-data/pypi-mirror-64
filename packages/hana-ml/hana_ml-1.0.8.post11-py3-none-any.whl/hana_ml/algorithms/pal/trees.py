...
class _DecisionTreeBase(PALBase):#pylint: disable=too-many-instance-attributes
    ...
    def _fit(self, data, key=None, features=None, label=None, categorical_variable=None):
        ...
        self.decision_rules_ = self.conn_context.table(rules_tbl) if self.output_rules else None
        self._confusion_matrix_ = self.conn_context.table(cm_tbl)
        self.stats_ = self.conn_context.table(stats_tbl)
        self.cv_ = self.conn_context.table(cv_tbl)
...
class DecisionTreeRegressor(_DecisionTreeBase):
    ...
    def __init__(self,#pylint:disable=too-many-branches, too-many-statements, too-many-locals
        ...
        super(DecisionTreeRegressor, self).__init__(conn_context,
                                                    algorithm=algorithm,
    ...
    def fit(self, data,
        ...
        if self.param_range is not None:
            param_rows.extend([(
                ...
                None,
                None,
                str(range_)) for cvparam, range_ in self.param_range])
        ...
        try:
            #self._materialize(data_tbl, data)
            #self._create(ParameterTable(param_tbl).with_data(param_rows))
            #print(self.conn_context.table(param_tbl).collect())
            #self._create(Table(model_tbl, model_spec))
            #self._create(Table(var_importance_tbl, var_importance_spec))
            #self._create(Table(cm_tbl, cm_spec))
            #self._create(Table(stats_tbl, stats_spec))
            #self._create(Table(cv_tbl, cv_spec))
            self._call_pal_auto("PAL_HGBT", data_, ParameterTable(param_tbl).with_data(param_rows), *out_tables)
        except dbapi.Error as db_err:
