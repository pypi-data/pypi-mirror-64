...
class ModelSavingServices(object): #pylint: disable=useless-object-inheritance
    ...
    def _load_model(cls, connection_context, name, version, js_str):
        ...
        model._decode(name=name, version=version, js_str=js_str)
        return model
    ...
    def _decode(self, name, version, js_str):
        """
    ...
    def _get_model_table_names(self):
        """
        Returns a dictionary mapping the model python attribute and the model table name
    ...
    def _save_model_tables(self, schema, model_table_names):
        ...
        """
        if self._is_APL():
            for att_name in model_table_names:
                # artifact_df = getattr(self, 'model_', None)
                table_name = model_table_names[att_name]
                artifact_df = getattr(self, att_name, None)
                # Columns to be saved
    ...
    def _load_model_tables(self, schema_name, model_table_names, name, version):
        """
