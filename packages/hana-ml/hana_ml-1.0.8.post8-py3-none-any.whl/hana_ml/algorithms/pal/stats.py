...
def chi_squared_goodness_of_fit(conn_context, data, key, observed_data=None, expected_freq=None):
    ...
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'chi_squared_goodness_of_fit')
...
def chi_squared_independence(conn_context, data, key, observed_data=None,#pylint: disable=too-many-locals,
    ...
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'chi_squared_independence')
...
def ttest_1samp(conn_context, data, col=None, mu=0, test_type='two_sides', conf_level=0.95):#pylint: disable=too-many-arguments, invalid-name
    ...
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'ttest_1samp')

    return _ttest_base(conn_context, data_, mu=mu, test_type=test_type, conf_level=conf_level)
...
def ttest_ind(conn_context, data, col1=None, col2=None, mu=0, test_type='two_sides',#pylint: disable=too-many-arguments, invalid-name
    ...
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'ttest_ind')

    return _ttest_base(conn_context, data_, mu=mu, test_type=test_type, paired=False,
...
def ttest_paired(conn_context, data, col1=None, col2=None, mu=0, test_type='two_sides',#pylint: disable=too-many-arguments, invalid-name
    ...
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'ttest_paired')

    return _ttest_base(conn_context, data_, mu=mu, test_type=test_type, paired=True,
...
def f_oneway(conn_context, data, group=None, sample=None,
    ...
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'f_oneway')
...
def f_oneway_repeated(conn_context, data, subject_id, measures=None,
    ...
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'f_oneway_repeated')
...
def univariate_analysis(conn_context, data,
    ...
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'univariate_analysis')
...
def covariance_matrix(conn_context, data, cols=None):
    ...
    """
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'covariance_matrix')
...
def pearsonr_matrix(conn_context, data, cols=None):
    ...
    """
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'pearsonr_matrix')
...
def iqr(conn_context, data, key, col=None, multiplier=None):
    ...
    >>> res, stat = iqr(conn_context=conn, data=df, key='ID', col='VAL', multiplier=1.5)
    >>> res.collect()
             ID  IS_OUT_OF_RANGE
    0    P1                0
    1    P2                0
    ...
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'Stats', 'iqr')
...
def wilcoxon(conn_context, data, col=None, mu=None, test_type=None, correction=None):#pylint: disable=invalid-name
    '''
    ...
    >>> res = wilcoxon(cc, df, mu=40, test_type='two_sides', correction=true)
...
def median_test_1samp(conn_context, data, col=None, mu=None, test_type=None,#pylint: disable=invalid-name
    ...
    >>> res = onesample_median_test(cc, df, mu=40, test_type='two_sides')
...
def grubbs_test(conn_context, data, key, col=None, method=None, alpha=None):
    '''
    ...
    >>> res, stats = grubbs_test(cc, data, key='ID', method='one_side_max', alpha=0.2):
    ...
    try:
        ...
        try_drop(conn_context, tables)
        raise
    return conn_context.table(res_tbl), conn_context.table(stats_tbl)

def entropy(conn_context, data, col=None,
    ...
    >>> res1, res2 = entropy(cc, data, col=['TEMP','WINDY'],
    ...
    try:
        ...
        try_drop(conn_context, tables)
        raise
    return conn_context.table(res1_tbl), conn_context.table(res2_tbl)

def condition_index(conn_context, data, key=None, col=None, scaling=True,
    ...
    >>> res, stats = condition_index(cc, data,  key='ID', scaling=True,
    ...
    try:
        ...
        try_drop(conn_context, tables)
        raise
    return conn_context.table(res_tbl), conn_context.table(helper_tbl)

def ftest_equal_var(conn_context, data_x, data_y, test_type=None):
    '''
    ...
    >>> res = ftest_equal_var(cc, data_x, data_y, test_type='two_sides'):
    ...
    try:
        ...
        logger.exception(str(db_err))
        try_drop(conn_context, res_tbl)
        raise
    return conn_context.table(res_tbl)

def factor_analysis(conn_context, data, key, factor_num, col=None,
    ...
    >>> res = factor_analysis(cc, data, key='ID', factor_num=2,
                              rotation='promax',
                              matrix='correlation'):
    ...
    try:
        ...
        try_drop(conn_context, tables)
        raise
    return conn_context.table(eigen_tbl), conn_context.table(var_explanation_tbl), conn_context.table(communalities_tbl), conn_context.table(loadings_tbl), conn_context.table(rotated_loadings_tbl), conn_context.table(structure_tbl), conn_context.table(rotation_tbl), conn_context.table(factor_correlation_tbl), conn_context.table(score_model_tbl), conn_context.table(scores_tbl), conn_context.table(stats_tbl)#pylint: disable=line-too-long

def kaplan_meier_survival_analysis(conn_context, data, event_indicator=None, conf_level=None):
    '''
    ...
    >>> survival_estimates, res, stats = kaplan_meier_survival_analysis(cc, data):
    ...
    try:
        ...
        try_drop(conn_context, tables)
        raise
    return conn_context.table(survival_estimates_tbl), conn_context.table(res_tbl), conn_context.table(stats_tbl)#pylint: disable=line-too-long


def _distr_table(conn_context, distr_info, unique_id):
    """
    ...
    create(conn_context, Table(distr_tbl, distr_spec).with_data(distr_data))
    return distr_tbl

def cdf(conn_context, data, distr_info, col=None, complementary=False):
    '''
    ...
    data_ = data[[col]]
    unique_id = str(uuid.uuid1()).replace('-', '_').upper()
    distr_tbl = _distr_table(conn_context, distr_info, unique_id)
    tables = ['RESULT']
    tables = ["#CDF_{}_TBL_{}".format(tbl, unique_id) for tbl in tables]
    res_tbl = tables[0]
    ...
    try:
        call_pal_auto(conn_context,
                      ...
                      data_,
                      conn_context.table(distr_tbl),
                      ParameterTable().with_data(param_rows),
                      *tables)
    except dbapi.Error as db_err:
        logger.exception(str(db_err))
        try_drop(conn_context, [distr_tbl, res_tbl])
        raise
    return conn_context.table(res_tbl)

def quantile(conn_context, data, distr_info, col=None, complementary=False):
    '''
...
def distribution_fit(conn_context, data, distr_type, optimal_method=None, censored=False):
    '''
    ...
    >>> res, stats = distribution_fit(cc, data, distr_type, optimal_method='maximum_likelihood')
