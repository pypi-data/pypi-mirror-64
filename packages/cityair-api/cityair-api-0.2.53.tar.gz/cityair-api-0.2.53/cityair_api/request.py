...
class CityAirRequest:
    ...
    def _expand_log_item(self, log_item: dict, extract_re) -> dict:
        ...
        log_item.update(msg=extract_re.search(log_item['msg']).group(0))
        return log_item
    ...
    def get_logs(self, serial_number: str, type: str = "packet",
        ...
        extract_pattern: str, default {LOG_EXTRACT_PATTERN}
        ...
        if type == 'packet':
            ...
            raise ValueError("type arg should one of the ('packet', "
                             "'checkinfo', 'custom')")
        filter_re = re.compile(filter_pattern or extract_pattern)
        extract_re = re.compile(extract_pattern)

        filter_ = {'AppSenderIds': app_sender_ids,
                   'BeginDate': to_date(start_date, str=True),
                   'EndDate': to_date(finish_date, str=True),
                   'EventTypeIds': [],
        ...
        for log_item in full_items:
            if to_include_date:
                yield to_date(log_item['Time']), log_item['msg']
            else:
                yield log_item['msg']
