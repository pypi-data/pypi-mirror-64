...
def label_points_polygon(poly_df, poly_geometry, point_df, point_geometry, label):
    # Label 311 Data with ACS Boundary IDs
    labels = {}

    for index, point in point_df.iterrows():
        for number, polygon in poly_df.iterrows():
            if pd.notna(polygon[poly_geometry]) and pd.notna(point[point_geometry]):
                if polygon[poly_geometry].contains(point[point_geometry]):
