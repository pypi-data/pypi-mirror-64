...
class Dataset:
    ...
    def create_url_options(self, query_list, kwargs):
        ...
        for num_query, query in enumerate(query_list):
            ...
            if operation in ['LT', 'EQ', 'GT', 'LTE', 'GTE']:
                ...
                kwargs.update({
                    'filter[' + str(filter_counter) + '][columnAlias]':
                        "|".join(query.name_list),
                    'filter[' + str(filter_counter) + '][data][type]': 'numeric',
                    'filter[' + str(filter_counter) + '][data][value]':
                        "|".join(query.value_list),
                    'filter[' + str(filter_counter) + '][data][comparison]': operation
                })
                filter_counter += 1
    ...
    def build_output_objects(self, output_list):
    ...
    def scan_limit_to_nb_res_max(self, nbr_results, url, temp_url,
        ...
        if 0 < limit_to_nb_res_max < kwargs['limit']:
            ...
            url = self.url + "/records" + '?' + urlencode(kwargs) + "&" + temp_url
