...
class ClientMedoc:
    ...
    Attributes defined here:
        ...
        detector: instrument detectors of the client
    ...
    def get_dates_optim(self, dates=None):
        ...
        if self.dates[1] <= self.dates[0]:
            raise ValueError("Error in search():\nd1=%s\nd2=%s\nfor "
                             ...
                             (self.dates[1].strftime("%Y-%m-%dT%H:%M:%S"),
                              self.dates[2].strftime("%Y-%m-%dT%H:%M:%S")))
        return dates_optim
        ...
        if series is not None:
            if self.server.startswith('http://idoc-medoc'):
                if series.startswith('aia'):
                    return SdoDataset(self.server + "/" + cfg.SDO_AIA_DATASET_ID)
                elif series.startswith('hmi'):
                    return SdoDataset(self.server+"/"+cfg.SDO_HMI_DATASET_ID)
            elif self.server.startswith('http://localhost'):
                if series.startswith('aia'):
                    return SdoDataset(self.server+"/"+cfg.AIA_LEV1_DATASET_ID)
                elif series.startswith('hmi'):
                    return SdoDataset(self.server+"/"+cfg.SDO_HMI_DATASET_ID)
            elif self.server.startswith('http://medoc-sdo'):
                return InstrumentDataset(self.server+"/"+cfg.SDO_DATASET_ID)
            else:
                raise ValueError(self.server + " is not known")
        else:
    ...
    def get_data_list(self, data_result):
        ...
        if len(data_result) != 0:
            for i, data in enumerate(data_result):
                data_list.append(self.dataset_data_class(data))
