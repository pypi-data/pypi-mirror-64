...
class SdoClientMedoc(ClientMedoc):
    ...
    def __init__(self):
        ...
        self.primary_key = "sunum"
        self.plugin_id = "plugin02"

    def get_class_name(self):
        """Return the class name in string format."""
        return 'SdoClientMedoc'

    def check_wave(self, waves=None, series=None):
        ...
        if self.waves is None:
            if series.startswith('hmi'):
                ...
                stdout.write(out_msg %
                             ('aia.lev1', cfg.AIA_ALLOWED_WAVE_LIST))
    ...
    def check_series(self, series, waves):
        ...
        if self.series not in cfg.SDO_ALLOWED_SERIE_LIST:
            raise ValueError("Error in search():\nseries = %s not "
                             "allowed\nseries must be in list %s" %
                             (self.series, cfg.SDO_ALLOWED_SERIE_LIST))
        if self.series.startswith('hmi'):
            ...
            if self.server.startswith('http://medoc-sdo'):
    ...
    def check_cadence(self, cadence):
        ...
        if self.cadence is None:
            if self.series.startswith('aia.lev1'):
                self.cadence = ['1m']
                stdout.write("cadence parameter not specified, default "
                             "value for aia.lev1 is set : cadence = "
                             "[1m]\n")
        ...
        if len(self.cadence) != 1:
            raise ValueError("Error in search():\n%d elements specified"
                             ...
                             "specified and a be list of only one "
    ...
    def search(self, dates=None, waves=None, series=None,
        ...
        wave_list = self.check_wave(waves, series)
        serie_name = self.check_series(series, wave_list)
    ...
    def get_data_primary_key_list(self, data_list):
        ...
        for item in data_list:
            if item.ias_location != '':
                data_primary_key_list.append(getattr(item,
                                                     self.primary_key))
            else:
                stdout.write("the data for recnum %s is not at IAS\n" %
                             str(item.recnum))
        ...
        if len(self.data_list) != 0:
            ...
            if len(count_series_list.keys()) > 1:
                ...
                if self.server is None:
                    self.server = cfg.SITOOLS2_URL
                    stdout.write("server parameter not specified, default"
                                 "value is set : %s\n" % self.server)
    ...
    def get_dataset_target(self):
        ...
        metadata_ds = None
        if self.server.startswith('http://idoc-medoc'):
            if self.series == 'aia.lev1':
                metadata_ds = SdoAiaDataset(self.server + "/" +
                                            cfg.AIA_LEV1_DATASET_ID)
            elif self.series.startswith('hmi'):
                metadata_ds = SdoDataset(self.server + "/webs_" +
                                         self.series + "_dataset")
        elif self.server.startswith('http://medoc-sdo'):
            metadata_ds = SdoAiaDataset(self.server + "/" +
    ...
    def metadata_info(self, series='aia.lev1'):
        ...
        self.check_server()
        metadata_ds = None
        if self.server == 'http://idoc-medoc-test.ias.u-psud.fr':
            metadata_ds = InstrumentDataset(self.server + "/webs_" +
                                            series + "_dataset")
        elif self.server == 'http://medoc-sdo-test.ias.u-psud.fr':
