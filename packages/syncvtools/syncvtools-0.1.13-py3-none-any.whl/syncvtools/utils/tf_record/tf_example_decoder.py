...
from syncvtools.utils._dependencies import is_tf_one

#apatsekin: Using these imports from TF API instead of internal modules (python.)
 #dtypes are aliased to root tf module!!
#array_ops = tf#tf.python.ops.array_ops
#control_flow_ops = tf#tf.python.ops.control_flow_ops
...
if is_tf_one(tf):
  ...
  sparse_tensor_to_dense = sparse_ops.sparse_tensor_to_dense
  sparse_reshape = sparse_ops.sparse_reshape
  from tensorflow.python.ops import math_ops
  #from tensorflow.python.ops import map_fn
  tf_cast = math_ops.cast
else:
  control_flow_ops = tf
  dtypes = tf
  array_ops = tf
  sparse_tensor = tf.sparse
  image_ops = tf.io #tf.python.ops.image_ops
  parsing_ops = tf.io  # tf.python.ops.parsing_ops
  sparse_tensor_to_dense = tf.sparse.to_dense
  sparse_reshape = tf.sparse.reshape
  math_ops = tf.math
  tf_cast = dtypes.cast
map_fn = tf  # tf.python.ops.map_fn




#sparse_ops = tf.python.ops.sparse_ops
    ...
    if self._shape_keys:
      ...
      for k in self._shape_keys:
        ...
        if isinstance(shape_dim, sparse_tensor.SparseTensor):
          shape_dim = sparse_tensor_to_dense(shape_dim)
        shape_dims.append(shape_dim)
      shape = array_ops.reshape(array_ops.stack(shape_dims), [-1])
    if isinstance(tensor, sparse_tensor.SparseTensor):
      if shape is not None:
        tensor = sparse_reshape(tensor, shape)
      tensor = sparse_tensor_to_dense(tensor, self._default_value)
    else:
      if shape is not None:
  ...
  def tensors_to_item(self, keys_to_tensors):
    ...
    if self._shape_key:
      ...
      if isinstance(shape, sparse_tensor.SparseTensor):
        shape = sparse_tensor_to_dense(shape)
    elif self._shape:
      shape = self._shape
    else:
    ...
    ids = tf.cast(indices.values, dtypes.int64)
...
class Image(ItemHandler):
  ...
  def _decode(self, image_buffer, image_format):
    ...
    val1 = math_ops.equal(image_format, 'raw')
    val2 = math_ops.equal(image_format, 'RAW')
    #changing dict to list to be compatible with TF 2.0
    pred_fn_pairs = [(math_ops.logical_or(val1,val2), decode_raw)]
