...
def compute_mt_loss(targets, conditional=False):
    ...
    def compute_loss(output, labels):
        ...
        loss_mask = labels.get('class', torch.ones(batch_size))
        ...
        for k, v in targets.items():
            ...
            if k != 'class':
                if conditional:
                    ...
                    loss = (loss * cond).sum() / cond.sum() if cond.sum() > 0 else torch.tensor(0)
                else:
                    loss = loss.mean()
...
class GradNormBalancing(nn.Module):
    ...
    def forward(self, output, labels):
        ...
        return torch.stack(all_loss), [l.item() for l in all_loss]
