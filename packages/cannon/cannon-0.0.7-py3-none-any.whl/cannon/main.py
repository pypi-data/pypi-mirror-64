...
class Shell(transitions.Machine):
    ...
    def after_CONNECT_cb(self):
        ...
        if (self.log_screen is True) and (self.log_file==''):
            self.child.log_file = sys.stdout

        elif (self.log_screen is False) and (self.log_file!=''):
            error_str = "Cannot overwrite existing file: {}".format(
                self.log_file)
            assert not os.path.isfile(self.log_file), error_str
            self.child.log_file = open(self.log_file, 'w')

        elif (self.log_screen is True) and (self.log_file!=''):
            raise ValueError("log_screen and log_file are mutually exclusive options")
