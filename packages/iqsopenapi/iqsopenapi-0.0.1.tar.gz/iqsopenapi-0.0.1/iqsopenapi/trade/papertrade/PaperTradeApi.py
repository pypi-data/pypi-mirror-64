...
class PaperTradeApi(ITradeApi):
    ...
    def __init__(self,connectionUrl,apiHost,strategyID,orderChangedCallback,orderExecutionCallback):
        """构造函数"""
        super(PaperTradeApi,self).__init__(connectionUrl,apiHost,strategyID,orderChangedCallback,orderExecutionCallback)
    ...
    def __Subscribe(self):
        ...
        req.StrategyKey=self.__strategyID
        obj = req.ToDict()
        ...
        if type == ResponseType.OrderChange.name:
            for item in orders:
                ...
                if self.__orderChangedCallback:
                    self.__orderChangedCallback(item)
        #成交回报
        if type == ResponseType.OrderExec.name:
            for item in orders:
                ...
                if self.__orderExecutionCallback:
                    self.__orderExecutionCallback(item)
    ...
    #    data.OrderType = OrderType(int(orderChange.get("OrderType")))
    #    data.OrderSide = OrderSide(int(orderChange.get("OrderSide")))
    #    data.Offset = Offset(int(orderChange.get("Offset")))
    #    data.OrderTime = datetime.datetime.strptime(orderChange.get("OrderTime"),'%Y%m%d%H%M%S')
    #    data.OldStatus = OrderStatus(int(orderChange.get("OldStatus")))
    #    data.NewStatus = OrderStatus(int(orderChange.get("NewStatus")))
    ...
    #    data.ExecID = orderExecution.get("ExecID")
    ...
    #    data.OrderSide = OrderSide(int(orderExecution.get("OrderSide")))
    #    data.OrderType = OrderType(int(orderExecution.get("OrderType")))
    #    data.Offset = Offset(int(orderExecution.get("Offset")))
    ...
    def SendOrder(self,clientOrderID, symbol, exchange, orderSide, price, quantity, orderType, offset,tag):
        ...
        params = {'symbol':symbol,'exchange':int(exchange),'orderSide':orderSide,'price':price,'quantity':quantity,'orderType':int(orderType),'offset':int(offset),'strategyKey':self.__strategyID,'clientOrderId':clientOrderID,'tag':tag}
    ...
    def GetAssetInfo(self):
        ...
        params = {'StrategyId':self.__strategyID}
        cacheKey = url + "." + self.__strategyID
    ...
    def GetOrders(self):
        ...
        for item in resData:
            ...
            order.StrategyKey = self.__strategyID
            orders.append(order)
        #5分钟缓存
    ...
    def GetPositions(self):
        ...
        for item in resData:
            ...
            pos.StrategyKey = self.__strategyID
            posList.append(pos)
        #5分钟缓存
    ...
    def __wirteError(self,error):
        '''写错误日志'''
        LogRecord.writeLog('PaperTradeApiError',error)

    def __wirteInfo(self,info):
        '''写日志'''
        LogRecord.writeLog('PaperTradeApi',info)
