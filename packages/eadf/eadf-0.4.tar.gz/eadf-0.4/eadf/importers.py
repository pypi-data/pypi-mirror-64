# Copyright 2020 S. Pawar, S. Semper, C. Wagner
...
def fromWidebandAngData(path: str, **kwargs) -> EADF:
    ...
    arrCoEle = (
        np.radians(data["pattern"][0][0][0][0][0][1].flatten()) + np.pi / 2
    )
    arrAzi = np.radians(data["pattern"][0][0][0][0][1][1].flatten())
    ...
    arrFreqUser = kwargs.get("arrFreqUser", 0)
...
def fromNarrowBandAngData(path: str, **kwargs) -> EADF:
    ...
    arrPorInd = data["pattern"][0][0][1][0][2][1] - 1
