...
class MSTPDirector(asyncore.dispatcher, Server, ServiceAccessPoint):
    ...
    def __init__(
        ...
        mstp_lib.start_server(mstp_dir)
        ...
        mstp_lib.set_interface_params(interface_filename, buf, interface_devname)
        mstp_debug_flag =  getattr(self.localDevice, '_mstp_debug', None)
        if mstp_debug_flag is not None:
            dbg_flag = int(mstp_debug_flag)
            print("Setting the debug flag from python")
            mstp_lib.set_dbg_flag(dbg_flag)
