...
class Rediz(RedizConventions):
    ...
    def _set_implementation(self,   names:Optional[NameList]=None,values:Optional[ValueList]=None, write_keys:Optional[KeyList]=None,budgets: Optional[List[int]] = None,
        ...
        confirm_message = {'time':str(datetime.datetime.now()),'write_key':write_key,'number':len(titles or []),'examples':titles[:2] }
        confirm_pipe = self.client.pipeline(transaction=False)
        confirm_pipe.lpush(cnfrms, json.dumps(confirm_message) )
...
# --------------------------------------------------------------------------
    ...
    def _set_scenarios_implementation(self, name, values, write_key, delay=None, delays=None):
        ...
        if len(values)==self.num_predictions and self.is_valid_key(write_key
            ...
            success = all( _close(actual,anticipate) for actual, anticipate in zip_longest(execut, anticipated_execut) )
            warn    = not( all( a1==a2) for a1,a2 in zip_longest(execut,anticipated_execut))

            if success:
                # Confirmation log
                cnfrms = self.confirms_name(write_key=write_key)
                confirmation = {'time':str(datetime.datetime.now()),'type':'prediction','success':success,'name':name,'delays':delays,'values':values[:5]}
                confirm_pipe = self.client.pipeline(transaction=False)
            ...
            if not(success) or warn:
                ...
                error_report = {'time':str(datetime.datetime.now()),'type': 'prediction', 'success': success, 'name': name, 'delays': delays,
