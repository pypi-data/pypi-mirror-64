...
class Rediz(RedizConventions):
    ...
    def donate(self,write_key, password, verbose=True):
        if password==self.get_donation_password():
            len = muid.difficulty(write_key)
            if len>=12:
                self.client.lpush(self.donation_name(len=len),write_key=write_key)
                return {"operation":"donation","success":True,"message":"Thanks, you can view it at","url":self.base_url+"/donations/"+str(len)} if verbose else 1
            else:
                return {"operation":"donation","success":False,"message":"Invalid write key or not long enough"} if verbose else 0
        else:
            return {"operation":"donation","success":False,"message":"Invalid password"} if verbose else 0
    ...
    def get_donations(self, len=None):
        if len is None:
            ...
            return self.client.lrange(self.donation_name(len=len))
