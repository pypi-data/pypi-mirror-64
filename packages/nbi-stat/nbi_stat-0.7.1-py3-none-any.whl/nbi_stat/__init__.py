...
# 
# Generated 2020-04-01 16:55:24.823318 UTC
...
def eval_cdf(f,x,dx=None):
    ...
    if dx is None: dx = diff(x,prepend=0)
    uncdf = cumsum(dx * f(x))
...
def sample_pdf(y,x,cdf):
    ...
    from numpy import isclose,atleast_1d,array
    def find_inter(y):
        if not (0 <= y <= 1):
            raise ValueError(f'Random number {y} not in [0,1]')
        
        i = bi(cdf,y)
        
        if i >= len(cdf):
            raise ValueError(f'Random number {f} beyond end of CDF {i}>{len(cdf)}')
    ...
    return array([find_inter(yy) for yy in atleast_1d(y)])
...
if __doc__ is not None:
    ...
    __doc__ += \
"""
2020-04-01 16:55:24.824439 UTC
"""
