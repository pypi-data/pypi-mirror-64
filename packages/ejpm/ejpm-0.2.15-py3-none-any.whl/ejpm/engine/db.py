...
def merge_db(orig_db, import_db, replace='keep'):
    ...
    for name in import_names:
        ...
        for install in installs:
            ...
            if not orig_db.get_install(name, install[INSTALL_PATH]):
                ...
                make_new_install_active = False
                current_active = orig_db.get_active_install(name)
                if not current_active:
                    # If there is no such installation, make it active
                    make_new_install_active = True
                else:
                    # If we have such installs, but they are not owned
                    if not current_active[IS_OWNED]:
                        make_new_install_active = True

                        # (!) removing current active status
                ...
                copied_install[IS_ACTIVE] = make_new_install_active
