...
from ...utils.plotting import plot_grid, plot, _get_plot_kwargs, _style_subplot
...
class _Simulate(_BaseSimulator):
    ...
    def simulate(self, t_end=None, steps=None, time_points=None, **integrator_kwargs):
        ...
        time, results = super().simulate(
            t_end=t_end, steps=steps, time_points=time_points, **integrator_kwargs,
        )
        self.full_results = None
        return time, results
    ...
    @staticmethod
    def _parameter_scan(
        parameter_value, parameter_name, model, Sim, integ_name, tolerance, y0
    ):
        m = model.copy()
        ...
        try:
            t, y = s.simulate_to_steady_state(tolerance=tolerance)
            return s.get_full_results_array()[-1]
        except ValueError:
            return np.full(len(m.get_all_compounds()), np.NaN)

    def parameter_scan(
        self, parameter_name, parameter_values, tolerance=1e-8, multiprocessing=True
    ):
        ...
        if is_windows_os or not multiprocessing:
            results = map(
                ...
                parameter_values,
                it.repeat(parameter_name),
                it.repeat(self.model),
                it.repeat(self.__class__),
                it.repeat(self.integrator_name),
                it.repeat(tolerance),
                it.repeat(self.y0),
            )
        else:
        ...
        return pd.DataFrame(
            data=results, index=parameter_values, columns=self.model.get_all_compounds()
        )

    def get_full_results_array(self):
        ...
        if self.full_results is None:
            results = self.model.get_full_concentration_dict(
                y=self.results, t=self.time
            )
            del results["time"]
            self.full_results = results
        return np.array(list(self.full_results.values())).T

    def get_full_results_dict(self):
        """Get simulation results and derived compounds.
    ...
    def get_full_results_df(self):
        ...
        if self.full_results is None:
            ...
            del results["time"]
            self.full_results = results
        return pd.DataFrame(data=self.full_results, index=self.time)
    ...
    def get_variable(self, variable):
        ...
        if self.full_results is None:
            ...
            del results["time"]
            self.full_results = results
        return self.full_results[variable]
    ...
    def get_variables(self, variables):
        ...
        if self.full_results is None:
            ...
            del results["time"]
            self.full_results = results
        return self.get_results_df().loc[:, variables].values
    ...
    def plot_grid(
        ...
        time = self.get_time()
        plot_groups = [
            (time, self.get_full_results_df().loc[:, compounds].values,)
    ...
    def plot_fluxes(
        ...
        x = self.get_time()
        y = self.get_fluxes_df().loc[:, rate_names]
        return plot(
            plot_args=(x, y),
            legend=rate_names,
            xlabel=xlabel,
    ...
    def plot_trajectories(
        ...
        ylabel = cpd2 if ylabel is None else ylabel
        _style_subplot(
            ax=ax, xlabel=xlabel, ylabel=ylabel, title=title, grid=grid, kwargs=kwargs,
        )
        if tight_layout:
            fig.tight_layout()
        return fig, ax
