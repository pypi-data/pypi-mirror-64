...
class DatapaneCfg:
    ...
    params: List[SDict] = dc.field(default_factory=list)
    ...
    def __post_init__(self):
        ...
        if self.params:
            ...
            jsonschema.validate(self.params, config_schema)

    @classmethod
    def create(cls, config_file: Path = DATAPANE_YAML, **kw) -> "DatapaneCfg":
        if config_file.exists():
            log.debug(f"Reading datapane confile file at {config_file}")
    ...
    def to_dict(self) -> SDict:
        d = dc.asdict(self)
        # TODO - HACK pending renaming config->params
        d["config"] = d.pop("params")
        return d
