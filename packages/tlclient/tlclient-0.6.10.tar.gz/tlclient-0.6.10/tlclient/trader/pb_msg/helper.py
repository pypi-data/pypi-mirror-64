...
def _rspAccountStr(self):
    return '<a tg={} avail={} withd={} marketV={} curMarr={}>'.format(
...
def _positionInfoStr(self):
    return '<pos-{}-{} tkr={} src={} dir={} mm={} pos={} ydpos={} cost={} frz={} avail={} brw={} i={} lpx={} rr={} pnl={} | {} | {}>'.format(
        message_pb.Exchange.Name(self.exchange), message_pb.SecurityType.Name(
            self.security_type), self.symbol, self.sub_account,
        message_pb.Direction.Name(self.direction), message_pb.MarginMode.Name(
            self.margin_mode), self.position, self.yd_pos, self.pos_cost,
        self.frz_pos, self.avail_pos, self.borrowed, self.interest, self.liq_price, self.risk_rate,
...
def _reqOrderInsertStr(self):
    return '<ord-{} pid={} tg={} exch={} ref={} tkr={} px={} vol={} type={} dir={}>'.format(
        self.order_id, self.parent_id, self.account_id, message_pb.Exchange.read(
            self.exchange), self.order_ref, self.symbol, self.entrust_price, self.entrust_vol,
        message_pb.OrderType.read(
            self.order_type), message_pb.Side.read(self.side)
    )
...
def _rtnOrderStr(self):
    return '<o oid={} ref={} tg={} exch={} ass={} tkr={} tp={} status={} traded={} create_t={} update_t={}>'.format(
        self.order_id, self.order_ref, self.account_id, message_pb.Exchange.Name(
            self.exchange), message_pb.SecurityType.Name(self.security_type),
        self.symbol, message_pb.OrderType.Name(
            self.order_type), message_pb.OrderStatus.Name(self.order_status), self.traded_vol,
        self.create_time.ToDatetime(), self.update_time.ToDatetime()
    )


def _rtnTradeStr(self):
    return '<t tid={} oid={} ref={} tg={} exch={} ass={} tkr={} dir={} tp={} px={} vol={} comm={} t={}>'.format(
        self.trade_ref, self.order_id, self.order_ref, self.account_id, message_pb.Exchange.Name(
            self.exchange), message_pb.SecurityType.Name(self.security_type),
        self.symbol, message_pb.Side.Name(self.side), message_pb.OrderType.Name(
            self.order_type), self.traded_price, self.traded_vol, self.commission,
...
def _rspOrderInfoStr(self):
    return '<ord-{} tg={} exch={} ref={} tkr={} px={} vol={} type={} dir={} status={} vol_traded={}>'.format(
        self.order_id, self.account_id, message_pb.Exchange.Name(
            self.exchange), self.order_ref, self.symbol, self.entrust_price, self.entrust_vol,
        message_pb.OrderType.Name(self.order_type), message_pb.Side.Name(
            self.side), message_pb.OrderStatus.Name(self.order_status), self.traded_vol
    )
