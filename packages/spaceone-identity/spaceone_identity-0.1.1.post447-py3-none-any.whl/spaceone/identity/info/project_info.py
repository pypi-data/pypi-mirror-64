...
def ProjectMemberInfo(project_map_vo):
    info = {
        'project_info': ProjectInfo(project_map_vo['project'], minimal=True),
        'user_info': UserInfo(project_map_vo['user']),
        'roles': list(map(lambda role: RoleInfo(role, minimal=True), project_map_vo['roles'])),
        'labels': change_list_value_type(project_map_vo['labels'])
    }
