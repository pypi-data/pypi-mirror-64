...
def __wordData__():
        global __sentiWords__
        word_file=pd.read_csv(path+"words.csv",index_col=0)
        __sentiWords__ ={'posScore':word_file.PosScore ,'negScore':word_file.NegScore ,'score':word_file.Score ,'pos':word_file.Pos}

def __load_data__(key):
    f = Fernet(key)
    f1=open(path+"words.py",'rb')
    f1_data=f1.read()
