...
from azureml.automl.core.shared.exceptions import ConfigException, DataException, InvalidValueException
...
class TimeSeriesInferenceDataset(TimeSeriesDataset):
    ...
    def _check_required_prediction_horizon(X_df, horizon, time_column, target_column):
        if np.any(np.isnan(X_df[target_column])):
            ...
            raise DataException(ForecastingErrors.ERR_NOTHING_TO_PREDICT, has_pii=False)
        if X_df[X_df[time_column] >= forecast_origin].shape[0] > horizon:
            raise DataException(ForecastingErrors.FATAL_MAX_HORIZON_EXCEEDED, has_pii=False)
