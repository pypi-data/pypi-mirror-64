...
def PAU_PYTORCH_A_F(x, weight_numerator, weight_denominator, training):
    ...
    denominator = 1.0
    for idx, w_d in enumerate(weight_denominator):
        denominator = denominator + w_d.mul(xps[i + 1]).abs()
...
def PAU_PYTORCH_B_F(x, weight_numerator, weight_denominator, training):
    ...
    denominator = 1.0
    for idx, w_d in enumerate(weight_denominator):
        denominator = denominator + w_d.mul(xps[i + 1])

    return numerator.div(denominator.abs()).view(x.shape)
