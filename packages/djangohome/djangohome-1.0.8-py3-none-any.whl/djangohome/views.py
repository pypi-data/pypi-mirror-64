...
class HomePageView(ListView):
    ...
    def get_queryset(self):
        post = ArticleModel.objects.published().filter(is_promote=False)
        article = ArticleModelScheme.objects.published().filter(is_promote=False)
    ...
    def get_context_data(self, **kwargs):
        ...
        post_promoted = ArticleModel.objects.promoted().filter(is_trend=False)
        article_promoted = ArticleModelScheme.objects.promoted().filter(is_trend=False)
