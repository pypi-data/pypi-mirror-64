...
algorithms_links = {'HOMFLY-PT': homfly, 'Jones': jones, 'Yamada': yamada, 'Kauffman Bracket': kauffman_bracket,
                    'Kauffman Polynomial': kauffman_polynomial, 'BLM/Ho': blmho}
algorithms_knots = {'Alexander': alexander, 'Conway': conway, 'Jones': jones, 'HOMFLY-PT': homfly, 'Yamada': yamada,
                    'Kauffman Polynomial': kauffman_polynomial, 'BLM/Ho': blmho} # 'Kauffman Bracket': kauffman_bracket}
...
def prepare_link_polynomials_codes():
    ...
    for algorithm in algorithms_links.keys():
        ...
        for code in link_codes:
            print("Calculating", algorithm, code)
            data = import_structure(code)
            polynomials[algorithm][code] = algorithms_links[algorithm](data, chiral=True)
...
def prepare_link_polynomials_real():
    ...
    for algorithm in algorithms_links.keys():
        ...
        for pdb in link_curves.keys():
            ...
            t0 = time()
            polynomials[algorithm][pdb] = algorithms_links[algorithm](link_data_prefix + pdb + '.xyz', tries=50,
                                                                      chiral=True, max_cross=25)
            times[algorithm][pdb] = time() - t0
    print('Results:')
    for algorithm in algorithms_links.keys():
...
def test_links_from_codes():
    ...
    for algorithm in algorithms_links.keys():
        for code in link_codes:
            if algorithm not in achiral:
                assert polynomials[algorithm][code] == code
            else:
                assert polynomials[algorithm][code] == code.replace('-', '').replace('+', '')
    return
...
def test_links_real_data():
    print("Testing the polynomials for links on codes real structures")
    ...
    for algorithm in algorithms_links.keys():
        ...
        for pdb in link_curves.keys():
            ...
            result = polynomials[algorithm][pdb].get(expected, 0)
            if result < 0.5:
                print("Warning! Structure " + pdb + ', knot ' + expected + ' with probability ' +
                      str(result) + ' in algorithm ' + algorithm)
            assert result > 0
    return
