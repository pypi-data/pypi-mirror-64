...
def alexander(input_data, closure=Closure.TWO_POINTS, tries=200, boundaries=None,
    ...
    Args:
        ...
        closure (str, optional):
                ...
                parameters of the :ref:`doc_Closure`_ class.
...
# BG:  Albo nie rozumiem o co chodzi, albo już to zostało zrobione. Która opcja?
def gln(input_data, chain2_data=None, chain1_boundary=(-1, -1),
        chain2_boundary=(-1, -1), mode = GlnMode.BASIC,
        max_density=-1, ave_tries=200, matrix_plot_format=PlotFormat.DONTPLOT,
    ...
    Args:
        ...
        max_density (int, optional):
                ...
                Default: -1
        ave_tries (int, optional):
                in mode GlnMode.AVE: the number of closures performed 
    ...
    return result.calculate(
                ...
                mode=mode, max_density=max_density,
                ave_tries=ave_tries, matrix_plot_format=matrix_plot_format,
                plot_ofile=matrix_plot_fname, output_file=matrix_output_fname,
                output_format=matrix_output_format, precision=precision_output, debug=debug)
...
def find_loops(structure, output_type=OutputType.TOPOLOGY, chain=None, bridges_type=Bridges.SSBOND, 
               model=None):
    """
    ...
    """
    g = Graph(structure)
    if output_type == 'topology':
        generator = g.find_loops(output_type=OutputType.PDcode)
        return list(map(yamada, generator))
    else:
        generator = g.find_loops(output_type=output_type)
        return list(generator)


def find_thetas(structure, output_type=OutputType.TOPOLOGY, chain=None,
                bridges_type=Bridges.SSBOND, model=None):
    """
    ...
    g = Graph(structure)
    if output_type == 'topology':
        generator = g.find_thetas(output_type=OutputType.PDcode)
        return list(map(yamada, generator))
    else:
        generator = g.find_thetas(output_type=output_type)
        return list(generator)


def find_handcuffs(structure, output_type=OutputType.TOPOLOGY, chain=None,
                   bridges_type=Bridges.SSBOND, model=None):
    """
    ...
    g = Graph(structure)
    if output_type == 'topology':
        generator = g.find_handcuffs(output_type=OutputType.PDcode)
        return list(map(yamada, generator))
    else:
        generator = g.find_handcuffs(output_type=output_type)
        return list(generator)
...
def import_structure(structure_name):
    ...
    """
    name_achiral = structure_name.replace('-', '').replace('+', '').replace(' ', '')
    if name_achiral not in PD.keys():
        raise TopolyException('The structure chosen is not available in the local library.')
    code = PD[name_achiral]
    if '+' in structure_name:
        code = Graph(code).invert()
    return code
