...
class Graph:
    def __init__(self, init_data='', chain=None, model=None, bridges=[], breaks=[], bridges_type=Bridges.SSBOND, debug=False):
        ...
        if not data['coordinates'] and not data['pdcode'] and not data['emcode']:
            raise TopolyException('The input data incorrect. Maybe its just missing file?')
    ...
    def find_handcuffs(self, output_type=OutputType.PDcode):
        if not self.coordinates:
            return self.find_handcuffs_abstract(output_type=output_type)
        else:
            return self.find_handcuffs_coords(output_type=output_type)
                ...
                for loop1 in [path for path in find_path(v1, v1, self.spatial_graph)]:
                    ...
                    for loop2 in [path for path in find_path(v2, v2, self.spatial_graph)]:
                        ...
                        if set(loop1 + path + loop2) not in found and len(set(loop1[1:-1]) | set(path[1:-1]) |
                            ...
                            print((cut_arc(loop1), cut_arc(path), cut_arc(loop2)))
