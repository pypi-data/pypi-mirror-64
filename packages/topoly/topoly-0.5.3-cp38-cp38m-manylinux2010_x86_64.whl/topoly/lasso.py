...
#            self.loop_indices = 
    ...
    def lasso_type(self, smoothing=0, precision=0, dens=1,
        ...
        def translate_string_result_to_dictionary(res):
            resD= {"cl": None, "beforeN": None, "beforeC": None, "crossingsN": None, "crossingsC": None, "surface_area": None, "loop_lenght": None, "r_giration": None, "smoothing_iter": 0}
    
            # res = (1) "<[SMOOTH_10]>  L0"  OR
            # res = (2) "<[SMOOTH_10]> 0 1 1 X X +36 XX 0 0 1 X X XX  L0 XX 9.59 34.90 2.23"
            ...
            if len(res.split())<5: #(1)
                ...
                if (lbefore[0][:3]=="[SM"):
                    resD["smoothing_iter"] = int(lbefore[0].split()[0].split("_")[1][:-1])
                resD["beforeN"], resD["beforeC"] = lbefore[-2].split(), lbefore[-1].split()
                resD["crossingsN"], resD["crossingsC"] = lafter[-2].split(), lafter[-1].split()
                resD["cl"] = lcl[0].strip()
                resD["surface_area"] = float(larea[0].split()[0])
                resD["loop_lenght"] = float(larea[0].split()[1])
                resD["r_giration"] = float(larea[0].split()[2])
