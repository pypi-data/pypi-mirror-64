...
class PerfSqlConnection(SqlConnection):
    ...
    def update_lifetime_bw(self, write_bws, read_bws):
        print("update_lifetime_bw")
        max_len = len(write_bws) if len(write_bws) > len(read_bws) else len(read_bws)
        for index in range(max_len):
            write_bw = write_bws[index] if index < len(write_bws) else 0
            read_bw = read_bws[index] if index < len(read_bws) else 0
            self.insert_to_table(PerfSqlConnection.real_time_table, read_iops=0, read_bw=read_bw,
                                 write_iops=0, write_bw=write_bw, temperature=0)
