...
class Fio(LinuxFio):
    ...
    def set_fio_parameters(self, fio_parameters):
        ...
        bw_log = "{}_bw.1.log".format(bw_log)
        return out_put, bw_log
    ...
    def analysis_bw_log(self, bw_log):
        ...
        if os.path.exists(bw_log):
            ...
            try:
                ...
                for result in results:
                    if result[2] == "0":
                        read_logs.append(float(result[1])*1024)
                    elif result[2] == "1":
                        write_logs.append(float(result[1])*1024)
            except Exception as all_exception:
                log.ERR(all_exception)
        return  write_logs, read_logs

    def analysis_result(self, out_put, bw_log, real_time):
        output, result = self.analysis_fio_report(out_put)
        if real_time != "enable":
            self.analysis_bw_log(bw_log)
        return output, result

    def run_main_fio_benchmark(self, parameters):
        import time
        time.sleep(1)
        out_put, bw_log = self.set_fio_parameters(parameters["fio"])
        popen = self.popen_start_fio()
        status = self.collect_prints_util_finished(popen, parameters["real_time"])
        output, result = self.analysis_result(out_put, bw_log, parameters["real_time"])
        return status, output, result
