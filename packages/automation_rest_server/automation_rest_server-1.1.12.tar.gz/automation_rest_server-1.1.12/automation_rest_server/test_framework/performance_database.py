...
class PerfSqlConnection(SqlConnection):
    ...
    def update_lifetime_bw(self, write_bw, read_bw):
        for index in range(len(read_bw)):
            self.insert_to_table(PerfSqlConnection.real_time_table, read_iops=0, read_bw=read_bw[index],
                                 write_iops=0, write_bw=write_bw[index], temperature=0)
