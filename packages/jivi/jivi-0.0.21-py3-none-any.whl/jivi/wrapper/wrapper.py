...
def Wrapper(propnames_to_skip:List[str]=[],skip_init=False):
	...
	def class_decorator(old_class):
		...
		class JivfoWrapperClass(old_class):
			...
			def __repr__(self):
				...
				return f"{self.__class__.__name__}  ----------------------------- {self._jvMeta.class_to_string(self)}"
		JivfoWrapperClass.__name__ = old_class.__name__
