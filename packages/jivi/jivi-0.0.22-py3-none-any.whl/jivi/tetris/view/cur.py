...
from jivi.fs import JDir,JFile
...
class Tetris(B.Tetris):
	...
	def _register_ev(self):
		...
		@self.on_key.register('f2')
		def f(ev):
			dF = JFile(r'C:\temp\dump.json')
			to_write = {k : v().to_dict() for k,v in self.dispData.getData.items()}
			dF.writer.json(to_write)
