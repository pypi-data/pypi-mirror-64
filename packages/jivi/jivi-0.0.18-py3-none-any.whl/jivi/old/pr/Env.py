...
from jivi.pr.objecth.all import *
...
class Env:
	def __init__(self,fp=shortcut_link,file_types=['ininame','pf'],sections=['Startup','Colors','fonts'],temp_dir="C:\\temp\\",reload=0):
	...
	def set_fields_and_tables(self):
		...
		tabs = {}
		for a in ['ttField','ttIndex','ttIndexField','ttFile','ttFont','ttSession']:
			tabs[a[2:].lower()] = jread(r"C:\temp\{a}.json".format(a=a))[a]
		
		self.session = Session(self,tabs)
		
			
		
	def set_fields_and_tables_old(self):
