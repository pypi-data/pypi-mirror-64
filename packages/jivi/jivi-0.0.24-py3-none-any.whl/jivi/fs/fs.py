...
from ..util import is_str_list,String
...
class JDir(JDirFileWrapper):
	...
	def copy(self,fp_to,deep=False):
		dDest        = JDir(fp_to)
		...
		if deep:
			to_copy += [[d,dDest.copyob.down(d.rel(self.fp))] for d in self.reader.dirs_ob(deep=True)]
		...
		for src,dest in to_copy:
			...
			os.system(f'copy "{src.fp}\\*.*" "{dest.fp}"')
	...
	def from_file(cls,fp):
		retval  : JDir= JFile(fp).parent_dir_ob
		return retval
	...
	def random_file(self,length=10,ex="tmp",prefix=""):
		random_part = String.random(length)
		return JFile(self.fp, f"{prefix}{random_part}.{ex}")
	...
	@classmethod
	def ToJDir(cls,unknown:any,alternative=None):
		if unknown is not None:
			if isinstance(unknown,JDir):
				return unknown
			if isinstance(unknown,str):
				return JDir(unknown)
			if is_str_list(unknown):
				return JDir(*unknown)
		if alternative is not None:
			return cls.ToJDir(alternative,alternative=None)
