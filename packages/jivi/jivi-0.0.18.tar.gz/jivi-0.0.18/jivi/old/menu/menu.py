...
def _get_keyname(keyname,quit=False):
...
class Command:
	def __init__(self,name,func=None,desc=None,cmd_keys=None,exit_after:bool=False,with_parameters=False,data=None):
		self.name 			:str		= name
...
def search_filter_default(c:Command,search_input):
	return c.name.lower().find(search_input.lower()) != -1
...
class SearchFilterHelper:
	...
	def fnmatch(cls):
		...
		def ret_val(c:Command,search_input:str):
			return fnmatch.fnmatch(c.name,search_input)
	...
	def default(cls):
		...
		return ret_val
class SearchFilter:
	fnmatch = SearchFilterHelper.fnmatch()
	default = SearchFilterHelper.default()

class Commands(list):
	def __init__(self, *commands:List[Command],max_rows:int=20):

		self.search_filter = search_filter_default
	...
	def set_items_filtered(self):
		def get() -> List[Command]:
			...
			def search_filter(c:Command):
				return self.search_filter(c,self.search_input)
			...
			return list(filter(search_filter,self))
...
class Menu:
	...
	def key(self,keyname):
		keyname = _get_keyname(keyname,quit=True)
	...
	def do_key(self,keyname):
		keyname = _get_keyname(keyname)
		if not keyname:
			return
	...
	def command(self,name,desc=None,cmd_keys=None,exit_after=False,with_parameters=False,data=None):
		def decorator(f):
			self.cmds.append(Command(name,f,desc=desc,cmd_keys=cmd_keys,exit_after=exit_after,with_parameters=with_parameters,data=data))
			return f
		return decorator
