...
class ExportTabularDataModule:
    ...
    def generate_to_sql_dataflow(cls, input_port_dfd: DataFrameDirectory,
        ...
        if cls.column_list_to_be_saved == '*':
            cls.column_list_to_be_saved = [*df_schema]
        else:
        ...
        if cls.column_list_datatable_columns == '*':
            cls.column_list_datatable_columns = cls.column_list_to_be_saved
        else:
        ...
        try:
            ...
            if error_code != '':
                ...
                if 'Could not find datastore' in error_message:
                    ...
                    ErrorMapping.rethrow(export_exceptions, UserError(error_message))
                elif error_code == 'Uncategorized' and any(m in error_message for m in EXPORT_USER_ERROR_GROUP):
                    ErrorMapping.rethrow(export_exceptions, UserError(error_message))
                elif 'Cannot access destination table' in str(error_message):
                    hint_message = 'Table name may not comply with Sql server name convention. Please retry. '
...
def main(args):
    ...
    logger.debug(f'Received argument --column_list_to_be_saved: {args.column_list_to_be_saved}.')
    logger.debug(f'Received argument --datatable_name: {args.datatable_name}.')
    logger.debug(f'Received argument --column_list_datatable_columns: {args.column_list_datatable_columns}.')
    logger.debug(f'Received argument --number_rows_per_operation: {args.number_rows_per_operation}.')
