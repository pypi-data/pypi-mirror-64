...
class LookupModule(LookupBase):
    ...
    def run(self, term, variables=None, **kwargs):
        ...
        :return:
        """
        if self.syspass_client is None:
            raise ReferenceError('syspass client is not found')
        if self.syspass_client.config is None:
            raise ReferenceError('syspass client config is not found')
        if self.syspass_client.config.authToken is None:
            raise TypeError('authToken can not be None')
