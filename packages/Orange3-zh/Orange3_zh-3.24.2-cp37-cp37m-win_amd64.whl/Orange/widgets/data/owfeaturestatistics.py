...
class OWFeatureStatistics(widget.OWWidget):
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, '直方图(Histogram)')
    ...
    def _format_variables_string(self, variables):
        ...
        for var_type_name, var_type in [
                ('个分类', DiscreteVariable),
                ('个数值', ContinuousVariable),
                ('个时间', TimeVariable),
                ('个字符串', StringVariable)
        ]:
            ...
            if var_type_list:
                ...
                agg.append((
                    '%d %s%s' % (len(var_type_list), var_type_name, ['', '(未显示)'][shown]),
                    len(var_type_list)
                ))
        ...
        if len(attrs) > 1:
            var_string = ', '.join(attrs[:-1]) + ' 和 ' + attrs[-1]
        else:
            var_string = attrs[0]
        return plural('%s变量' % var_string, sum(counts))

    def set_info(self):
        if self.data is not None:
            self.info_summary.setText('<b>%s</b>包含%s个具有%s特征的实例' % (
                self.data.name,
                plural('{number}', self.model.n_instances),
