...
def format_summary(summary):
    ...
    if isinstance(summary, ApproxSummary):
        if summary.len.done():
            text += ["{} 个实例".format(summary.len.result())]
        else:
            text += ["~{} 个实例".format(summary.approx_len)]

    elif isinstance(summary, Summary):
        text += ["{} 个实例".format(summary.len)]
    ...
    def format_part(part):
        ...
        if isinstance(part, DenseArray):
            ...
            text = " ({}, 密度 {:.2f}%)"
            tag = "稀疏" if isinstance(part, SparseArray) else "标签"
            total = part.nans + part.non_nans
    ...
    text += [("%s 个特征" % sp(len(summary.domain.attributes)))
    ...
    if not summary.domain.class_vars:
        ...
        if len(summary.domain.class_vars) > 1:
            c_text = "%s结果" % sp(len(summary.domain.class_vars))
        elif summary.domain.has_continuous_class:
            c_text = "连续目标变量"
        else:
            c_text = "具有 %s 个值的离散类" % sp(
                len(summary.domain.class_var.values))
        c_text += format_part(summary.Y)
        text += [c_text]

    text += [("%s 个元属性" % sp(len(summary.domain.metas)))
