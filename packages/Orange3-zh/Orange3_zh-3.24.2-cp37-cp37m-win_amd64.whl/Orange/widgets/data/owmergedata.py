...
class OWMergeData(widget.OWWidget):
    ...
    def __init__(self):
        ...
        self.info_box_data = gui.label(box, self, no_info, box="数据(Data)")
        self.info_box_extra_data = gui.label(box, self, no_info, box="附加数据(Extra Data)")
    ...
    def data_info(data):
        if data is None:
            ...
            return \
                f"{data.name}\n" \
                f"{len(data)} 个实例\n" \
                f"{len(data.domain) + len(data.domain.metas)} 个变量"
