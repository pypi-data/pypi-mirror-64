...
class sf_Manager():
  ...
  def get_opportunities(self):
    query_text = "SELECT CreatedDate, Name, StageName, ExpectedRevenue, Amount, LeadSource, IsWon, " \
                 "IsClosed, Type, Probability FROM Opportunity"
    try:
  ...
  def get_cases(self):
    query_text = "SELECT CreatedDate, Type, Reason, Status, Origin, Subject, Priority, IsClosed, OwnerId, " \
                 "IsDeleted, AccountId FROM Case"
    try:
      query_result = self.sf.query(query_text)
    except SalesforceExpiredSession as e:
      self.login()
