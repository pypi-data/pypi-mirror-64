...
class ZooProject(object):
    ...
    def get_golden_subject_sets(self):
        ...
        for iWorkflow in list(self.workflow_info.keys()):
            try:
    ...
    def get_golden_images(self, workflow=None):
        ...
        if workflow:
            ...
            workflows = list(workflowGoldenSetDict.keys())
    ...
    def get_answers(self, workflow=None):
        ...
        if workflow:
            ...
            workflows = list(self.workflow_info.keys())
...
class GravitySpyProject(ZooProject):
    ...
    def get_level_structure(self, workflow=None, IDfilter=''):
        ...
        for iworkflow in list(workflowDictSubjectSets.keys()):
    ...
    def calculate_confusion_matrices(self):
        ...
        numClasses = len(list(self.get_answers(workflow=7766).values())[0])
    ...
    def determine_level(self, alpha=None):
        ...
        answers_dict = dict((str(v),k) for k,v in list(answers_dict_rev.items()))
        # Determine what indices of the confusion matrix we evaluate for each level based on the answers for that level
        promotion_Level1 = set([answers_dict[answer] for answer in list(answers['1610'].keys()) if answer not in ['NONEOFTHEABOVE']])
        promotion_Level2 = set([answers_dict[answer] for answer in list(answers['1934'].keys()) if answer not in ['NONEOFTHEABOVE']])
        promotion_Level3 = set([answers_dict[answer] for answer in list(answers['1935'].keys()) if answer not in ['NONEOFTHEABOVE']])
        promotion_Level4 = set([answers_dict[answer] for answer in list(answers['7765'].keys()) if answer not in ['NONEOFTHEABOVE']])
        promotion_Level5 = set([answers_dict[answer] for answer in list(answers['7766'].keys()) if answer not in ['NONEOFTHEABOVE']])

        
        level_dict = dict(enumerate(self.workflow_order))
        workflow_level_dict = dict((v, k + 1) for k, v in
                                   list(level_dict.items()))

        promotions = list(zip(list(range(4, -1,-1)), [promotion_Level5, promotion_Level4,
                                           promotion_Level3, promotion_Level2,
                                           promotion_Level1]))

        if alpha is None:
            numClasses = len(list(answers_dict.keys()))
    ...
    def check_level_by_classification(self):
        ...
        workflowLevelDict = dict((v, k + 1) for k,v in levelWorkflowDict.items())
