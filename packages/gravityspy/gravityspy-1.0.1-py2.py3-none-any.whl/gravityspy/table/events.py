...
class Events(GravitySpyTable):
    ...
    def classify(self, path_to_cnn, **kwargs):
        ...
        Returns:
            `Events` table
        """
        if 'event_time' not in list(self.keys()):
        ...
        inputs = list(zip(self['event_time'], self['ifo'],
                     self['gravityspy_id']))
    ...
    def to_glitch_db(self, table='GSMetadata', engine=None, **kwargs):
        ...
        def makelink(x):
            ...
            intermediate_path = '/'.join([_f for _f in str(x.Filename1).split('/') if _f][3:-1])
            if x.ifo == 'L1':
    ...
    def update_scores(self, path_to_cnn, nproc=1, **kwargs):
        ...
        """
        if not all(elem in list(self.keys()) for elem in ['Filename1', 'Filename2',
                                                    'Filename3', 'Filename4']):
    ...
    def determine_workflow_and_subjectset(self, project_info_pickle):
        ...
        """
        if 'ml_confidence' not in list(self.keys()) or 'ml_label' not in list(self.keys()):
        ...
        for label, confidence in zip(self['ml_label'], self['ml_confidence']):
            for iworkflow in ['1610', '1934', '1935', '7765', '7766', '7767']:
                if label in list(workflows_for_each_class[iworkflow].keys()):
