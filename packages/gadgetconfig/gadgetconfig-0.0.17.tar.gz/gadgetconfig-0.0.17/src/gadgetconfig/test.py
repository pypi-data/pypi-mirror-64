...
def systemctl(service_name):
	...
	#return status
	result = subprocess.run(['systemctl', 'status', 'gadget', '--lines', '0'], stdout=subprocess.PIPE)
...
class Tabs:
	...
	def nb_update(self):
		...
		if self.currentID == 0:
			...
			s = sysfs(["/sys/kernel/config/usb_gadget/%s" % (self.tabIDs[self.currentID])], -1, sort=False)
...
class Editor:
	...
	def get_new_device_name(self, file):
		...
		if new_device_name is None:
			return None
		while True:
			new_device_name = simpledialog.askstring("Device Name Exists", "New Device name: ", initialvalue=m.check_device_file(file))
			if new_device_name is None:
				return None
	...
	def gadget_add_button_pressed(self):
		...
		if new_device_name is not None:
			...
			if new_device_name is None:
				return
		print("gadget_add_button_pressed: file: %s %s" % (f, new_device_name), file=sys.stderr)
		try:
			self.m.add_device_file(f, new_device_name=new_device_name)
		except FileExistsError:
			messagebox.showerror(title="Error", message="Gadget Definition for %s already exists!" % (m.check_device_file(f)))
	...
	def tk(self):
		...
		self.tk.title("Gadget USB Device Configuration")
