...
class Transfer:
    ...
    """
        ...
        if self.myOperation == self.OPERATIONS["SEND_DATA"]:
            ...
            if self.myDataType == self.DATA_TYPES['VALIDATED_SEISMIC_DATA_PH5'] and not self.network_id:
                raise Exception("For PH5 data, please provide the extended network ID (eg. --network 3C2019 )")
    ...
    def send_data(self):
        ...
        rsyncmodule="INCOMING_"
        rsync_destination = self.myDirectoryName
        if self.myDataType == self.DATA_TYPES['VALIDATED_SEISMIC_DATA_PH5']:
            rsync_module=rsyncmodule + 'PH5_'+ self.__CONFIG["my resif node"]["my node name"][1]
            rsync_destination = self.network_id
            self.myDirectoryName = self.myDirectoryName + '/'
        else:
            rsync_module=rsyncmodule + self.__CONFIG["my resif node"]["my node name"][1]
