...
class Client:
    ...
    def fit(self,transform_def, keras_model_def, x=None, y=None, non_DP_training=False):
        """ NEW SIGNATURE!
        X and Y are columns
        ...
        request = requests.post(f'{self.base_url}/fit',
            data=cloudpickle.dumps((transform_def, keras_model_def, x, y, non_DP_training)),
            headers={'Content-Type': 'application/octet-stream'}, cookies=self.cookies)
        if request.status_code>200:
            raise Exception(f"Error while training the model. Full Gateway answer was:{request}")
        task_id=request.json()['task']
