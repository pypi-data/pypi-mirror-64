...
class latin_design:
    ...
    def __init__(self, point:gpd.GeoDataFrame, buffer_length:int = None):
        ...
        self.geodesign = gpd.GeoDataFrame(geometry=gpd.GeoSeries(polygonize(unary_union(MultiLineString(lines))))).iloc[[1,3,4,5,7]]
        if self.geodesign.crs == None:
            self.geodesign.crs = {'init': 'epsg:3857'}
    ...
    def __init__(self, gdf_path, length:int = None):
        ...
        for index, row in tqdm(datas.iterrows(), total = datas.shape[0]):
            if self.length is None:
                uid = latin_design(gpd.GeoDataFrame(geometry=gpd.GeoSeries(Point((row['geometry'].x, row['geometry'].y)))), 71).geodesign
            else:
                uid = latin_design(gpd.GeoDataFrame(geometry=gpd.GeoSeries(Point((row['geometry'].x, row['geometry'].y)))), self.length).geodesign
            uid['UID'] = int(index)
...
class composite_union:
    ...
    def __init__(self, data, buffer = None):
        ...
        if self.data.crs == {'init': 'epsg: 4326'} and buffer == None:
            unary_data = gpd.GeoDataFrame(
                geometry=gpd.GeoSeries(unary_union(self.data.geometry).intersection(
                    unary_union(self.data.geometry).buffer(0.0005)))).explode().reset_index().drop(['level_0', 'level_1'],
                                                                                              axis = 1)
            if unary_data.crs == None:
                unary_data.crs = self.data.crs
            self.geodesign = unary_data
            
        elif self.data.crs == {'init': 'epsg: 3857'} and buffer == None:
            unary_data = gpd.GeoDataFrame(
                geometry=gpd.GeoSeries(unary_union(self.data.geometry).intersection(
                    unary_union(self.data.geometry).buffer(50)))).explode().reset_index().drop(['level_0', 'level_1'],
                                                                                              axis = 1)
            if unary_data.crs == None:
                unary_data.crs = self.data.crs
            self.geodesign = unary_data
            
        elif buffer is not None and self.data.crs != None:
            unary_data = gpd.GeoDataFrame(
                geometry=gpd.GeoSeries(unary_union(self.data.geometry).intersection(
                    unary_union(self.data.geometry).buffer(self.buffer)))).explode().reset_index().drop(['level_0', 'level_1'],
                                                                                              axis = 1)
            if unary_data.crs == None:
                unary_data.crs = self.data.crs
            self.geodesign = unary_data
            ...
            unary_data = gpd.GeoDataFrame(
                geometry=gpd.GeoSeries(unary_union(self.data.geometry).intersection(
                    unary_union(self.data.geometry).buffer(0.005)))).explode().reset_index().drop(['level_0', 'level_1'],
                                                                                              axis = 1)
            if unary_data.crs == None:
                unary_data.crs = self.data.crs
            self.geodesign = unary_data
        else:
            self.geodesign = gpd.GeoDataFrame(geometry=gpd.GeoSeries(Polygon(unary_union(self.data.geometry).exterior.coords[:])))
