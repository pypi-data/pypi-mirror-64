...
def rasterRaw(mapId, bands, timestamp, tileId = None, xMin = None, xMax = None, yMin = None, yMax = None, threads = 1, token = None):
    ...
    if len(bands) == 0 and bands[0] == 'label':
        dtype = 'int8'
                        ...
                        if tileX == 'done':
                            ...
                            while retries < 10:
                                try:
                                    ...
                                    loadingBar(original_size - q.qsize() ,original_size - threads)
                                    ...
                                    retries = retries + 1
                                    if retries == 10:
                                        q_result.put({'x_index':x_index, 'y_index':y_index, 'r':np.zeros((256,256,len(bands)))})
...
def rasterVisual(mapId, timestampMin, timestampMax, layerName, xMin= None,xMax= None,yMin=None, yMax=None , tileId=None, downsampled = True, threads = 1, token = None):
    if token != None:
                    ...
                    if tileX == 'done':
                        ...
                        while retries < 10:
                            try:
                                ...
                                retries = retries + 1
                                if retries == 10:
                                    q_result.put({'x_index':x_index, 'y_index':y_index,'r':np.zeros(256,256,4)})
