...
class MovementApp(ShowBase):
    def __init__(self):
        ...
        self.render_pipeline.set_effect(
            render,
            str(pathlib.Path(__file__).parent.joinpath('scene-effect.yaml')),
            {},
            sort=250
        )
        ...
        self.movement_ui = movement_ui = MovementUI(self, display_region)
        self.initialize_ui_in = 20
    ...
    def update_ui(self, task):
        if self.initialize_ui_in > 0:
            self.initialize_ui_in -= 1
            return task.cont

        elif self.initialize_ui_in == 0:
            print('WE ARE GO')
            self.movement_ui.run()
            self.initialize_ui_in -= 1
