...
def order_inspect_docx(order_uid: str, file: str):
    case = case_db.find_one({'order_uid': order_uid})

    with tempfile.TemporaryDirectory() as p:
        doc = vmi.docx_document('生产单')
        ...
        body_table.cell(0, 0).paragraphs[0].text = '生产单\nProduction order'
        ...
        add_row_text(body_table, ['生产单号\nOrder UID', case['order_uid'],
        ...
        designer = role_product_designer_db.find_one({'_id': case['role_product_designer_id']})['name']

        add_row_text(body_table, ['产品设计师\nProduct designer', designer,
        ...
        for kw in info:
            text += '\n{}: {}'.format(kw, info[kw])
        add_row_text(body_table, ['影像输入\nRadiological input', text[1:]], [s_body_center, s_body_left], [1, 3])

        text = '髋臼杯半径：{:.0f} mm'.format(case['cup_radius'])
        text += '\n影像学前倾角：{:.1f}°\n影像学外展角：{:.1f}°'.format(case['cup_RA'], case['cup_RI'])
...
def order_inspect():
    ...
    r = vmi.askButtons(['下载生产单', '接受生产单', '驳回生产单'])
    if r == '下载生产单':
        user = ask_user()
        if not user:
            return False
        ...
        f = '{} {}.pdf'.format(order_uid, time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime()))
        f = pathlib.Path(p) / f
        order_inspect_docx(order_uid, str(f))
    elif r == '接受生产单':
        user = ask_user()
        if not user:
            return False
...
if __name__ == '__main__':
    ...
    tray_icon_menu.addSection('质量检验员')
    tray_icon_menu.addAction('检验生产单').triggered.connect(order_inspect)
    tray_icon_menu.addSection('设备操作员')
    tray_icon_menu.addAction('下载设计文件').triggered.connect(order_download)
    tray_icon_menu.addSection('质量检验员')
    tray_icon_menu.addAction('完成生产单').triggered.connect(order_complete)
    ...
    admin_menu.addAction('更新质量检验员').triggered.connect(csv_update_role_quality_inspector)
