...
def clean_signal(signal, cut_angle=1000.0, filter_angle=5.0, 
...
def demo():
    ...
    plot_raw_signals(Signals, title='demo_measures',
                     flag_show=True)
    
    # clean the signals and make the approximations
    for signal in Signals:
        clean_signal(signal, cut_angle=130.0)
        sigmoid_approximation(signal)
    
    # define the target data
    threshold_torque = 15.0     # Nm
    applied_angle = 50.0        # deg
    angle_tolerance = 5.0       # deg
    lower_torque = 30.0         # Nm
    upper_torque = 45.0         # Nm
    target = TargetType(threshold_torque, applied_angle, 
             angle_tolerance, lower_torque, upper_torque)
...
def create_demo_python(xls_filename):
    ...
    with open("demo.py", 'w') as file:
        ...
        data+= '\n# clean the signals and make the approximations'
        data+= '\nfor signal in Signals:'
        data+= '\n    bs.clean_signal(signal, cut_angle=130.0)'
        data+= '\n    bs.sigmoid_approximation(signal)'
        data+= '\n'
        data+= '\n# define the target data'
        data+= '\nthreshold_torque = 15.0     # Nm'
        data+= '\napplied_angle = 50.0        # deg'
        ...
        data+= '\nlower_torque = 30.0         # Nm'
        data+= '\nupper_torque = 45.0         # Nm'
        data+= '\ntarget = bs.TargetType(threshold_torque, applied_angle,'
        data+= '\n         angle_tolerance, lower_torque, upper_torque)'
        data+= '\n'
        data+= '\n# plot the survey'
