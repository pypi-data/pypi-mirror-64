...
class Elexon(object):
    ...
    def query(self, report, payload, servicetype='xml'):
        ...
        # print(request.url)
        return request.text
        return xmltodict.parse(request.text)['response']
        ...
        for parent in root.findall('./responseBody/responseList/item'):
            # Time_Series_ID = parent.find('timeSeriesID').text
            # Quantity_MW = parent.get('timeSeriesID')
            # print(Time_Series_ID, Quantity_MW)
            settlementDay = parent.find('settlementDay').text
            settlementExchangeRate = parent.find('settlementExchangeRate').text
            dateTimeRetrieved = parent.find('dateTimeRetrieved').text
            print(settlementDay, settlementExchangeRate, dateTimeRetrieved)
    ...
    def PHYBMDATA(self, SettlementDate, SettlementPeriod = '*', BMUnitId = '', BMUnitType = '', LeadPartyName = '', NGCBMUnitName = ''):
        """Physical Data"""
        endpoint = 'PHYBMDATA'
        query_parameters = {
            'SettlementDate': SettlementDate,
