...
def model_update(
    ...
    upload_help(nomitall_url, nomitall_secret, doc)
    icons = doc.pop("icons", {})
    if model_type == "engine":
...
def upload_help(nomitall_url, nomitall_secret, doc):
    help_files = process_help(doc)
    _logger.info("Pushing mds to nomitall @ : {}".format(nomitall_url))
    nomitall_uri = f"{nomitall_url}/engine/upload-md/{doc['uuid']}"
    resp = requests.post(nomitall_uri, files=help_files, auth=KeyAuth(nomitall_secret))
    check_response(resp)
