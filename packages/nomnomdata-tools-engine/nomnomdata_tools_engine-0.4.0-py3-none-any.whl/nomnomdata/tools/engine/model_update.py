...
def model_update(
    ...
    update_model(
        nomitall_url, nomitall_secret, doc, model_type,
    )
...
def update_model(
    nomitall_url, nomitall_secret, model, model_type,
):
    session = NomitallSession(prefix_url=nomitall_url)
    session.auth = KeyAuth(nomitall_secret)
    help_files = {}
    icons = {}
    if model_type == "engine":
        ...
        resp = session.request("POST", "engine/deploy", json=model)
        check_response(resp)
