...
class square_grid:
    ...
    def __init__(self,
        ...
        if cut:
            ...
            self.geosolution =  gpd.GeoDataFrame(geometry=res)
        else:
            self.geosolution = gpd.GeoDataFrame(index=[i for i in range(len(res_geoms))],geometry=gpd.GeoSeries(res_geoms).apply(lambda x: Polygon(x)),crs=data.crs)