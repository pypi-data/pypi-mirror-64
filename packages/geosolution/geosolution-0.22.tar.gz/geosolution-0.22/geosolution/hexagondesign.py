...
class hexagon_grid:
    ...
    def __init__(self,
        ...
        if self.area_threshold == None:
            ...
            hexagon = gpd.overlay(data, disolve_hexa, how = 'union')
            df = hexagon.explode().reset_index().drop(['level_0', 'level_1'], axis = 1)
            check_topology(df)
            self.geosolution = df
        elif 0.0 >= self.area_threshold >= 0.8 and self.area_threshold != None:
            ...
            disolve_hexa = dissolve_polygon(final_hexa, area_thres).geosolution
            hexagon = gpd.overlay(data, disolve_hexa, how = 'union')
            df = hexagon.explode().reset_index().drop(['level_0', 'level_1'], axis = 1)
            check_topology(df)
            self.geosolution = df
