...
class square_grid:
    ...
    def __init__(self,
        ...
        if cut:
            ...
            if datax.crs ==None:
                datax.crs ={'init': 'epsg:4326'}
            final_hexa =gpd.overlay(data.explode().reset_index().drop(['level_0', 'level_1'], axis = 1),
                                    gpd.overlay(data.explode().reset_index().drop(['level_0', 'level_1'], axis = 1),
                                                datax, how='intersection'),
                                    how = 'union')[['geometry']].copy()
            area_thresholda = (length*length)*0.33
