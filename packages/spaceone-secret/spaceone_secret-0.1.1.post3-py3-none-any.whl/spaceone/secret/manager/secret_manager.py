...
class SecretManager(BaseManager):
    ...
    def get_secret(self, secret_id, domain_id):
        secret_model: Secret = self.locator.get_model('Secret')
        return secret_model.get(secret_id=secret_id, domain_id=domain_id)

    def list_secrets(self, query, include_secret_group):
        # if secret_group_id is not None:
        #     query = self._add_secret_group_filter_query(query, secret_group_id)
    ...
    # def _add_secret_group_filter_query(self, query, secret_group_id):
    #     map_query = {
    #         'filter': [{
    #             'k': 'secret_group_id',
    #             'v': secret_group_id,
