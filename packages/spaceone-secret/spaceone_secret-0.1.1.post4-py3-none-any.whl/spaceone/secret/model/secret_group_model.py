...
class SecretGroup(MongoModel):
    ...
    def append(self, key, data):
        if key == 'secrets':
            data['secret_group'] = self
            secret_group_map_vo = SecretGroupMap.create(data)
    ...
    def _change_secret_id_filter(condition):
        ...
        locator = Locator()
        secret_group_model = locator.get_model('SecretGroupMap')
        map_vos, total_count = secret_group_model.query(map_query)
