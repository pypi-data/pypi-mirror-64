...
def get_version(args):
    ...
    if git_version != 'N/A':
        update = False
        r = urlopen("https://github.com/karmab/kcli/commits/master")
        for line in r.readlines():
            if 'https://github.com/karmab/kcli/commits/master?' in str(line, 'utf-8').strip():
                tag_match = re.match('.*after=(.*).*">Older</a></div>', str(line, 'utf-8'))
                if tag_match is not None:
                    upstream_version = tag_match.group(1).split('+')[0][:6]
