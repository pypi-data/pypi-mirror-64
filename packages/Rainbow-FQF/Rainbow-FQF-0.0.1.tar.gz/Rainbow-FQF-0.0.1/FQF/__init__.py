...
class FQF:
    ...
    def get_action(self, state: torch.Tensor) -> int:
        ...
        self.writer.add_scalar(
            "q_value/action", q_value[0, action].cpu().item(), self.current_step)
        self.writer.add_scalar(
            "q_value/max", q_value.max().item(), self.current_step)
        self.writer.add_scalar(
            "q_value/avg", q_value.mean().item(), self.current_step)
        self.writer.add_histogram(
            "value_dist", support.squeeze()[action].detach().numpy(),
            self.current_step
        )
        return action.item()
    ...
    def train(self):
        ...
        n_sup, tau = self.target_net.forward(ns)
        ...
        loss = loss * (tau - delta_mask)
        loss = torch.mean(loss)
        loss = torch.clamp(loss, -1, 1)
        self.memory.update(
            mem_idx, torch.abs(torch.sum(delta, dim=1)).cpu().detach().numpy())
        # train
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        # write log
        self.writer.add_scalar("loss", loss.cpu().item(), self.current_step)
        self.write_net_hist()
        self.soft_target_update()

    def write_net_hist(self):
        self.writer.add_histogram(
            "fc/fc1", self.main_net.fc1.weight.detach().numpy(), self.current_step)
        self.writer.add_histogram(
            "fc/fc2", self.main_net.fc2.weight.detach().numpy(), self.current_step)
        self.writer.add_histogram(
            "fc/supports", self.main_net.support.weight.detach().numpy(),
            self.current_step)
        self.writer.add_histogram(
            "fc/quantiles", self.main_net.quantile.weight.detach().numpy(),
            self.current_step)
