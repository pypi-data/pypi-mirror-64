...
def sample(plot_dir='plot', length=10, bpm=65, sampling_rate=90, png=False, seed=None):
    from pypda.api import sample
    logger.info(f"Random seed: {int(seed)}")
    return sample(plot_dir=plot_dir, length=length, bpm=bpm, sampling_rate=sampling_rate, png=png, seed=seed)
...
@click.option('--png/--no-png', default=False)
@click.option('--seed', type=int, default=None)
def sample_test(plot_dir='plot', length=10, bpm=65, sampling_rate=90, png=False, seed=None):
    from pypda.api import sample_test

    return sample_test(plot_dir=plot_dir, length=length, bpm=bpm, sampling_rate=sampling_rate, png=png, seed=seed)
