...
class Trainer:
    ...
    def __init__(self, config_file_path=None, eslib=None, classifier=False, connection_params=None):
        """
        ...
        if config_file_path is None:
            ...
            self.initialize_config(config_file_path, eslib, classifier)

    def initialize_config(self, config_file_path="./config.ini", eslib=None, classifier=False):
        ...
        # MINIO
        helpers.minioClient = Minio(helpers.config["MINIO"]["minio_host"] + ':' + helpers.config["MINIO"]["minio_port"], 
            helpers.config["MINIO"]["minio_access"], helpers.config["MINIO"]["minio_secret"], secure=False)

        # Elasticsearch
        if eslib is None:
            eslib = Elasticsearchlib(Logger('elasticsearchlib', 'elasticsearchlib.py'))
            ok, _es = eslib.start_connection( str(helpers.config['ELK']['elastic_host']), 
                int(helpers.config['ELK']['elastic_port']))
            if not ok:
    ...
    def initialize(self, connection_params, classifier=False):
        ...
        helpers.config['ELASTIC']['index_models'] = index_models
    ...
    def extract_data_from_elk(self, entity, index_data, time_window_to=None, data_size=200000):
        """
        ...
        data_size : Number of elements to extract. Defaults to 20000. 
        """
        ...
        if entity is not None and entity != index_data:
            if self.time_index != '':
                ...
                ok, aux = self.eslib.scrolled_query(queries.query_by_id(entity), index_data, max_size=data_size) 
                if not ok:                                    
            ...
            ok, aux = self.eslib.scrolled_query(queries.QUERY_ALL, index_data, max_size=data_size) 
            if not ok:                                    
                self.logger.error('Error on getting all data from docs from elasticsearch for: ' + str(entity))    
                return pandas.DataFrame()
            return self.get_dataset_from_docs(aux)
    ...
    def scale_split_data(self, dataframe_global, classifier=False, scaler = None, ft_range = None):
        """
    ...
    def train_model(self, x_train, y_train=None, classifier=False, keras_callbacks=None, balance="False", model = None):
        """
    ...
    def evaluate_model(self, x_test, y_test=None):
        ...
        """    
        scores = self.model_train.evaluate(x_test, y_test, verbose=0)

        return scores


# --------------------- EASIER ready functions ----------------------
...
def easier_train(trainer, entities, index_data=None, inference_type=constants.ESTIMATOR):
    ...
    for entity in entities:
        ...
        with tf.Graph().as_default():
            ...
            model_train = trainer.train_model(x_train, y_train, keras_callbacks=trainer.keras_callbacks, 
                balance = "False")
