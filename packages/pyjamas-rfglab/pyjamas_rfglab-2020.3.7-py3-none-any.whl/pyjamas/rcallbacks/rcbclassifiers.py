...
class RCBClassifiers(RCallback):
    ...
    def cbCreateNNMLP(self, parameters: dict = None, wait_for_thread: bool = False) -> bool:  # Handle IO errors.
        """
        Create a neural network-multilayer perceptron classifier using an lbfgs solver.
        ...
        :param parameters: dictionary containing the parameters to create a logistic regression classifier; a dialog opens if this parameter is set to None; keys are:
            ...
            ``step_sz``:
                number of pixel rows and columns to skip when scanning test images for target structures, formatted as a tuple of two integers
            ``n_hidden``:
                number of hidden neurons, a tuple in which the ith element represents the number of neurons in the ith hidden layer
            ``l2``:
                regularization term, as a float
            ``epochs``:
                maximum number of interations, as an int; the solver iterates until convergence or this number of iterations
            ``eta``:
                initial learning rate, as a float, controls the step size when updating the weights (this parameter is ignored with an lbfgs solver)
            ``shuffle``:
                boolean indicating whether to shuffle samples in each iteration (this parameter is ignored with an lbfgs solver)
            ``minibatch_size``:
                size of minibatches for stochastic optimizers, as a string -can use 'auto' to select min(200, n_samples)- (this parameter is ignored with an lbfgs solver)
        :param wait_for_thread: True if PyJAMAS must wait for the thread running this operation to complete, False otherwise.
        :return: True if the classifier was successfully created, False otherwise.
        """
        continue_flag = True
