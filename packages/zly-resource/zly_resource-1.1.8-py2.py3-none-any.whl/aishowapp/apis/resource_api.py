...
class TotalResource(Resource,BaseAPI):
    ...
    permission_dict = { 0: '前端的入口',1: 'a 资源部，所有权限', 2: 'b 运营部，都能看，但不能改', 3: 'c  bd部门，不能看报价',4: '没有任何权限'}
    ...
    def __init__(self):
        ...
        self.parse.add_argument('department_type', type=str)
    ...
    def get(self):
        ...
        lessfans = 0 if not args.get('lessfans') else float(args.get('lessfans'))
        morefans = 0 if not args.get('morefans') else float(args.get('morefans'))
        id_nickname = '' if not args.get('id_nickname') else args.get('id_nickname')
        flag = cooperation_type + platform
        select_type = 14 if not args.get('select_type') else int(args.get('select_type'))
        department_type = 0 if not args.get('department_type') else int(args.get('department_type'))
        ...
        if department_type==4:
            print('all_list', json.dumps(
                {'code': 20000, 'data': {'total': totle, 'items': all_list}, 'department_type': department_type},
                ensure_ascii=False))
            ...
            if flag.startswith('直播'):
                ...
                for item in live_query:
                    dict = {}
                    live_dict=Live.queryToDict(item)
                    ...
                    if  flag.find('淘宝')!=-1:
                        ...
                        if livetaobaoprice_query:
                            qttb_dict = QitengTaobaoLiveOffer.queryToDict(livetaobaoprice_query)
                            # print('直播淘宝对应的价格',dysl_dict)
                            for k, v in qttb_dict.items():
                                if department_type==3:
                                    if k == 'cost_price' or k == 'single_chain_cost_price' or k == 'special_cost_price':
                                        pass
                                    else:
            ...
            #短视频
                ...
                for item in video_query:
                    dict = {}
                    svideo_dict=ShortVideo.queryToDict(item)
                    # 判断是不是bd用户
                    if department_type == 3:
                        demo_dict ={}
                        for k, v in svideo_dict.items():
                            if k == 'cost_0_20s' or k == 'cost_21_60s':
                                pass
                            else:
                                demo_dict[k] = v
                        dict['video'] = demo_dict
                    else:
                        dict['video'] = svideo_dict
            ...
            #图文
            elif  flag.startswith('图文') and (not flag.startswith('图文视频')):
                print('图文')
                if id_nickname:
                ...
                for item in image_query:
                    dict = {}
                    image_dict=ImageText.queryToDict(item)
                    # 判断是不是bd用户
                    if department_type == 3:
                        demo_dict = {}
                        for k, v in image_dict.items():
                            if k == 'image_cost_price' :
                                pass
                            else:
                                demo_dict[k] = v
                        dict['image'] = demo_dict
                    else:
                        dict['image'] = image_dict
                    res_query = item.resource_table
                    if res_query:
                        res_dict = ResourceTable.queryToDict(res_query)
                        dict['rsst'] = res_dict
            ...
            #个性匹配
                ...
                print('图文视频')
                if id_nickname:
                    image_video_query = ImageVideo.query.join(ResourceTable).filter(
                        or_(ResourceTable.kol_name.like("%" + id_nickname + "%") if id_nickname is not None else "",
                            ResourceTable.kol_id.like("%" + id_nickname + "%") if id_nickname is not None else "")).offset(
                        (page - 1) * limit).limit(
                        limit).all()
                    ...
                    image_video_query = ImageVideo.query.join(ResourceTable).filter(*conditions).offset((page - 1) * limit).limit(
                        limit).all()
                    totle = ImageVideo.query.join(ResourceTable).filter(*conditions).count()
                for item in image_video_query:
                    dict = {}
                    image_dict=ImageVideo.queryToDict(item)
                    # 判断是不是bd用户
                    # 判断是不是bd用户
                    if department_type == 3:
                        demo_dict = {}
                        for k, v in image_dict.items():
                            if k == 'cost_price_image_text' or k == 'red_book_cost_price_video':
                                pass
                            else:
                                demo_dict[k] = v
                        dict['imagevideo'] = demo_dict
                    else:
                        dict['imagevideo'] = image_dict
                    ...
                    if flag.find('小红书')!=-1:
                        ...
                        if redbook_price_query:
                            ...
                            for k, v in redbook_dict.items():
                                if department_type == 3:
                                    if k == 'image_cost_price' or k == 'viedo_cost_price' or k == 'brand_image_cost_price' or k == 'brand_viedo_cost_price':
                                        pass
                                    else:
                                        if not dict['imagevideo'].get(k, None):
                                            dict['imagevideo'][k] = v
        ...
        print('all_list',json.dumps({'code': 20000, 'data': {'total': totle, 'items': all_list },'department_type': department_type },ensure_ascii=False))
        return jsonify({'code': 20000, 'data': {'total': totle, 'items': all_list }})
    ...
    def resource_get_list(self,select_type,department_type,page,limit):
        """
        资源
        ...
        response = self._get('platformresource/list/resourcetable',{'select_type':select_type,'department_type':department_type,'page':page,'limit':limit})
...
class PriceResource(Resource):
    ...
    def delete(self):
        ...
        print('********')
        print(live_name)
        print(live_name.startswith('直播'))
        print(live_name.startswith('短视频'))
        print(live_name.startswith('图文'))
        print(live_name.startswith('图文视频'))
...
class ShowResource(Resource):
    ...
    select_dict = {1: '直播淘宝', 2: '直播抖音', 3: '直播快手', 4: '直播苏宁',
                   5: '短视频淘宝', 6: '短视频抖音', 7: '短视频快手', 8: '短视频苏宁',
                   9: '图文淘宝', 10: '图文抖音', 11: '图文快手', 12: '图文京东',
                   13: '图文视频小红书',14:''}
    ...
    def __init__(self):
        ...
        self.parse.add_argument('less_single_chain_offer', type=str)
        self.parse.add_argument('more_single_chain_offer', type=str)
        self.parse.add_argument('less_special_offer', type=str)
        self.parse.add_argument('more_special_offer', type=str)
        self.parse.add_argument('less_0_20s_offer', type=str)
        self.parse.add_argument('more_0_20s_offer', type=str)
        self.parse.add_argument('less_21_60s_offer', type=str)
        self.parse.add_argument('more_21_60s_offer', type=str)
        self.parse.add_argument('less_image_text_offer', type=str)
        self.parse.add_argument('more_image_text_offer', type=str)
        self.parse.add_argument('less_video_offer', type=str)
        self.parse.add_argument('more_video_offer', type=str)
        self.parse.add_argument('platform', type=str)
        self.parse.add_argument('cooperation_type', type=str)
        self.parse.add_argument('select_type', type=int)

    def get(self):
        print('resourceshow  get',)

        args = self.parse.parse_args()
        print('resourceshow args ', args)
    ...
    def search(self, **kwargs):
        ...
        less_single_chain_offer = 0 if not kwargs.get('less_single_chain_offer') else float(kwargs.get('less_single_chain_offer'))
        more_single_chain_offer = 0 if not kwargs.get('more_single_chain_offer') else float(kwargs.get('more_single_chain_offer'))
        less_special_offer = 0 if not kwargs.get('less_special_offer') else float(kwargs.get('less_special_offer'))
        more_special_offer = 0 if not kwargs.get('more_special_offer') else float(kwargs.get('more_special_offer'))
        less_0_20s_offer = 0 if not kwargs.get('less_0_20s_offer') else float(kwargs.get('less_0_20s_offer'))
        more_0_20s_offer = 0 if not kwargs.get('more_0_20s_offer') else float(kwargs.get('more_0_20s_offer'))
        less_21_60s_offer = 0 if not kwargs.get('less_21_60s_offer') else float(kwargs.get('less_21_60s_offer'))
        more_21_60s_offer = 0 if not kwargs.get('more_21_60s_offer') else float(kwargs.get('more_21_60s_offer'))
        less_image_text_offer = 0 if not kwargs.get('less_image_text_offer') else float(kwargs.get('less_image_text_offer'))
        more_image_text_offer = 0 if not kwargs.get('more_image_text_offer') else float(kwargs.get('more_image_text_offer'))
        less_video_offer = 0 if not kwargs.get('less_video_offer') else float(kwargs.get('less_video_offer'))
        more_video_offer = 0 if not kwargs.get('more_video_offer') else float(kwargs.get('more_video_offer'))
        ...
        if lessoffer and morefans :
            if cooperation_type == '直播' and platform == '淘宝':
        ...
        if less_single_chain_offer and  more_single_chain_offer:
             conditions.append(Live.offer.between(less_single_chain_offer, more_single_chain_offer))
        if less_special_offer and more_special_offer:
            conditions.append(Live.offer.between(less_special_offer, more_special_offer))
        if less_0_20s_offer and more_0_20s_offer:
            conditions.append(ShortVideo.offer.between(less_0_20s_offer, more_0_20s_offer))
        if less_21_60s_offer and more_21_60s_offer:
            conditions.append(ShortVideo.offer.between(less_21_60s_offer, more_21_60s_offer))
        if less_image_text_offer and more_image_text_offer:
            conditions.append(ShortVideo.offer.between(less_image_text_offer, more_image_text_offer))
        if less_video_offer and more_video_offer:
            conditions.append(ShortVideo.offer.between(less_video_offer, more_video_offer))
