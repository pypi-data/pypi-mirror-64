...
from thompcoutils.log_utils import get_logger, start_function, end_function
import smtplib
import os
...
class EmailSender:
    ...
    def send(self, to_email, subject, message, attach_files=None):
        logger = get_logger()
        logger.debug("sending to:{} subject:{}, message:{}, files:{}", to_email, subject, message, attach_files)
...
def main():
    ...
    if create_file:
        ...
        print("Writing out configuration file to {} and stopping.".format(temp_filename))
    ...
    mail_from_cfg.send_embedded(to_email="Jordan@ThompCo.com", subject="this is a test from emailSender",
                                html_message=html_message,
                                attach_files=["../attachments/image_1.png"])
