    ...
    def strfpcrontime(source_string, format_string):
        ...
        for i in range(0, len(format_string)):
            if format_string[i] == '%':
                ...
                if fmt == 'w':
                    cron_time.day_of_week = int(source_string[i:i+2])
                elif fmt == 'm':
                    cron_time.month = int(source_string[i:i+2])
                elif fmt == 'd':
                    cron_time.day_of_month = int(source_string[i:i+2])
                elif fmt == 'H':
                    cron_time.hour = int(source_string[i:i+2])
                elif fmt == 'M':
                    cron_time.minute = int(source_string[i:i+2])
                else:
    ...
    def strfcrontime(self, format_string):
        ...
        for y in range(0, len(format_string)):
            ...
            if format_string[i] == '%':
                fmt = format_string[i+1]
                if fmt == 'w':
                    str += '{:02d}'.format(self.day_of_week)
                elif fmt == 'm':
...
def _test_config_mgr(path):
    ...
    for write in [True, False]:
        ...
        cfg_mgr.write_entry("User 1", "cron_format", '%%w %%d %%m %%H %%M')
        ...
        if write:
            ...
            val = cfg_mgr.read_entry("User 1", "cron_format", '%%w %%d %%m %%H %%M')
