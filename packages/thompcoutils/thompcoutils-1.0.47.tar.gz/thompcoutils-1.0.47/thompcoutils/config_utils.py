...
class ConfigManager:
        ...
        if self.create:
            ...
            try:
                if isinstance(default_value, str):
                    ...
                    format = self.config.get(section, '{}_format'.format(entry))
                    value = self.config.get(section, '{}_time'.format(entry))
                    value = CronTime.strfpcrontime(value, format)
                else:
    ...
    def write_entry(self, section, entry, value, note=None, format=None):
        try:
            if isinstance(value, CronTime):
                self.config.set(section, '{}_format'.format(entry), format.replace('%', '%%'))
                self.config.set(section, '{}_time'.format(entry), value.strfcrontime(format))
            else:
    ...
    def write(self, out_file, stop=True, overwrite=False):
        ...
        f.write(contents)
        f.close()
        logger.debug("Done writing {}".format(out_file))
        if stop:
            sys.exit()
...
def _test_config_mgr(path):
    ...
    for write in [True, False]:
        ...
        cfg_mgr.write_entry("User 1", "cron_time", CronTime(day_of_month=1, hour=2, minute=3), format='%w %d %m %H %M')
