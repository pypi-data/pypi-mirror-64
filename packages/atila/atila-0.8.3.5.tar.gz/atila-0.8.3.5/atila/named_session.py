...
class NamedSession:
	...
	def mount (self, name = None, session_timeout = None, securekey = None, path = "/", domain = None, secure = False, http_only = False, extend = True):
		...
		if self.__wclass == "session":
			obj = self.__get_session (name, securekey, session_timeout, extend)
		else:
	...
	def __get_session (self, name, securekey, session_timeout, extend):
		return session.Session (name, self.__cookie, self.__request, securekey, session_timeout, extend)
