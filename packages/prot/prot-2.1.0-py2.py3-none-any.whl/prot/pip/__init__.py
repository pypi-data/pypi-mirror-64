...
from prot.converters import list2str
try:
...
def updateBuiltinList(export=False, pList=False, fList='txt'):
	...
	try:
		if export:
			...
			_p = pl
			f = open(pipPath+'/packagesList.py', 'w')
...
def main(action, **values):
	...
	if action in ['install', 'download']:
		...
		for c in range(len(options.listContents)):
			...
			try:
				if options.local:
					runAsMain(str('pip '+action+' '+value+'$EX'+' --no-index -f '+options.local).replace('$EX', (' '+options.extraArgs) if options.extraArgs else ''))
				else:
					runAsMain(str('pip '+action+' '+value+'$EX').replace('$EX', (' '+options.extraArgs) if options.extraArgs else ''))
			except:
				pass
			VAL2.value += 1
			progress.update(c+1)
