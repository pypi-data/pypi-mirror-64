    ...
    def add_nodes(self, *nodes):
        ...
        for node in nodes:
            ...
            for unit in self._model_units_list:
                ...
                if hasattr(unit, 'costs' or 'incomes' or 'transaction'):
                    #TODO definir
                    unit.eco_balance = Quantity(name='eco_balance', vlen=1,
                                                parent=unit)
                    unit.calc_eco_balance = DefinitionConstraint(
                        name='calc_eco_balance',
                        exp='{}_eco_balance = {}_incomes - {}_costs + {'
                            '}_transaction'.format(unit.name),
                        parent=unit
                    )
