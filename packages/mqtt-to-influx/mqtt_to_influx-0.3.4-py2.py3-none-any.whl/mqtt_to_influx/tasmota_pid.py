...
class Process_mqtt_message:
    def __init__(self, mqtt_client, userdata, msg):
        ...
        if message_type == "SENSOR": # ignore sensore messages. They're handled in another plugin
            logger.debug ("SENSOR message obtained")
            return None
        # elif message_type == "STATE": # Ignore STATE messages for now
        #     logger.debug ("STATE message obtained")
        # elif message_type == "PID":
        #     logger.debug ("PID message obtained")
