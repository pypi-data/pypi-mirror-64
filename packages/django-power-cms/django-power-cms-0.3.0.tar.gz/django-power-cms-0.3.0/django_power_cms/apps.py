...
class DjangoPowerCmsConfig(AppConfig):
    ...
    def get_site_config_url(self, request, *args, **kwargs):
        from .admin import CURRENT_SIET_ID_SESSION_KEY
        try:
    ...
    def register_app_navigation_portlet(self):
        ...
        register_portlet(nav, app_label=self.name, model_name="site")
        register_portlet(nav, app_label=self.name, model_name="page")
        register_portlet(nav, app_label=self.name, model_name="widget")
        register_portlet(nav, app_label=self.name, model_name="article")
