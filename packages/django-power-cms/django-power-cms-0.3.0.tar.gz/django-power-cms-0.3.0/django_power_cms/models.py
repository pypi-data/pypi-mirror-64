...
class TopbarBrand(models.Model):
    ...
    def url(self):
        if not self.url:
            return "#"
        if self.url_raw.startswith("site:"):
            site_code = self.url_raw[5:]
            return reverse("django-power-cms.site", kwargs={"site_code": site_code})
        if self.url_raw.startswith("page:"):
            _, site_code, page_code = self.url_raw.split(":")
...
class Article(MPTTModel):
    ...
    HIGH = 0
    NORAML = 99
    site = models.ForeignKey(Site, on_delete=models.CASCADE, null=True, blank=True, related_name="articles", verbose_name=_("Site"))
