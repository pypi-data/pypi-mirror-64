...
async def load_interfaces(hub):
    ...
    if ip_path:
        ...
        log.info("Unable to find commands `ip` or `ifconfig` to detect network devices")
        return

    interface: str
    device: Dict[str, Any]
    for interface, device in interfaces.items():
        hub.corn.CORN.hw_addr_interfaces[interface] = device.get(
