...
class MongoDB():
    ...
    def dynamic_runs_remaining(self, job_id):
        '''
        decrements and returns the jobs "dynamic_runs_remaining" property. 
        '''
        dd = self.mongo_db["__jobs__"].find_and_modify( {"_id": job_id}, update={"$inc": {"dynamic_runs_remaining": -1} })

        # we subtract -1 in return value because it is the before-decrement value
        remaining = (dd["dynamic_runs_remaining"] - 1) if dd else None
