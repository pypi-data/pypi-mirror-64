...
class Philly(BackendBase):
    ...
    def build_env_vars(self, user_env_vars, workspace, experiment, run_name, use_controller, node_id, args):
    ...
    def process_data_action(self, cmds, store_data_dir, data_action):
        if data_action == "mount":
            ...
            self.append(cmds, "echo MOUNTING data share from store to {}".format(data_mnt))
            self.append(cmds, 'export XT_DATA_MNT="' + data_mnt + '"')
            self.append(cmds, 'export XT_DATA_DIR="' + data_mnt + '"')
            self.append(cmds, "echo running 'ls {}'".format(data_mnt))
            self.append(cmds, "ls {}".format(data_mnt))
        elif data_action == "download":
            dest_dir = "/home/$USER/data"

            self.append(cmds, "echo DOWNLOADING data share from store to {}".format(dest_dir))
    ...
    def process_model_action(self, cmds, store_model_dir, model_action):
        if model_action == "mount":
            # point to parent data directory (not target, e.g., mnist)
            model_mnt = "/mnt/models/" 

            self.append(cmds, "echo MOUNTING model share from store to {}".format(model_mnt))
            self.append(cmds, 'export XT_MODEL_MNT="' + model_mnt + '"')
            self.append(cmds, 'export XT_MODEL_DIR="' + model_mnt + '"')
            self.append(cmds, "echo running 'ls {}'".format(model_mnt))
            self.append(cmds, "ls {}".format(model_mnt))
        elif model_action == "download":
            dest_dir = "/home/$USER/models"
            self.append(cmds, "echo DOWNLOADING model from store to {}".format(dest_dir))
    ...
    def wrap_cmd_line_for_azure_storage(self, user_cmd, store_before_dir, store_output_dir, store_after_dir, 
        ...
        # activate environment and add conda, pip packages as needed
        super().add_setup_cmds(cmds, pip_freeze=pip_freeze, args=args)
        ...
        if code_zip and code_zip != "none":
            ...
            self.append(cmds, 'unzip ' + constants.CODE_ZIP_FN)
    ...
    def submit_job(self, job_id, job_runs, workspace, compute_def, resume_name, 
        ...
        for i, node_runs in enumerate(job_runs):
            ...
            philly_job_id = self.submit_runs_for_node(job_id, node_runs, workspace, compute_def, resume_name, repeat_count, using_hp, 
                experiment, runs_by_box, node_id, box_name, args)

            philly_job_ids.append(philly_job_id)
        ...
        return philly_job_ids, monitor_url
    ...
    def build_curl(self, cmd):
        ...
        if pc_utils.is_windows():
            ...
            if os.path.exists(fn_pass):
                ...
                prefix = "curl {} -su '{}' --ntlm  : ".format(cert_cmd, self.username)
    ...
    def submit_runs_for_node(self, job_id, node_runs, workspace, compute_def, resume_name, repeat_count, using_hp, experiment, 
        ...
        for run_data in node_runs:
            ...
            wrapped_cmd = self.wrap_cmd_line_for_azure_storage(cmd_line, store_before_dir, 
        ...
        vc = utils.safe_value(compute_def, "vc")              # msrlabs"
        cluster = utils.safe_value(compute_def, "cluster")    # "cam"
        sku = utils.safe_value(compute_def, "sku").upper()    # "G1"
        queue = utils.safe_value(compute_def, "queue")        # interactive
        ...
        env_vars = self.build_env_vars(user_env_vars, workspace, experiment, first_run_name, use_controller, node_id, args)
    ...
    def view_console(self, job_id, job_info, info_verb, parse_result=True, extra=""):
        ...
        vc = pool_info["vc"]
        philly_job_id = job_info["service_job_id"]  
