...
def get_controller_script(box_name, box_class, for_windows, config):
    ...
    run_windows = [
        ...
        '''@python -u -c "from xtlib.controller import run as run; run(max_runs=1, my_ip_addr='localhost')" > $HOME/.xt/cwd/controller_run.log''']

    run_linux = [
        "#!/bin/sh",
        '''nohup python -u -c "from xtlib.controller import run as run; run(max_runs=1, my_ip_addr=$IP_ADDR)" </dev/null  > ~/.xt/cwd/controller_run.log 2>&1 &''']

    run_aml = [
        "#!/bin/sh",
        '''python -u -c "from xtlib.controller import run as run; run(max_runs=1, my_ip_addr=$IP_ADDR, multi_run_context_fn='{}', multi_run_hold_open=$HOLD)" > controller.log'''.format(constants.FN_MULTI_RUN_CONTEXT)
        ]
    ...
    if config.get("code", "xtlib-upload"):
        ...
        if for_windows:
            ...
            script.insert(-1, "@set PYTHONPATH=c:\\github\\ExperimentTools")
        else:
            # assume fixed location on linux
            script.insert(-1, "export PYTHONPATH=~/ExperimentTools")
...
def fixup_script(lines, for_windows, is_local, run_cmd=None, run_info=None, max_runs=None, 
        hold=False):
    ...
    for line in lines:
        ...
        line = fixup_user_cmd(line, for_windows, is_local, run_info, max_runs, hold)
        newlines.append(line)

    #console.print("after FIXUP: newlines=", newlines)
    return newlines

def fixup_user_cmd(flat_cmd, for_windows, is_local, run_info, max_runs, hold):
    '''
    Macros supported:
        $HOME       - same as "~/"
    ...
    if run_info:
        ...
        if "." in run_name:
            ...
            if not right_part.startswith("run"):
                ...
                mr_index = str((run_index-1) % max_runs)
