...
class Runner():
    ...
    def build_first_run_for_node(self, node_index, box_name, run_script_path, parent_script_path, using_hp, use_aml_hparam, run_specs, 
            job_id, parent_name, cmds, pool_info, repeat_count, fake_submit, search_style, args):
    ...
    def build_runs_by_box(self, job_runs, workspace):
        ...
        for run_data_list in job_runs:
            for run_data in run_data_list:   
                ...
                runs_by_box[box_name].append( {"ws_name": workspace, "run_name": run_data["run_name"]} )
    ...
    def process_run_command(self, args):
        ...
        for i in range(run_count):
            ...
            run_data = self.build_first_run_for_node(i, boxes[i], target_file, ps_path, using_hp, using_aml_hparam, run_specs, job_id, 
                parent_name, cmds, pool_info, repeat_count, fake_submit, search_style, args)
        ...
        if use_controller:
            ...
            controller_scripts = self.core.adjust_job_for_controller_run(job_id, job_runs, cmds, using_hp, experiment, service_type, snapshot_dir, 
        ...
        # we add with "node0", but backend service will override
        scriptor.add_controller_env_vars(env_vars, self.config, job_secret, "node0")
        ...
        # ADUST CMDS: this allows backend to write scripts to snapshot dir, if needed, as a way of adjusting/wrapping run commands
        ...
        service_job_id, monitor_url = self.backend.submit_job(job_id, job_runs, workspace, pool_info, resume_name, 
        ...
        if not fake_submit:
            ...
            dd = {"job_id": job_id, "job_num": job_num, "compute": compute, "ws_name": workspace, "exper_name": experiment, 
                "pool_info": compute_def, "runs_by_box": runs_by_box, "service_job_id": service_job_id, "primary_metric": args["primary_metric"], 
