...
from xtlib.store import store_from_context
...
class XTController(rpyc.Service):
    ...
    def __init__(self, max_runs=1, my_ip_addr=None, multi_run_context_fn=None, multi_run_hold_open=False, 
        ...
        console.print("==========================================================")
        console.print("ts:", datetime.datetime.now())

        console.print("max_runs={}, my_ip_addr={}, multi_run_context_fn={}, multi_run_hold_open={}, port={}, is_aml={}".format(
            max_runs, my_ip_addr, multi_run_context_fn, multi_run_hold_open, port, self.is_aml))
           
        console.print("self.rundir_parent=", self.rundir_parent, ", PYTHONPATH=", os.getenv("PYTHONPATH"))
        console.print("CONDA:", os.getenv("CONDA_DEFAULT_ENV"))
        ...
        self.hparam_search = HParamSearch(is_aml)
    ...
    def checkpoint_state(self):
        ...
        if runs:
            ...
            console.print("checkpoint written to: job={}, path={}".format(context.job_id, store_path))
    ...
    def get_checkpoint_info(self, store, job_id, node_index):
        ...
        console.print("get_checkpoint_info: path={}, exists={}".format(store_path, cp_exists))
    ...
    def process_multi_run_context(self, multi_run_context_fn):
        ...
        # until we rework the restart code, turn it off so it doesn't interfere with normal full rereun of job on this node
        cp_exists = False
    ...
    def restart_checkpoint_runs(self, store, cp_data):
        ...
        was_queued = []
        console.print("----------------------------------------------------")
        console.print("--- RESTART DETECTED: processing checkpoint runs ---")
        console.print("----------------------------------------------------")
        ...
        for run_data in cp_data["runs"]:
            ...
            console.print("  processing checkpoint run: name={}, status={}, repeats_remaining={}".format(run_name, status, repeats_remaining))

            # no processing needed if run has ended
            if status in ["completed", "cancelled", "aborted", "error", "unknown"]:
                continue
            ...
            if status == "running":
                ...
                console.print("    found child or flat run: setting resume_name=", context.resume_name)
            elif context.is_parent:
                # parent run 
                context.repeats_remaining = repeats_remaining
                console.print("    found is_parent=True, run={}, repeat={}, setting repeats_remaining={}".format(run_name, context.repeat, context.repeats_remaining))

            if status in ["spawning", "queued"]:
                was_queued.append(context)
                console.print("    ==> added run to 'was_queued'")
            else:
                # requeue this run immediately
                console.print("    ==> immediate requeuing: ", run_name)
                self.queue_job_core(context, context.cmd_parts, True)

        # now queue previously queued jobs (to keep order approx. correct)
        console.print("processing 'was_queued'=", was_queued)

        for context in was_queued:
            console.print("  (at end) requeuing: ", context.run_name)
            self.queue_job_core(context, context.cmd_parts, True)

        console.print("--- end of processing checkpoint runs ---")
    ...
    def queue_check(self, max_starts=1):
        ...
        # console.print("queue_check # " + str(self.queue_check_count))
        ...
        for steps in range(max_starts):       
            ...
            if run_info:
                if run_info.parent_prep_needed:
                    ...
                    remaining_runs = store.mongo.dynamic_runs_remaining(run_info.job_id)
                    print("==> remaining for job={}, run={}: {}/{}".format(run_info.job_id, run_info.run_name, remaining_runs, context.total_run_count))
                    if remaining_runs >= 0:

                        # yes: CREATE CHILD
                        self.run_template(run_info, remaining_runs)
    ...
    def run_template(self, run_info, runs_remaining):
        run_name = run_info.run_name
        ...
        child_info = self.spawn_child(run_info, runs_remaining)
        child_name = child_info.run_name

        # add to runs
        self.add_to_runs(child_info)

        # since we just added a run (and allocated a child name), checkpoint our state now
        self.checkpoint_state()
        ...
        #             else:
        #                 #debug_break()
        #                 remaining = store.mongo.dynamic_runs_remaining(run_info.job_id)
        #                 print("==> remaining for job={}, run={}: {}".format(run_info.job_id, run_info.run_name, remaining))
        ...
        #                 else:
        #                     # mark as completed
        #                     console.print("marking --repeat parent as completed: ", run_info.run_name)
        #                     run_info.status = "completed"
    ...
    def spawn_child(self, parent, runs_remaining):
        ...
        run_index = context.total_run_count - (1 + runs_remaining) 
        run_index = run_index % len(self.mrc_cmds)

        cmd = self.mrc_cmds[run_index]
        ...
        child_name = store.start_child_run(context.ws, parent.run_name, context.exper_name,
            ...
            aggregate_dest=context.aggregate_dest,  aml_child_name=child_name, 
            compute=context.compute, service_type=context.service_type, username=context.username, 
            is_aml=False, search_type=search_type)
    ...
    def queue_job_core(self, context, cmd_parts, previously_queue=False, aml_run=None):
        ...
        if context.max_runs is not None:
            console.print("--> setting self.max_runs =", context.max_runs)
    ...
    def start_local_run_core(self, run_info, cmd_parts, rundir, run_index):
        '''
        Note: 
        ...
        run_script = scriptor.fixup_script(run_script, pc_utils.is_windows(), True, run_info=run_info, max_runs=self.max_runs)

        # write RUN SCRIPT LINES to a run_appXXX script file
        if pc_utils.is_windows():
            if not fn_script:
                fn_script = os.path.expanduser("~/.xt/cwd/run_app{}.bat".format(run_index))
            ...
            if not fn_script:
                fn_script = os.path.expanduser("~/.xt/cwd/run_app{}.sh".format(run_index))
        ...
        # write context file, in case run needs to access additional info
        json_text = json.dumps(context.__dict__)
        fn_context = os.path.join(rundir, constants.FN_RUN_CONTEXT)
        ...
        if pc_utils.is_windows():
            ...
            console.print("rundir=", rundir, ", parts=", parts)
    ...
    def exposed_get_status_of_workers(self, worker_name):
        self.validate_request(token)
        status_list = []
    ...
    #     console.print("exposed_open: path=", path, ", flags=", flags)
    #     path = os.path.expanduser(path)
    #     return open(path, flags)

    # # used for uploading files to controller (still used?)
    # def exposed_mkdir(self, path, raise_error_if_exists=False):
    #     console.print("exposed_mkdir: path=", path)
    #     path = os.path.expanduser(path)
    ...
    def exposed_get_max_runs(self, token):
        self.validate_request(token)
        return self.max_runs

    def exposed_set_max_runs(self, token, value):
        #console.print("exposed_set_max_runs: value=", value, ", type(value)=", type(value))
        self.validate_request(token)
        self.max_runs = value
...
def run(max_runs=1, my_ip_addr=None, multi_run_context_fn=constants.FN_MULTI_RUN_CONTEXT, multi_run_hold_open=False, port=constants.CONTROLLER_PORT, is_aml=False):
    '''
    ...
    # write file JIT from env var values
    fn_server_cert = os.path.expanduser(constants.FN_SERVER_CERT)
    ...
    service = XTController(max_runs, my_ip_addr, multi_run_context_fn, multi_run_hold_open, port, is_aml)
