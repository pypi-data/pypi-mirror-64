...
class ImplStorage(ImplBase):
    ...
    @command(kwgroup="list", help="list experiments defined in the current workspace")
    ...
    @example(task="view the console output for job201 (running on Philly)", text="xt view console job201")
    @command(kwgroup="view",  help="view console output for specified run")
    def view_console(self, target, workspace):
        if target.startswith("job"):
            ...
            # run the command and let output go to console
    ...
    #---- VIEW RUN command ----
    @argument(name="run_name", help="the name of the run")
    @option(name="workspace", default="$general.workspace", help="the workspace that the run resides in")
    ...
    #---- VIEW LOG command ----
    @argument(name="target", help="the name of the run, job, or 'controller'")
    ...
    #---- PLOT command ----
    # args, flags, options
    @argument(name="run_list", type="str_list", help="a comma separated list of: run names, name ranges, or wildcard patterns", required=False)
    @argument(name="col_list", type="str_list", required=False, help="a comma separated list of metric names to plot")
    @option(name="layout", help="specifies how the plots should be layed out, e.g., '2x3'")
    @option(name="aggregate", default="none", values=["none", "mean", "median", "min", "max", "std"], help="how to aggregate data from multiple runs into a single plot")
    @option(name="x", help="the metric to use for plotting along the x axis")
    @option(name="break-on", type="str_list", help="the entity that triggers a plot change: usually 'run' or 'col'")
    @option(name="colors", type="str_list", help="the colors to use the the plot objects")
    ...
    @option(name="range-type", default="none", values=["none", "std", "minmax", "error"], help="how to form the min/max values for a range plot")
    @option(name="range_alpha", type=float, default=.3, help="the alpha blending factor used to draw the range area")
    ...
    # command
    def plot(self, run_list, col_list, x, layout, break_on, title, colors, show_legend, plot_titles, legend_titles, 
        smoothing_factor, plot_type, marker_size, marker_shape, alpha, edge_color, workspace, timeout, aggregate, 
        range_type, range_alpha, style, show_toolbar):

        args = {"run_list": run_list, "workspace": workspace, "all": True, "sort_col": "name", 
            "columns": ["run", "metrics.*"]}

        x_col = x
        
        # store col names used here
        col_dict = {"run_name": 1, "log_records": 1}
        ...
        pb = plot_builder.PlotBuilder(run_names, col_list, x_col, layout, break_on, title, colors, show_legend, plot_titles,
    ...
    #---- EXTRACT command ----
    @argument(name="run_name", help="the name of the run")
    @argument(name="dest_dir", help="the path of the directory")
    ...
    @option(name="step-name", default="$hyperparameter-explorer.step-name", help="the name of the step/epoch metric")
    ...
    #---- VIEW PORTAL command ----
    @argument(name="target_name", help="the name of the target whose portal is to be opened")
    ...
    # @option(name="run_name", help="the name of the run to navigate to in the portal")
    ...
    @option(name="last", type=int, help="limit the output to the last N items")
    ...
    @option(name="experiment", help="the experiment that the path is relative to")
    @option(name="interval", type=int, default=10, help="specifies interval between polling for changes in the run's 'mirrored' storage")
    @option(name="job", help="the job id that the path is relative to")
