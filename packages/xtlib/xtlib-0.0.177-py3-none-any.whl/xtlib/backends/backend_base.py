...
class BackendBase(implements(BackendInterface)):
    ...
    # common code
    def append(self, cmds, cmd, echo_title=None, echo_before=False, echo_after=False, expand=False):
    ...
    def create_blobfuse_commands(self, storage_name, storage_key, sudo_available, mount_requests, install_blobfuse, 
            use_username=True, use_allow_other=True):
        username = "$USER"
        cmds = []
        ...
        for md in mount_requests:
            ...
            sub_cmds += [
                ...
                "{}blobfuse {} --tmp-path={}  --config-file={} {} -o attr_timeout=240 -o entry_timeout=240 -o negative_timeout=120 {}" \
                    .format(sudo, mnt_dir, tmp_dir, fn_config, readonly_opt, allow_other),
    ...
    def process_action(self, cmds, action, mnt_path, env_var_name, env_var_name2, container, store_data_dir, is_writable,
        ...
        install_blobfuse=False, use_allow_other=True):
        ...
        if action == "mount":
            ...
            self.append(cmds, 'export {}="{}"'.format(env_var_name, mnt_path))
            self.append(cmds, 'export {}="{}"'.format(env_var_name2, mnt_path))
    ...
    def wrap_user_command(self, user_parts, snapshot_dir, store_data_dir, data_action, data_writable, \
        ...
        setup=None, post_setup_cmds=None, args=None):
        ...
        if is_windows:
            self.append(cmds, "echo start of WRAPPED.bat")
        else:
            self.append(cmds, "echo start of WRAPPED.sh")

        self.append(cmds, "cd ~/.xt/cwd")
        ...
        if "data" in actions:
            self.process_action(cmds, data_action, data_mount_dir, "XT_DATA_DIR", "XT_DATA_MNT", utils.DATA_STORE_ROOT, 
                ...
                is_windows=is_windows, use_username=use_username, install_blobfuse=install_blobfuse)
