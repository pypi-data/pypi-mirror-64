...
class ImplShared():
    ...
    def write_controller_scripts(self):
        dir_name = os.path.expanduser("~/.xt/controller")
        file_utils.ensure_dir_exists(dir_name)

        source_dir = os.path.join(file_utils.get_xtlib_dir(), "controller_scripts")
        filenames, _ = file_utils.get_local_filenames(source_dir)  

        # write any missing controller scripts
        for fn in filenames:
            dest_fn = os.path.join(dir_name, os.path.basename(fn))
    ...
    def init_config(self, mini=False, args=None):
        self.write_controller_scripts()

        self.config = xt_config.get_merged_config(mini=mini)
