...
from .qfe import inner_dispatch, get_dispatch_cmd
from .qfe import command, argument, option, flag, root, clone, hidden, example, command_help, keyword_arg
...
class ImplCompute(ImplBase):
    ...
    #---- VIEW STATUS command ----
    @argument(name="run_name", required=False, help="return status only for the specified run")
    ...
    #---- ATTACH command ----
    @argument(name="run_name", help="The name of the run to attach")
    ...
    @argument(name="run_names", type="str_list", help="comma-separated list of run names")
    ...
    #---- RERUN command ----
    @argument("run_name", help="the name of the original run")
    @option("workspace", default="$general.workspace", type=str, help="the workspace that the runs reside within")
    ...
    #---- CANCEL RUN command ----
    @argument("run_names", type="str_list", help="the list of run names to cancel")
    ...
    @command(kwgroup="cancel",  help="cancels the specified job and its active or queued runs")
    ...
    @argument("job_id", help="the name of the job whose node will be restarted")
    @option("node_index", default=0, help="the 0-based node index to be restarted")
    @option("delay", default=1, help="the number of seconds to delay after cancelling runs and before restarting the controller")
    ...
    @example(task="run the script miniMnist.py on Azure ML", text="xt --target=aml run miniMnist.py")
