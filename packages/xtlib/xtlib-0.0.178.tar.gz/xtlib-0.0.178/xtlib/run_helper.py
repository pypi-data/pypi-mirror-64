    ...
    if name_list:
        ...
        if len(name_list)==1 and file_utils.has_wildcards(first_name):
            ...
            if first_name in ["*", "run*"]:
                ...
                run_list = [rec["_id"] for rec in records]
                actual_ws = workspace
        else:            
            run_list, actual_ws = parse_run_list(store, workspace, name_list)
    else:
        actual_ws = workspace
        run_list = name_list     # empty or None
...
def get_run_property_dicts():
    ...
    user_to_actual = {"box": "box_name", "created": "create_time", "child": "is_child", "cluster": "cluster",
        ...
        "node": "node", "outer": "is_outer", "parent": "is_parent", "path": "path", 
...
def get_filtered_sorted_limit_runs(store, config, show_gathering, col_dict=None, args=None):
    ...
    # optional
    pool = args["target"] if "target" in args else None
    available = args["available"] if "available" in args else None
    ...
    run_list, actual_ws = expand_run_list(store, mongo, workspace, run_list)

    # build a filter dict for all specified filters
    filter_dict = build_run_filter_dict(run_list, user_to_actual, builder, args)
