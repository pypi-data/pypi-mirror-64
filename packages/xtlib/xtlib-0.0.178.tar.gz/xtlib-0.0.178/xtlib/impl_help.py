...
class ImplHelp(ImplBase):
    ...
    def command_summary(self):
        ...
        console.print("Usage: xt [FLAGS] COMMAND [ARGS]...")
        console.print()
        ...
        console.print("  To get help for a specific command, use 'xt help <command>'")
    ...
    def gen_args(self, args, gen_docs=False):
        ...
        if args:
            if gen_docs:
                ...
                text += "Arguments:\n"
            text += self.gen_name_help_aligned(args, separator="-")
            #text += "\n"
        return text
    ...
    def gen_options(self, options, gen_docs=False):
        text = ""    
        if options:       
    ...
    def command_help(self, cmd_info, syntax_only=False, args_only=False):
        ...
        if opts_before_cmd:
            usage = "Usage: xt" + opts_text + words + args_text
        else:
            usage = "Usage: xt" + words + args_text + opts_text
    ...
    @argument(name="dest_dir", help="the local directory where the files are written")
