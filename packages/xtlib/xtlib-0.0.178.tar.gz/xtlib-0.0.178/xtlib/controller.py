...
class XTController(rpyc.Service):
    ...
    def reset_state(self):
        ...
        console.print("max_runs={}, my_ip_addr={}, multi_run_context_fn={}, multi_run_hold_open={}, port={}, is_aml={}".format(
            self.max_runs, self.my_ip_addr, self.multi_run_context_fn, self.multi_run_hold_open, self.port, self.is_aml))
    ...
    def queue_check(self, max_starts=1):
        ...
        if active_count == 0 and not self.restarting:
            self.on_idle()
            ...
            if run_info:
                if run_info.parent_prep_needed:
                    ...
                    if entry:
                        ...
                        restarting = entry["status"] == "restarted"

                        print("==> running INDEX for job={}, run={}: {}/{}".format(run_info.job_id, run_info.run_name, run_index, context.total_run_count))

                        # yes: CREATE CHILD
                        self.run_template(run_info, run_index, restarting)
    ...
    def run_template(self, run_info, run_index, restarting):
        run_name = run_info.run_name
        ...
        child_info = self.spawn_child(run_info, run_index, restarting)
    ...
    def spawn_child(self, parent, run_index, restarting):
        ...
        child_info = RunInfo(child_name, context.ws, cmd_parts, context.run_script, 
            ...
            aml_run=child_aml_run, node_id=self.node_id, run_index=run_index)
