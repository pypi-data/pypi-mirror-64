...
class CmdCore():
    ...
    def get_client_context(self, exper_name, run_name, app_info, box_info, job_id, node_index, run_specs, resume_name=None, 
        ...
        context.max_runs = args["concurrent"]
    ...
    def adjust_job_for_controller_run(self, job_id, job_runs, cmds, using_hp, experiment, service_type, 
        ...
        submit direct job:
            ...
            - backend.source_files: the user's source files
        ...
        # write 1st file to SNAPSHOT (list of node_contexts, 1 for each node)
        fn_context = snapshot_dir + "/" + constants.FN_MULTI_RUN_CONTEXT 
        file_utils.ensure_dir_exists(file=fn_context)
        
        # write MULTI_RUN_CONTEXT file (used by controller)
    ...
    def adjust_box_runs_for_controller(self, node_id, box_runs):
        ...
        run_data = {"box_info": box_info, "box_name": box_info.box_name, "repeat": repeat_count, "run_name": run_name, "run_specs": run_specs}
    ...
    def create_backend(self, compute, username=None, cluster=None, vc=None):
        ...
        # if service_type in ["local", "pool"]:
        #     # POOL
        #     backend = backend_pool.PoolBackend(compute, compute_def, username, core=self)

        # elif service_type == "philly":
        #     # PHILLY
        #     backend = backend_philly.Philly(compute, compute_def, username=username, cluster=cluster, vc=vc, core=self)

        # elif service_type == "aml":
        #     # AML
        #     backend = backend_aml.AzureML(compute, compute_def, username, core=self)

        # elif service_type == "batch":
        #     # BATCH 
        #     backend = backend_batch.AzureBatch(compute, compute_def, core=self)
        # else:
        #     errors.internal_error("unexpected service_type: " + service_type)
