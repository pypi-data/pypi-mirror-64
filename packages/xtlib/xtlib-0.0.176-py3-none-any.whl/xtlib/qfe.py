...
class Dispatcher():
    ...
    def add_default_args_for_cmd(self, cmd, arg_dict):
        cmd_info = get_command("run")
        options = cmd_info["options"]
        arguments = cmd_info["arguments"]

        self.add_default_args(arguments, options, arg_dict)
    ...
    def add_default_args(self, arguments, options, arg_dict):
        ...
        for info in all_args:
            ...
            if not name in arg_dict:
                ...
                default_value = info["default"] if "default" in info else None
    ...
    def dispatch(self, args, is_rerun=False, capture_output=False):
        
        # TODO: change to cmd_parts parsing, which naturally separates options cleanly (utils.cmd_split)

        # add back quotes that were removed by windows/shell command processor
        #console.print("BEFORE adding quotes, args=", args)
        self.add_quotes_to_string_args(args)
        #console.print("AFTER adding quotes, args=", args)
        ...
        # ensure all key have "-" replaced with "-"
        arg_dict = { key.replace("-", "_"): value for key, value in arg_dict.items() }
        ...
        if self.preprocessor:
            self.preprocessor("command", caller, arg_dict)

        if cmd_info["pass_by_args"]:
            func(caller, args=arg_dict)
        else:
            func(caller, **arg_dict)
