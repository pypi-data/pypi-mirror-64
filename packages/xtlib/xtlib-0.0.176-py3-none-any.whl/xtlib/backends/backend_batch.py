...
class AzureBatch(BackendBase):
    ...
    def wrap_cmd_for_batch(self, user_cmd, node_id, store_data_dir, data_action, data_writable, 
        ...
        if code_zip and code_zip != "none":
            ...
            self.append(cmds, 'unzip -n ' + constants.CODE_ZIP_FN)
    ...
    def submit_job(self, job_id, job_runs, workspace, compute_def, resume_name, 
        ...
        monitor_url = None    
        batch_job_id = job_id     # same job_id as xt

        return batch_job_id, monitor_url    
    ...
    def wrapup_target_run(self, store, ws, run_name):
        '''
        wrap up a run from an azure self.  run may have started, or may have completed.  
        '''
        ...
        if not end_record:
            ...
            store.wrapup_run(ws, run_name, aggregate_dest, dest_name, status, exit_code, 
                primary_metric, maximize_metric, report_rollup, rundir, after_files_list, log, capture, job_id=job_id)
    ...
    def cancel_runs_by_names(self, workspace, run_names, box_name):
        ...
        if ip_addr and controller_port:
            ...
            try:
                # connect to CONTROLLER on batch node
                
                # avoid threading problems by creating a new client for each service node we connect to (for now)
    ...
    def cancel_runs_by_job(self, job_id, runs_by_box, workspace=None):
        ...
        # for now, do both of these to ensure the job is completed shut down
        ...
        if terminate_job_explictly:
            ...
            try:
                ...
                #raise ex
                #errors.user_error("job not running: " + job_id)
                pass
