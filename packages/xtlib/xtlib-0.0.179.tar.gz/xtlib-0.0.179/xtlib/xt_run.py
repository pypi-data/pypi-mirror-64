...
def main(cmd=None, disable_quickstart = False, capture_output=False):
    ...
    if not cmd or cmd == "$":
        # get from env var xt_args
        cmd = os.getenv("xt_args", "")
        cmd = cmd.strip()

    console.diag("fixed cmd={}".format(cmd))
    ...
    if not use_server or disable_quickstart:
        ...
        output = xt_cmds.main(cmd, capture_output=capture_output)
    else:
        # QUICK-START mode
        output = None
