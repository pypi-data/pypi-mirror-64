...
class ReportBuilder():
    ...
    def wildcard_matches_in_list(self, wc_name, name_list):
        matches = []

        if name_list:
            matches = [name for name in name_list if fnmatch(name, wc_name)]
            
        return matches

    def get_requested_cols(self, request_list, avail_list):
        #console.print("request_list=", request_list)
        #console.print("avail_list=", avail_list)
        actual_cols = []
        ...
        for key in request_list:
            ...
            if "*" in key:
                # wildcard match
                 matches = self.wildcard_matches_in_list(key, avail_list)
                 actual_cols += matches
            elif key in avail_list:
                actual_cols.append(key)
        
        #console.print("actual_cols=", actual_cols)
        return actual_cols
    ...
    def flatten_records(self, records, sort_col, args):
        ...
        avail_list.sort()       # make it easy to debug

        # parse out the custom column NAMES and FORMATS provided by the user
        requested_list = utils.safe_value(args, "columns", [])
        #requested_list.append(sort_col)
        user_col_args = self.build_user_col_args(requested_list)

        # rebuild requested_list to be true requested columns
        requested_list = list(user_col_args.keys())
    ...
    def get_mongo_records(self, mongo, filter_dict, workspace, which, actual_to_user, 
        ...
        if not all and not first and not last:
            last = 10
    ...
    def build_report(self, records, report_type, args):
        ...
        if fn_export:
            ...
            #self.old_generate_report(records, avail_list, actual_cols)
    ...
    def build_formatted_table(self, records, avail_cols, col_list=None, max_col_width=None, 
        ...
        if not col_list:
            col_list = avail_cols
        #console.print("col_list=", col_list)
    ...
    def generate_report(self, col_infos, records, right_align_num_cols, uppercase_hdr_cols, truncate_with_ellipses, 
        ...
        for col_info in col_infos:
            ...
            col_name = col_info["user_name"].upper() if uppercase_hdr_cols else col_info["name"]
