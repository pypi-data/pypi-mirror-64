...
class KitchenerWilmotHydroAPI:
    ...
    def download_hourly_data(self, start_date=None, end_date=None):
        ...
        try:
            ...
            self._hourly_data_directory = os.path.join(self._data_directory,
                                                       self.name,
                                                       'hourly data')
            ...
            for date in date_range:
                ...
                new_filepath = os.path.join(self._hourly_data_directory,
                                            '%s.csv' % (date.isoformat()))

                if os.path.exists(new_filepath):
                    continue

                print('Downloading hourly data for %d-%02d-01 to %d-%02d-%02d...' % (
                ...
                url = ('https://www3.kwhydro.on.ca/app/capricorn?para=smartMeterConsum&inquiryType=hydro'
                ...
                def is_valid_date_range():
                    ...
                    try:
                        valid_date_range = not self._driver.find_element_by_class_name('alert.alert-danger'). \
                ...
                index = pd.date_range(df_csv['Reading Date'].iloc[0],
                ...
                # previous files
                if date == yesterday:
                    files = glob.glob(os.path.join(self._hourly_data_directory,
    ...
    def download_invoices(self, start_date=None, end_date=None, timeout=5):
        ...
        try:
            ...
            rows = [[y for y in x.find_elements_by_tag_name('td')] for x in
                    bills_table.find_element_by_tag_name('tbody'). \
