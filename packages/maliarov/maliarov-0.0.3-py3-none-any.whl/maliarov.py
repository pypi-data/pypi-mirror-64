...
class CommonActions():
    ...
    def create_dates_list(self, start_date, end_date, input_format = "%m/%d/%Y", output_format = "%m/%d/%Y"):
        sdate = datetime.strptime(start_date, input_format)
        edate = datetime.strptime(end_date, input_format)
        date_list = [(edate - timedelta(days=x)).strftime(output_format) for x in range((edate - sdate).days + 1)]
        return date_list
