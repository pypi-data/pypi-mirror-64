"""
   ...
   53、overlap_with：重叠命令。本命令查找有无和角色最小矩形重叠的项目（包括线条，圆点，多边形，圆弧，图章，填充区域，其它角色)。它的参数可以是序列、整数与角色或字符串，返回重叠的所有项目编号。
...
class Sprite(Turtle):
    ...
    def _undo(self, action, data):
        ...
        if action == "rot":
            ...
            self._undogoto(data)
        elif action in ["wri", "dot","arc","polygon"]:
            item = data[0]
...
if __name__ == "__main__":  
  ...
  im = Image.open(path + os.sep + 'ww.png')
  screen = Screen()
  screen.colormode(255)
  screen.bgpic(path + os.sep + "bizhi.jpg")
