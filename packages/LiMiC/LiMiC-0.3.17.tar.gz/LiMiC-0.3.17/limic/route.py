...
CONFIG = [
    ...
    ("cnx",{'help':"route using CNX graph",'args':[SOURCE,TARGET,OUT,VISUALIZE,GRAPH]}),
...
def astar_cnx(g,source_id=None,target_id=None):
    ...
    #TODO handle locate_by_id correctly by constructing ids?
    cs,node2closest,node2c = g
    if isinstance(source_id,tuple) and isinstance(target_id,tuple):
        source, target = source_id[0], target_id[0]
    else:
    ...
    if source not in node2closest:
        if target in node2closest or node2c[source] != node2c[target]:
            ...
            path = None,[(0.,0.,False)+source,(float('inf'),dist,True)+target]
            return path
        path = astar_nx(c,(source,source_id),(target,target_id))
        return path
    paths = []
    def extend(cp1,cp2):
        cost = cp1[0]
    ...
    for ns,ps,cs in node2closest[source]:
        for nt,pt,ct in node2closest[target]:
            ...
            if c != node2c[nt]:
                ...
                path = None,[(0.,0.,False)+source,(float('inf'),dist,True)+target]
                return path
            cp = astar_nx(ps,(source,None),(ns,None))
            cpc = astar_nx(c,(ns,None),(nt,None))
            for u,v in zip(cpc[1],cpc[1][1:]):
                u,v = u[3:], v[3:]
                cuv = c[u][v]
                sg = cuv["path"]
                cpuv = astar_nx(sg,(u,None),(v,None))
                cp = extend(cp,cpuv)
            cpt = astar_nx(pt,(nt,None),(target,None))
            cp = extend(cp,cpt)
            paths.append(cp)
    return min(paths)

def route_cnx(file_name,source_id=None,target_id=None,out_file=None,visualize=False):
    from limic.util import start, end, load_pickled, save_path
    start("Loading from",file_name)
    g = load_pickled(file_name)
    end()
