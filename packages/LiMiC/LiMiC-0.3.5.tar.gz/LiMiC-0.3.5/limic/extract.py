...
def osm_post(lim,file_name_out,around=1000,eps=0.01,safe_dist=100,penalize=20):
    ...
    towers_tree = KDTree(list(map(lambda x:transformer.transform(x.latlon[0],x.latlon[1]),towers))) if towers else None
    m_towers = (towers,towers_tree,transformer)
    nodes_tree = KDTree(list(map(lambda x:transformer.transform(x.latlon[0],x.latlon[1]),nodes))) if nodes else None
    m_nodes = (nodes,nodes_tree,transformer)
    end()
    #print(m_towers.size)
    #print(m_nodes.size)
...
def osm_pre(file_name_in,white=[("power","line")],black=[("power","substation")],conserve_mem=False):
    from limic.util import start, end, status, file_size, save_pickled, split
    ...
    f = bopen(file_name_in,"rt") if file_name_in[-4:] == ".bz2" else open(file_name_in,"rt")
    if not conserve_mem:
    ...
    status(len(nodes))
    start("Checking that all nodes were extracted for",file_name_in)
    assert(set(id2lines.keys())==set(map(lambda x:x.id,towers)))
    assert(set(id2substations.keys())==set(map(lambda x:x.id,nodes)))
    end()
