...
class WeAdapter:
  ...
  def servo_write(self,slot,angle):  # slot:1~4  angle:0~180
    cmd='M11 '+str(self.port)+' 2 2 '+str(slot)+' '+str(angle)+'\r\n'
    weSerial.write(bytearray(cmd.encode('utf-8')))
    time.sleep(0.05)
  ...
  def rgb_show(self,slot,index,color):  # slot:1~4  index:0(All),1,2,3,...  color:#FFFFFF
    ...
    green=(aa[1])
    blue=(aa[2])
    cmd='M11 '+str(self.port)+' 3 5 '+str(slot)+' '+str(index)+' '+str(red)+' '+str(green)+' '+str(blue)+'\r\n'
    weSerial.write(bytearray(cmd.encode('utf-8')))
    time.sleep(0.05)

  def ds18b20_read(self,slot):
    cmd='M13 '+str(self.port)+' '+str(0x04+slot-1)+' 2\r\n'
    weSerial.write(bytearray(cmd.encode('utf-8')))
    time.sleep(0.05)
    count = weSerial.inWaiting() 
    if count > 0:
      value=weSerial.read(count)
      if len(value) == count:
        return round((value[1]<<8|value[0])/100.00,2)

  def analog_read(self,slot):
    cmd='M13 '+str(self.port)+' '+str(0x08+slot-1)+' 2\r\n'
    weSerial.write(bytearray(cmd.encode('utf-8')))
    time.sleep(0.05)
    count = weSerial.inWaiting() 
    if count > 0:
  ...
  def digital_read(self,slot):
    cmd='M13 '+str(self.port)+' '+str(0x0d+slot-1)+' 1\r\n'
    weSerial.write(bytearray(cmd.encode('utf-8')))
    time.sleep(0.05)
    count = weSerial.inWaiting() 
    if count > 0:
  ...
  def digital_write(self,slot,value):  # slot:1~4  value:0/1
    cmd='M11 '+str(self.port)+' 12 2 '+str(slot)+' '+str(vaule)+'\r\n'
    weSerial.write(bytearray(cmd.encode('utf-8')))
    time.sleep(0.05)