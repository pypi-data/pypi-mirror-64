...
class ResiliencyLinkBrownoutPacketDuplication(SdWanTest):
    """
    Class for test case ResiliencyLinkBrownoutPacketDuplication
    """
    ...
    def run(self):
        ...
        if self.error_info:
            ...
            logger.console_info("done")
        #######################################################################################
        # Make packets duplication ratio > 5% by SNE, then traffic should switch to Mpls link #
        #######################################################################################
        ...
        if self.error_info:
            ...
            stc.perform('GeneratorStopCommand', GeneratorList=[generator_local_head])
            time.sleep(2)
            duplicate_frame_tcp01 = stc.get(stc.get(stream_tcp01, 'children-rxstreamsummaryresults'))['DuplicateFrameCount']
            duplicate_frame_udp01 = stc.get(stc.get(stream_tcp01, 'children-rxstreamsummaryresults'))['DuplicateFrameCount']
            logger.info('Duplicate Frame', duplicate_frame_tcp01=duplicate_frame_tcp01, duplicate_frame_udp01=duplicate_frame_udp01)
