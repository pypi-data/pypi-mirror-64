"""
Module to test blackout remote no congestion functionality of DUT/SUT
"""
import os
import sys
import time
...
class ResiliencyLinkBlackoutRemoteNoCongestion(SdWanTest):
    """
    Class for test case ResiliencyLinkBlackoutRemoteNoCongestion
    """
    ...
    def run(self):
        ...
        self.save_stc_results(result_file_name=self.test_input.testcase_id + '_before_sne_config.db')
        if self.error_info:
        ...
        ##########################################################################
        logger.console_info('Blackout Internet link by SNE and verify traffic switch to Mpls link...')
        sne.configure()
        ...
        self.save_stc_results(result_file_name=self.test_input.testcase_id + '_after_sne_config.db')
        if self.error_info:
            logger.info('Traffic can not switch to Mpls link in {0}s after blackout Internet link by SNE, failure info in detail as below.'.format(config_global.max_wait_time_after_break_link))
            logger.info(self.error_info)
            stc.perform('GeneratorStopCommand', GeneratorList=[generator_local_head])
            time.sleep(2)
            ...
            raise RuntimeError('Traffic can not switch to Mpls link in {0}s after blackout Internet link by SNE.'.format(config_global.max_wait_time_after_break_link))
        else:
            stc.perform('GeneratorStopCommand', GeneratorList=[generator_local_head])
            time.sleep(2)
