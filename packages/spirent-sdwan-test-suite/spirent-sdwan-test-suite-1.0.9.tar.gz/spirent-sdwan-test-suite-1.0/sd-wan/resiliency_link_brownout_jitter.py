"""
Module to test brownout jitter functionality of DUT/SUT
"""
import os
import sys
import time
...
class ResiliencyLinkBrownoutJitter(SdWanTest):
    """
    Class for test case ResiliencyLinkBrownoutJitter
    """
    ...
    def run(self):
        ...
        stc.config(stc.get(project, 'children-ResultOptions'), ResultViewMode='LATENCY_JITTER')
        #########################
        ...
        time.sleep(10)
        self.save_stc_results(result_file_name=self.test_input.testcase_id + '_internet_link.db')
        ...
        if self.error_info:
            ...
            logger.console_info("done")
        ####################################################################
        # Make jitter>50ms by SNE, then traffic should switch to Mpls link #
        ####################################################################
        logger.console_info('Brownout Internet link by SNE and verify traffic switch to Mpls link...')
        sne.configure()
        ...
        # It is necessary to put this statement between verify description and error_info.
        self.save_stc_results(result_file_name=self.test_input.testcase_id + '_brownout_internet_link.db')
        if self.error_info:
            logger.info('Traffic can not switch to Mpls link in {0}s after Brownout Internet link by SNE, failure info in detail as below.'.format(config_global.max_wait_time_after_break_link))
            logger.info(self.error_info)
            stc.perform('GeneratorStopCommand', GeneratorList=[generator_local_head])
            time.sleep(2)
            max_latency_tcp01 = stc.get(stc.get(stream_tcp01, 'children-rxstreamsummaryresults'))['MaxLatency']
            avg_latency_tcp01 = stc.get(stc.get(stream_tcp01, 'children-rxstreamsummaryresults'))['AvgLatency']
            max_latency_udp01 = stc.get(stc.get(stream_udp01, 'children-rxstreamsummaryresults'))['MaxLatency']
            avg_latency_udp01 = stc.get(stc.get(stream_udp01, 'children-rxstreamsummaryresults'))['AvgLatency']
            max_jitter_tcp01 = stc.get(stc.get(stream_tcp01, 'children-rxstreamsummaryresults'))['MaxJitter']
            avg_jitter_tcp01 = stc.get(stc.get(stream_tcp01, 'children-rxstreamsummaryresults'))['AvgJitter']
            max_jitter_udp01 = stc.get(stc.get(stream_udp01, 'children-rxstreamsummaryresults'))['MaxJitter']
            avg_jitter_udp01 = stc.get(stc.get(stream_udp01, 'children-rxstreamsummaryresults'))['AvgJitter']
            logger.info('Frame Latency', max_latency_tcp01=max_latency_tcp01, avg_latency_tcp01=avg_latency_tcp01, max_latency_udp01=max_latency_udp01, avg_latency_udp01=avg_latency_udp01)
            logger.info('Frame Jitter', max_jitter_tcp01=max_jitter_tcp01, avg_jitter_tcp01=avg_jitter_tcp01, max_jitter_udp01=max_jitter_udp01, avg_jitter_udp01=avg_jitter_udp01)
            raise RuntimeError('Traffic can not switch to Mpls link in {0}s after Brownout Internet link by SNE.'.format(config_global.max_wait_time_after_break_link))
        else:
            stc.perform('GeneratorStopCommand', GeneratorList=[generator_local_head])
            time.sleep(2)
            ...
            msg_robot = 'Traffic switch to mpls link successfully after brownout internet link and Out of service time is {0} ms\n'.format(time_switch)
            logger.console_info("done")
        ##################################################################################
        ...
        logger.console_info("started\n")
        time.sleep(10)
        logger.console_info('Recover Internet link by SNE and verify traffic switch back to Internet link...')
        sne.stop()
        ...
        # It is necessary to put this statement between verify description and error_info.
        self.save_stc_results(result_file_name=self.test_input.testcase_id + '_recover_internet_link.db')
        if self.error_info:
            logger.info('Traffic can not switch back to Internet link in {0}s after Recover Internet link, failure info in detail as below.'.format(config_global.max_wait_time_after_recover_link))
            logger.info(self.error_info)
            stc.perform('GeneratorStopCommand', GeneratorList=[generator_local_head])
            raise RuntimeError('Traffic can not switch back to Internet link in {0}s after Recover Internet link.'.format(config_global.max_wait_time_after_recover_link))
        else:
            stc.perform('GeneratorStopCommand', GeneratorList=[generator_local_head])
            time.sleep(2)
            tx_frame_local_head = stc.get(stc.get(stc.get(port_handle_local_head, 'children-generator'), 'children-generatorportresults'))['GeneratorSigFrameCount']
