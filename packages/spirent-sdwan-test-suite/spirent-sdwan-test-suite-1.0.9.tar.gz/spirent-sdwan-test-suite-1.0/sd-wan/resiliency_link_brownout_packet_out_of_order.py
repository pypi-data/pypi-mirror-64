"""
Module to test brownout packet out-of-order functionality of DUT/SUT
"""
import os
import sys
import time
...
class ResiliencyLinkBrownoutPacketOutOfOrder(SdWanTest):
    """
    Class for test case ResiliencyLinkBrownoutPacketOutOfOrder
    """
    ...
    def run(self):
        ...
        if self.error_info:
            ...
            logger.console_info("done")
        ##########################################################################
        # Brownout Internet link by SNE, then traffic should switch to Mpls link #
        ##########################################################################
        ...
        if self.error_info:
            ...
            stc.perform('GeneratorStopCommand', GeneratorList=[generator_local_head])
            time.sleep(2)
            reordered_frame_tcp01 = stc.get(stc.get(stream_tcp01, 'children-rxstreamsummaryresults'))['ReorderedFrameCount']
            reordered_frame_udp01 = stc.get(stc.get(stream_tcp01, 'children-rxstreamsummaryresults'))['ReorderedFrameCount']
            logger.info('Reordered Frame', reordered_frame_tcp01=reordered_frame_tcp01, reordered_frame_udp01=reordered_frame_udp01)
