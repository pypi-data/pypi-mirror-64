...
def list_backups():
    out = run_cmd(f"kubectl get cm --no-headers -o custom-columns=Name:.metadata.name 2>&1 | grep {TRACKER_CONFIGMAP_BACKUP}")
...
class ClusterManager(object):
    ...
    def to_configmap(self, backup=False):
        return {
            ...
            "kind": "ConfigMap",
            "metadata": {
                "name": TRACKER_CONFIGMAP_BACKUP + datetime.today().strftime('%Y%m%d-%H%M') if backup else TRACKER_CONFIGMAP,
            },
