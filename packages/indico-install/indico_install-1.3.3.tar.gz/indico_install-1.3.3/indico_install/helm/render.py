...
from indico_install.utils import options_wrapper, run_cmd, string_to_tag, current_tag
    ...
    **kwargs,
    ...
    if remote_configs:
        ...
        if cluster_manager.indico_version:
            click.secho("Not using `indico updraft -v` for version changes!", fg="yellow")
        else:
        ...
        cluster_manager.indico_version = current_tag(deployment_root)
    else:
        click.secho("No render version supplied! Please use `indico updraft -v`", fg="red")
        return
    ...
    with tempfile.NamedTemporaryFile(suffix=".tmp", delete=False) as input_yaml:
        ...
        old_conf = cluster_manager.clean_services()
        input_yaml.write(
