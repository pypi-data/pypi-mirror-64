    ...
    for i in range(0, test_number, max_batch):
        ...
        if type(input_shape1) is list:
            ...
            for i1 in input_shape1:
                matrix.append(np.random.uniform(-1.0, 1.0, (tst,) + i1[1:]))
        else:
            # None shape fix
            ...
            for i in range(1, len(inp_shape_fix)):
                if inp_shape_fix[i] is None:
                    inp_shape_fix[i] = 224
            matrix = np.random.uniform(-1.0, 1.0, (tst,) + tuple(inp_shape_fix[1:]))
...
if __name__ == '__main__':
    ...
    for model_name in models_to_test:
        ...
        elif model_name in ['mobilenet_small', 'conv3d_model']:
