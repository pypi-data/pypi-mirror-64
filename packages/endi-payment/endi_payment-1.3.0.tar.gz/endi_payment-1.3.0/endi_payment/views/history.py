...
ACTION_LABELS = {
    'ADD': u'Ajout', 'UPDATE': u'Modification', 'DELETE': u'Suppression'
}


class HistoryListView(BaseListView):
    title = u"Journal des modifications sur les encaissements"
    add_template_vars = (
        'title',
        'action_labels',
        'stream_actions',
    ...
    def filter_created_at(self, query, appstruct):
        ...
        if period.get('start') not in self.null_values:
            logger.debug(u"  + Filtering by date : %s" % period)
    ...
    def filter_amount(self, query, appstruct):
        ...
        if amounts.get('start') not in self.null_values:
            logger.debug(u"Filtering by amount : %s" % amounts)
    ...
    def filter_invoice_id(self, query, appstruct):
        ...
        if invoice_id:
            logger.debug(u"Filtering by invoice_id {}".format(invoice_id))
    ...
    def filter_invoice_number(self, query, appstruct):
        ...
        if invoice_number:
            ...
            logger.debug(
                u"Filtering by invoice_id {} (number {})".format(
    ...
    def stream_actions(self, item):
        yield Link(
            ...
            u"Voir",
            title=u"Voir le détail de cette entrée",
            icon="arrow-right"
        )
...
def history_item_view(request):
    ...
    return dict(
        title=u"Vue détaillée d'une entrée du journal de modification des "
        u"encaissements",
        history=history,
...
def includeme(config):
    ...
    config.add_admin_menu(
        parent="accounting",
        permission="admin_treasury",
        label=u"Journaux des encaissements",
