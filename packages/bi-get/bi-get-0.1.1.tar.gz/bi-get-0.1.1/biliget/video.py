...
from biliget.vtools.headers import get_header
from biliget.vtools.downloader import CCDownloader, SingleVideoDownloader
from utils.pyffmpeg import merge, merge_cc, merge_double_cc
import os
from biliget.vtools.utils import _get_video_message
...
class Video:
    ...
    def access(self):
        video_msg = _get_video_message(self.bv, self.page_list[0]['cid'])
        tags = self._get_tags_info(video_msg['aid'])
        bullet_comments = self._save_bullet_comments()
        self.data = {
            'bvid': video_msg['bvid'],
            'aid': video_msg['aid'],
    ...
    def _save_bullet_comments(self):
        url = 'https://api.bilibili.com/x/v1/dm/list.so'
        params = {
            'oid': self.page_list[0]['cid']
        }
