...
##
# Module/script example of the xlrd_demo API for extracting information
# about named references, named constants, etc.
#
# <p>Copyright © 2006 Stephen John Machin, Lingfo Pty Ltd</p>
# <p>This module is part of the xlrd_demo package, which is released under a BSD-style licence.</p>
##
...
import xlrd_demo
from xlrd_demo.timemachine import REPR
...
def show_name_details(book, name, show_contents=0, f=sys.stdout):
    """
    book -- Book object obtained from xlrd_demo.open_workbook().
...
def showable_cell_value(celltype, cellvalue, datemode):
    if celltype == xlrd_demo.XL_CELL_DATE:
        try:
            showval = xlrd_demo.xldate_as_tuple(cellvalue, datemode)
        except xlrd_demo.XLDateError as e:
            showval = "%s:%s" % (type(e).__name__, e)
    elif celltype == xlrd_demo.XL_CELL_ERROR:
        showval = xlrd_demo.error_text_from_code.get(
...
def show_name_object(book, nobj, show_contents=0, f=sys.stdout):
    ...
    if kind >= 0:
        ...
        pass
    elif kind == xlrd_demo.oREL:
        # A list of Ref3D objects representing *relative* ranges
        for i in range(len(value)):
            ref3d = value[i]
            print("Range %d: %s ==> %s"% (i, REPR(ref3d.coords), REPR(xlrd_demo.rangename3drel(book, ref3d))), file=f)
    elif kind == xlrd_demo.oREF:
        # A list of Ref3D objects
        for i in range(len(value)):
            ref3d = value[i]
            print("Range %d: %s ==> %s"% (i, REPR(ref3d.coords), REPR(xlrd_demo.rangename3d(book, ref3d))), file=f)
            ...
            for shx in range(ref3d.shtxlo, ref3d.shtxhi):
                ...
                for rowx in range(ref3d.rowxlo, rowlim):
                    for colx in range(ref3d.colxlo, collim):
                        ...
                        if cty == xlrd_demo.XL_CELL_EMPTY and show_contents == 1:
                            continue
                        cval = sh.cell_value(rowx, colx)
                        sval = showable_cell_value(cty, cval, datemode)
                        print("      (%3d,%3d) %-5s: %s"
                            % (rowx, colx, xlrd_demo.cellname(rowx, colx), REPR(sval)), file=f)

if __name__ == "__main__":
    def usage():
        text = """
usage: xlrd_demonameAIPdemo.py glob_pattern name scope show_contents
...
"""
    ...
    for fname in glob.glob(arg_pattern):
        book = xlrd_demo.open_workbook(fname)
        if arg_name == "*":
