...
import xlrd_demo
from xlrd_demo.timemachine import xrange
...
class TestSheet(TestCase):
    ...
    def setUp(self):
        self.book = xlrd_demo.open_workbook(from_this_dir('profiles.xls'), formatting_info=True)
    ...
    def test_cell(self):
        sheet = self.book.sheet_by_index(SHEETINDEX)
        self.assertNotEqual(xlrd_demo.empty_cell, sheet.cell(0, 0))
        self.assertNotEqual(xlrd_demo.empty_cell, sheet.cell(NROWS-1, NCOLS-1))
    ...
    def test_getitem_tuple(self):
        sheet = self.book.sheet_by_index(SHEETINDEX)
        self.assertNotEqual(xlrd_demo.empty_cell, sheet[0, 0])
        self.assertNotEqual(xlrd_demo.empty_cell, sheet[NROWS-1, NCOLS-1])
...
class TestSheetRagged(TestCase):
    ...
    def test_read_ragged(self):
        book = xlrd_demo.open_workbook(from_this_dir('ragged.xls'), ragged_rows=True)
...
class TestMergedCells(TestCase):
    ...
    def test_tidy_dimensions(self):
        book = xlrd_demo.open_workbook(from_this_dir('merged_cells.xlsx'))
