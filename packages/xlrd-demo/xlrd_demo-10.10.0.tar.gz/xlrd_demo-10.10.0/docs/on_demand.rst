...
to the :func:`~xlrd_demo.open_workbook` function and allows saving memory and time by
loading only those sheets that the caller is interested in, and releasing sheets
when no longer required.

``on_demand=False`` (default):
  No change. :func:`~xlrd_demo.open_workbook` loads global data
...
``on_demand=True`` and BIFF version >= 5.0:
  :func:`~xlrd_demo.open_workbook` loads global
