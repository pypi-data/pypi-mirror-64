...
.. currentmodule:: xlrd_demo.xldate
...
2. Excel for Windows stores dates by default as the number of
   ...
   from a workbook, you must use the :attr:`~xlrd_demo.Book.datemode` attribute of
   the :class:`~xlrd_demo.Book` object. If you guess, or make a judgement depending
...
4. The Macintosh-default 1904-based date system counts ``1904-01-02`` as day 1
   ...
   The functions in :mod:`~xlrd_demo.xldate` take the view that such a number is a
...
5. Usage of the Excel ``DATE()`` function may leave strange dates in a
   ...
   functions in :mod:`~xlrd_demo.xldate`.
