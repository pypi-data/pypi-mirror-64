...
# This script is part of the xlrd_demo package, which is released under a
# BSD-style licence.
...
version[0]      Print versions of xlrd_demo and Python and exit
...
if __name__ == "__main__":
    ...
    from xlrd_demo.timemachine import xrange, REPR
    ...
    null_cell = xlrd_demo.empty_cell
            ...
            for colx, ty, val, cxfx in get_row_data(bk, sh, rowx, colrange):
                if printit:
                    print("cell %s%d: type=%d, data: %r, xfx: %s"
                        % (xlrd_demo.colname(colx), rowx+1, ty, val, cxfx))
        else:
            for colx, ty, val, _unused in get_row_data(bk, sh, rowx, colrange):
                if printit:
                    print("cell %s%d: type=%d, data: %r" % (xlrd_demo.colname(colx), rowx+1, ty, val))
    ...
    def get_row_data(bk, sh, rowx, colrange):
        ...
        for colx in colrange:
            ...
            if bk.formatting_info:
                ...
                cxfx = ''
            if cty == xlrd_demo.XL_CELL_DATE:
                try:
                    showval = xlrd_demo.xldate_as_tuple(cval, dmode)
                except xlrd_demo.XLDateError as e:
                    showval = "%s:%s" % (type(e).__name__, e)
                    cty = xlrd_demo.XL_CELL_ERROR
            elif cty == xlrd_demo.XL_CELL_ERROR:
                showval = xlrd_demo.error_text_from_code.get(cval, '<Unknown error code 0x%02x>' % cval)
            else:
                showval = cval
    ...
    def bk_header(bk):
        ...
        print("BIFF version: %s; datemode: %s"
            % (xlrd_demo.biff_text_from_num[bk.biff_version], bk.datemode))
    ...
    def print_labels(sh, labs, title):
        ...
        for rlo, rhi, clo, chi in labs:
            print("%s label range %s:%s contains:"
                % (title, xlrd_demo.cellname(rlo, clo), xlrd_demo.cellname(rhi-1, chi-1)))
            for rx in xrange(rlo, rhi):
                for cx in xrange(clo, chi):
                    print("    %s: %r" % (xlrd_demo.cellname(rx, cx), sh.cell_value(rx, cx)))
    ...
    def show(bk, nshow=65535, printit=1):
        bk_header(bk)
        if 0:
            rclist = xlrd_demo.sheet.rc_stats.items()
    ...
    def main(cmd_args):
        ...
        cmd = args[0]
        xlrd_demo_version = getattr(xlrd_demo, "__VERSION__", "unknown; before 0.5")
        if cmd == 'biff_dump':
            xlrd_demo.dump(args[1], unnumbered=options.unnumbered)
            sys.exit(0)
        if cmd == 'biff_count':
            xlrd_demo.count_records(args[1])
            sys.exit(0)
        if cmd == 'version':
            print("xlrd_demo: %s, from %s" % (xlrd_demo_version, xlrd_demo.__file__))
        ...
        for pattern in args[1:]:
            for fname in glob.glob(pattern):
                ...
                except xlrd_demo.XLRDError as e:
