...
# 都会实时变
# print("1", Config.zk_cfg)
# from ..utils import zk
# print('3', zk.Config.zk_cfg)
# print('4', zk.Config_.zk_cfg)
...
@app.post("/app/email/{biz}")
async def report(request: Request, kwargs: dict, biz):
    """插数、发邮件
    ...
    try:
        # 通过 path_params 获取邮件配置
        email_config = Config.zk_cfg.get('email', {}).get(biz, 'biz').copy()
        email_config['date'] = dic['date']
        email_config['msg'] = kwargs.get('msg', '邮件测试信息')
...
@app.post("/app/mongo/form/{collection}")
async def report(request: Request, collection):
    """插数、发邮件
    jobtype参数触发邮件服务(需要润色html或者web或者sreamlit)
    TODO: 耗时信息
    """
    ...
    try:
        ...
        params = dic['querypath']  # jobid jobkey biz
        # jobtype in （'hive', 'spark') 触发邮件服务
        if params.get('jobtype') == 'hive':
            dic['status'] = 'succeed'
            print(dic)
            ...
            # 通过 querypath 获取邮件配置
            email_config = Config.zk_cfg.get('email', {}).get(params.get("biz", "biz")).copy()  # 会变
            email_config['date'] = dic['date']
            email_config['msg'] = df2html.df2html(email_config['subject'], email_config['date'], df.to_html())
            email_config['msg_fn'] = eval(email_config['msg_fn'])
            send_email.send_email(**email_config)  # TODO美化：add 责任人
