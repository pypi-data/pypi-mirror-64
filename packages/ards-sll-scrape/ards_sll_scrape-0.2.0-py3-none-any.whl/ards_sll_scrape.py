...
class CollectDatabase:
    ...
    def queryUnSuccessSlowLogJob(self, inst_id):
        return T_JOBSTATUS.select(T_JOBSTATUS.jobid, T_JOBSTATUS.gmt_create).where(
            (T_JOBSTATUS.status == 0) & (T_JOBSTATUS.DBInstanceId == inst_id)).order_by(T_JOBSTATUS.gmt_create.asc()).limit(1).tuples().execute()

    def queryLastSuccessSlowLogJob(self, inst_id):
        return T_JOBSTATUS.select(T_JOBSTATUS.gmt_create).where(
            (T_JOBSTATUS.status == 1) & (T_JOBSTATUS.DBInstanceId == inst_id)).order_by(T_JOBSTATUS.gmt_create.desc()).limit(1).tuples().execute()
