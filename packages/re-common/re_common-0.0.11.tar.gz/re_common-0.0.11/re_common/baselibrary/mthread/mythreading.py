...
class ThreadPoolManger(object):
    ...
    def __init_threading_pool(self, thread_num):
        ...
        for i in range(int(thread_num)):
            # 新建一个线程
            thread = ThreadManager(self.work_queue, self.result_queue, self.thread_queue, self.logger, args=self.args,
                                   kwargs=self.kwargs)
            # 线程名
            threadName = "my_threading_task_{0}".format(i + 1)
    ...
    def set_thread_pool_dicts(self, thread_pool_dicts, threadName, thread, args, kwargs):
        ...
        if self.callback:
            # 对象使用
            dicts = self.callback(thread_pool_dicts, thread, args, kwargs)
        else:
            # 继承时使用
            dicts = self.thread_pool_hook(thread_pool_dicts, thread, args, kwargs)
        if not dicts:
            dicts = {}
        dictsfull["thread"] = thread
        dictsfull["kwargs"] = kwargs
        dictsfull["args"] = args
        dictsfull["callback"] = dicts
        thread_pool_dicts[threadName] = dictsfull
    ...
    def set_task_thread(self, thread_name):
        """
        通过线程名重启线程
        :param thread_name:
        :return:
    ...
    def add_thread(self, thread_num):
        ...
        while thread_num > count:
            ...
            thread = ThreadManager(self.work_queue, self.result_queue, self.thread_queue,
                                   ...
                                   args=self.args,
                                   kwargs=self.kwargs)
            thread_name = "my_threading_thread_{}".format(num)
            if thread_name in self.thread_pool_dicts:
                continue
            thread.setName(thread_name)
            # self.threadpool.append(thread)
    ...
    def __set_add_thread(self, func, thread_name, *args, **kwargs):
        thread = ThreadManager(self.work_queue, self.result_queue, self.thread_queue,
                               self.logger,
    ...
    def set_add_especial_thread(self, func, threadname, *args, **kwargs):
        ...
        """
        # 得到一个线程对象
        thread = self.__set_add_thread(func, threadname, *args, **kwargs)
    ...
    def start(self):
        for thread in self.thread_pool_dicts:
            ...
            if self.thread_pool_dicts[thread]:
                thread = self.thread_pool_dicts[thread]["thread"]
    ...
    def especial_start(self):
        for threadname in self.especial_thread_pool_dicts:
            if self.especial_thread_pool_dicts[threadname]:
                thread = self.especial_thread_pool_dicts[threadname]["thread"]
                if thread and not thread.is_alive():
    ...
    def checkThread(self):
        ...
        for name in list(self.thread_pool_dicts.keys()):
            ...
            if name in nowThreadsName and thread.is_alive():
                ...
                self.set_task_thread(name)  # 重启线程
        # 如果不是静态最大值 线程可以波动
    ...
    def checkThreadRunFinish(self):
        ...
        if self.result_queue.empty() and self.work_queue.empty():
            # self.event.set()
            time.sleep(10)
            t1 = self.result_queue.empty()
            time.sleep(10)
            t2 = self.result_queue.empty()
            if t1 and t2:
