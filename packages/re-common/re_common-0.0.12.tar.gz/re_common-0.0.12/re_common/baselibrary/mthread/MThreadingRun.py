...
class MThreadingRun(ABC):
    ...
    def setProxy(self, proxysList=None):
        ...
        if proxysList:
            ...
            for proxy in proxysList:
                if proxy:
                    # 判断代理是否在环型列表中
                    if not self.list_proxy.exist(proxy):
                        # 没在则加入
    ...
    def __setProxy(self, *args, **kwargs):
        """
        设置代理的循环
        :param args:
        :param kwargs:
    ...
    def deal_results_no_event(self, *args, **kwargs):
        ...
        while True:
            ...
            if len(
                    self.results) > self.dealresultsnum or self.thread_pool.work_queue.empty() or not self.thread_pool.thread_queue:
                if len(self.results) > 0 or not self.thread_pool.thread_queue.empty():
                    # 处理结果
                    self.dealresult()
    ...
    def deal_results(self, *args, **kwargs):
        ...
        while True:
            ...
            # 从结果队列获取结果到results
            self.getreustlFromQueue()
            if len(self.results) > self.dealresultsnum or self.thread_pool.work_queue_is_empty() \
                    or not self.thread_pool.thread_queue.empty():
                if len(self.results) > 0 or not self.thread_pool.thread_queue.empty():
                    # 处理结果
    ...
    def getreustlFromQueue(self):
        # 从结果队列获取结果到results
        once_result_num = 0
        while not self.thread_pool.result_queue_is_empty():
            self.resultnum += 1
            once_result_num += 1
    ...
    def other(self):
        self.logger.info("工作队列任务量为{},结果队列任务量为{}".format(self.thread_pool.get_workqueue_size(),
                                                        self.thread_pool.get_resultqueue_size()))
        # 通过工作队列和结果队列观察是否结束
    ...
    def check_especial_thread(self):
        ...
        for name in list(self.thread_pool.especial_thread_pool_dicts.keys()):
            thread = self.thread_pool.especial_thread_pool_dicts[name]["thread"]
            # 如果线程字典为空 代表已被删除
            if name in nowThreadsName and thread.is_alive():
                ...
                if name in ["my_threading_taskthread_1", "my_threading_proxythread_1", "my_threading_dealresult_1"]:
                    if name == "my_threading_taskthread_1":
                        taskin = task
                    elif name == "my_threading_proxythread_1":
                        taskin = proxy
                    elif name == "my_threading_dealresult_1":
                        taskin = result
                    else:
                        raise Exception("没有对应的任务，请检查")
                    is_start = False
                    if name in self.thread_pool.especial_thread_pool_dicts:
                        if self.thread_pool.especial_thread_pool_dicts[name]["thread"].is_alive():
                            is_start = True
                    if not is_start:
                        args = self.thread_pool.especial_thread_pool_dicts[name]["args"]
                        kwargs = self.thread_pool.especial_thread_pool_dicts[name]["kwargs"]
                        self.thread_pool.set_add_especial_thread(taskin, name, *args, **kwargs)

    def start_especial_thread(self):
        # 开启一个线程设置任务
        self.thread_pool.set_add_especial_thread(self.__setTask, "my_threading_taskthread_1")
        self.thread_pool.set_add_especial_thread(self.__setProxy, "my_threading_proxythread_1")
        self.thread_pool.set_add_especial_thread(self.deal_results, "my_threading_dealresult_1")
        self.thread_pool.especial_start()

    def start_especial_thread_no_evnt(self):
        # 开启一个线程设置任务
        self.thread_pool.set_add_especial_thread(self.setTask_noevent, "my_threading_taskthread_1")
        self.thread_pool.set_add_especial_thread(self.__setProxy, "my_threading_proxythread_1")
        self.thread_pool.set_add_especial_thread(self.deal_results_no_event, "my_threading_dealresult_1")
        self.thread_pool.especial_start()
    ...
    def run(self, model=1):
        ...
        while True:
            ...
            # # print(sys.getsizeof(self.thread_pool.threadpool))
            # print(sys.getsizeof(self.thread_pool.especial_thread_pool))
            # print(self.thread_pool.especial_thread_pool)
            # print("************************************************")
            # 检查线程是否因为意外死掉，自动重启
            # print("work:" + str(self.thread_pool.get_workqueue_size()))
            # print("result:" + str(self.thread_pool.get_resultqueue_size()))
            time.sleep(3)
            ...
            if self.other():
                if not self.checkResultsfininsh():
                    ...
                    if self.thread_pool.work_queue.empty() and self.thread_pool.result_queue.empty() \
