...
def pick_msg(speech_category) -> str:
    ...
    if isinstance(speech_category, SpeechCategory):
        if len(speech_category.speechs_objects) > 0:
            ...
            raise Exception(f"No speech item are in the current speech category : {speech_category}")
    else:
        raise Exception(f"The messages were not a SpeechCategory object but {speech_category}")

def pick_msg_old(msgs) -> str:
    """ The key of a dict element is the message, and the value its probabilities """
    from inoft_vocal_framework.speechs.ssml_builder_core import SpeechCategory

    if isinstance(msgs, SpeechCategory):
        msgs_dict = msgs.speechs_objects
    elif isinstance(msgs, dict):
        msgs_dict = msgs
    else:
        raise Exception(f"The messages object was neither a SpeechCategory or a dict but was {msgs}")
