...
def highs_wrapper(
    ...
    if modelstatus == 0:
        ...
        # It could be that the problem is unbounded
        # This may or may not do the right thing?
        if info.primal_status != 0 and info.dual_status != 0:
            modelstatus = 8 # PRIMAL_UNBOUNDED
            for ii in range(numcol):
                colbasisstatus[ii] = -1 # unset
            for ii in range(numrow):
                rowbasisstatus[ii] = -1 # unset
