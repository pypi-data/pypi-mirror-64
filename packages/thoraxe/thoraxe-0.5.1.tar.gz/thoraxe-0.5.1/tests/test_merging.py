...
def test_read_transcript_info(kif1b):
    ...
    assert sorted(
        transcript_table.loc[rows_a, 'TranscriptIDCluster'].unique()) == [
            'ENST00000263934', 'ENST00000377083/ENST00000377093',
            'ENST00000622724'
        ]
    assert sorted(
        transcript_table.loc[rows_b, 'TranscriptIDCluster'].unique()) == [
            'ENST00000377081', 'ENST00000377086', 'ENST00000620295'
        ]

    # the two exons share the 3' region:

    assert all(transcript_table.loc[rows_a, 'GenomicCodingStart'].unique() ==
               [10282400])
    assert all(transcript_table.loc[rows_a, 'GenomicCodingEnd'].unique() ==
               [10282533])

    assert all(transcript_table.loc[rows_b, 'GenomicCodingStart'].unique() ==
               [10282322])
    assert all(transcript_table.loc[rows_b, 'GenomicCodingEnd'].unique() ==
               [10282533])

    # there is not differences at the nucleotide level:

    assert len(
        transcript_table.loc[rows_a, 'ExonSequence'].map(str).unique()) == 1
    assert len(
        transcript_table.loc[rows_b, 'ExonSequence'].map(str).unique()) == 1

    # but the residue at the beginning changes depending on the transcript
    # i.e. there is a shared residue with another exon:

    assert len(transcript_table.loc[rows_a, 'ExonProteinSequence'].map(
        str).unique()) == 2
    assert len(transcript_table.loc[rows_b, 'ExonProteinSequence'].map(
        str).unique()) == 2
    ...
    assert sorted(
        subexon_table.loc[subexon_table.ExonIDCluster ==
                          exon_a, 'SubexonIDCluster'].unique()) == [
                              'ENSE00000955614_SE_14/ENSE00000955628_SE_14'
                          ]
    assert sorted(
        subexon_table.loc[subexon_table.ExonIDCluster ==
                          exon_b, 'SubexonIDCluster'].unique()) == [
                              'ENSE00000955614_SE_14/ENSE00000955628_SE_14',
