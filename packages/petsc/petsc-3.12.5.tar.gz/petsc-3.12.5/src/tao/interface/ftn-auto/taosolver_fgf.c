...
PETSC_EXTERN void PETSC_STDCALL  taosetresidualweights_(Tao tao,Vec sigma_v,PetscInt *n,PetscInt *rows,PetscInt *cols,PetscReal *vals, int *__ierr){
*__ierr = TaoSetResidualWeights(
	(Tao)PetscToPointer((tao) ),
	(Vec)PetscToPointer((sigma_v) ),*n,rows,cols,vals);
}
PETSC_EXTERN void PETSC_STDCALL  taocomputeresidual_(Tao tao,Vec X,Vec F, int *__ierr){
