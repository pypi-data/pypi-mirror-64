...
PETSC_EXTERN void PETSC_STDCALL  matmattransposemultequal_(Mat A,Mat B,Mat C,PetscInt *n,PetscBool *flg, int *__ierr){
*__ierr = MatMatTransposeMultEqual(
	(Mat)PetscToPointer((A) ),
	(Mat)PetscToPointer((B) ),
	(Mat)PetscToPointer((C) ),*n,flg);
}
PETSC_EXTERN void PETSC_STDCALL  matislinear_(Mat A,PetscInt *n,PetscBool  *flg, int *__ierr){
*__ierr = MatIsLinear(
