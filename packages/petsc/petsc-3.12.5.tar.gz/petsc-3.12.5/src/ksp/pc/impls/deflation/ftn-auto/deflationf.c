...
extern "C" {
#endif
PETSC_EXTERN void PETSC_STDCALL  pcdeflationsetinitonly_(PC pc,PetscBool *flg, int *__ierr){
*__ierr = PCDeflationSetInitOnly(
	(PC)PetscToPointer((pc) ),*flg);
}
PETSC_EXTERN void PETSC_STDCALL  pcdeflationsetlevels_(PC pc,PetscInt *max, int *__ierr){
*__ierr = PCDeflationSetLevels(
	(PC)PetscToPointer((pc) ),*max);
}
PETSC_EXTERN void PETSC_STDCALL  pcdeflationsetreductionfactor_(PC pc,PetscInt *red, int *__ierr){
*__ierr = PCDeflationSetReductionFactor(
	(PC)PetscToPointer((pc) ),*red);
}
PETSC_EXTERN void PETSC_STDCALL  pcdeflationsetcorrectionfactor_(PC pc,PetscScalar *fact, int *__ierr){
*__ierr = PCDeflationSetCorrectionFactor(
	(PC)PetscToPointer((pc) ),*fact);
}
PETSC_EXTERN void PETSC_STDCALL  pcdeflationsetspacetocompute_(PC pc,PCDeflationSpaceType *type,PetscInt *size, int *__ierr){
*__ierr = PCDeflationSetSpaceToCompute(
	(PC)PetscToPointer((pc) ),*type,*size);
}
PETSC_EXTERN void PETSC_STDCALL  pcdeflationsetspace_(PC pc,Mat W,PetscBool *transpose, int *__ierr){
*__ierr = PCDeflationSetSpace(
	(PC)PetscToPointer((pc) ),
	(Mat)PetscToPointer((W) ),*transpose);
}
PETSC_EXTERN void PETSC_STDCALL  pcdeflationsetprojectionnullspacemat_(PC pc,Mat mat, int *__ierr){
*__ierr = PCDeflationSetProjectionNullSpaceMat(
	(PC)PetscToPointer((pc) ),
	(Mat)PetscToPointer((mat) ));
}
PETSC_EXTERN void PETSC_STDCALL  pcdeflationsetcoarsemat_(PC pc,Mat mat, int *__ierr){
*__ierr = PCDeflationSetCoarseMat(
	(PC)PetscToPointer((pc) ),
	(Mat)PetscToPointer((mat) ));
}
PETSC_EXTERN void PETSC_STDCALL  pcdeflationgetcoarseksp_(PC pc,KSP *ksp, int *__ierr){
*__ierr = PCDeflationGetCoarseKSP(
	(PC)PetscToPointer((pc) ),ksp);
}
PETSC_EXTERN void PETSC_STDCALL  pcdeflationgetpc_(PC pc,PC *apc, int *__ierr){
