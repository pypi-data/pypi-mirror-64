...
extern "C" {
#endif
PETSC_EXTERN void PETSC_STDCALL  pchmgsetreuseinterpolation_(PC pc,PetscBool *reuse, int *__ierr){
*__ierr = PCHMGSetReuseInterpolation(
	(PC)PetscToPointer((pc) ),*reuse);
}
PETSC_EXTERN void PETSC_STDCALL  pchmgsetusesubspacecoarsening_(PC pc,PetscBool *subspace, int *__ierr){
*__ierr = PCHMGSetUseSubspaceCoarsening(
	(PC)PetscToPointer((pc) ),*subspace);
}
PETSC_EXTERN void PETSC_STDCALL  pchmgsetcoarseningcomponent_(PC pc,PetscInt *component, int *__ierr){
*__ierr = PCHMGSetCoarseningComponent(
	(PC)PetscToPointer((pc) ),*component);
}
PETSC_EXTERN void PETSC_STDCALL  pchmgusematmaij_(PC pc,PetscBool *usematmaij, int *__ierr){
