...
class SceneClipboard():
    ...
    def serializeSelected(self, delete=False):
        ...
        for item in self.scene.grScene.selectedItems():
            ...
            if isinstance(item, QAbstractGraphicsNode):
                sel_nodes.append(item.parent.serialize())  # node序列化数据放到sle_node里面
                for socket in (item.parent.sockets):
    ...
    def deserializeFromClipboard(self, data):
        ...
        for node_data in data['nodes']:  # 还原各个node
            new_node = Node(self.scene, node_data['type'])
            new_node.deserialize(node_data, hashmap, restore_id=False)

            # read just the new node's position
            pos = new_node.pos
            # 到最近鼠标处粘贴
            new_node.setPos(mouse_scene_pos.x(),mouse_scene_pos.y())
