...
"""
TODO: 
- cstop: exception or stop
- get right form of REPLY
{"OP": "DESCRIBE", "TYPE": "8989_3", "LIMIT": "5000", "AFTERIEN": "0", "OFFSET": "0", "CSTOP": "1000"}, "type": "KERNEL SYSTEM PARAMETERS-8989_3", "referencedTypes": ["DOMAIN-4_2", "INSTITUTION-4", "NEW PERSON-200"], "timestamp": "2018-02-27T10:29:45"
"""
...
class OldReplyToNew:
    ...
    def transformReply(self, reply):
        ...
        for result in reply["results"]:
            ...
            if not nresult:
                raise Exception("Couldn't transform ", json.dumps(result))
...
Note: adds __cnodes marker which must be put into query form above
"""
class OldRecordToNew:
    ...
    def __newProp(self, prop, propValue):
        ...
        if re.match(r'\d', prop):
            raise Exception("decimal prop - change to _prop")
        return prop
    ...
    def __reframe(self, record, isMultiple=False):
        ...
        for prop in record:
            ...
            # Dive down - note: DM 1 didn't flatten singletons
            # TODO: properly add into a reply if called from one
            if propValue["type"] == "cnodes": # fmType == "9"
                if "stopped" in propValue:
                    return None # suppress if cnodes suppressed
                nrecord[nprop] = []
