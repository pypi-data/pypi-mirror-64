...
class TestBase:
    ...
    def __call__(self, *args, **kwargs):
        ...
        for i, function in enumerate([func for func in self.__dir__() if func.startswith("test_")]):
            error = check_error(self.__getattribute__(function))
            if error:
                errors[str(i) + ". " + function] = error
        return errors
...
class HRTest(TestBase):
    def __call__(self):
        ...
        if test_result:
            ...
            print("No error was found. Add more quicktests to make sure your code works.")
