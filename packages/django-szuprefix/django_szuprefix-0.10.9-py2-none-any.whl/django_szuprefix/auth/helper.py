...
def find_user_ids_by_tag(tag):
    ...
    if len(ps) == 2:
        ...
        if len(ps2) == 2:
            fvn = ps2[1]
    model = modelutils.get_model_verbose_name_map().get(mvn)[0]
    if not model:
        return
    field = modelutils.get_model_field_verbose_name_map(model).get(fvn)
    if not field:
        field = model._meta.get_field('first_name' if mvn == 'user' else 'name')
    lookup = {}
    lookup_name = '%s__name__in' % field.name if field.is_relation else '%s__in' % field.name
    lookup[lookup_name] = lookup_values.split(',')
    qset = model.objects.filter(**lookup)
