...
class LearningPlayer_v1(Player):
    ...
    HAND_FEATURES = len(Card)

    def __init__(self, name, epsilon=0.1, net_dir=None):
        super().__init__(name)
        if net_dir is None:
            self.create_nnet()
        else:
    ...
    def decide_best_move(self, game, player: TurnPosition):
        ...
        hand_vector = self.get_hand_vector(game.get_hand(player))
        ...
        predictions = self.position_net.predict([history_matrix, move_options_matrix, hand_matrix], batch_size=1024).reshape(len(legal_moves))
