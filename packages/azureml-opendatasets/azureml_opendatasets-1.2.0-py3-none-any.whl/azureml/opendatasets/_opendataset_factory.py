...
class TabularOpenDatasetFactory:
    ...
    def get_tabular_dataset(
        ...
        if enable_telemetry:
            ...
            log_properties['StartDate'] = start_date.strftime("%Y-%m-%dT%H:%M:%S") if start_date is not None else ''
            log_properties['EndDate'] = end_date.strftime("%Y-%m-%dT%H:%M:%S") if end_date is not None else ''
