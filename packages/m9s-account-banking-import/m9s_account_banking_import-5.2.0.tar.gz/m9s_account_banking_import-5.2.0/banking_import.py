...
class BankingImportConfiguration(ModelSQL, ModelView):
        ...
        for configuration in configurations:
            ...
            if len(configurations) > 1:
                raise UserError(gettext(
                        'account_banking_import.bank_account_used'))
            configurations = cls.search([
            ...
            if len(configurations) > 1:
                raise UserError(gettext(
                        'account_banking_import.journal_used'))
                return False
            if not configuration.bank_account.currency:
                raise UserError(gettext(
                        'account_banking_import.missing_currency_account'))
            if (configuration.bank_account.currency !=
                    configuration.journal.currency):
                raise UserError(gettext(
                        'account_banking_import.wrong_currency'))
    ...
    def get_transactions(cls, configurations):
        for configuration in configurations:
            ...
            if not hasattr(configuration, method_name):
                raise UserError(gettext(
                        'account_banking_import.method_not_available',
    ...
    def _get_batch(self):
        ...
        if (not self.journal.account_journal.credit_account or
                not self.journal.account_journal.debit_account):
        ...
        account_ids = [self.journal.account_journal.credit_account.id,
            self.journal.account_journal.debit_account.id]
...
class RunImportShow(ModelView):
    ...
    @fields.depends('batch_lines', 'line', 'invoices')
    def on_change_batch_lines(self):
        ...
        for line in self.batch_lines:
            ...
            account, side = line.__class__._choose_account(
...
class RunImportInfoUnposted(ModelView):
    ...
    __name__ = 'banking.run_import.info_unposted'
...
class RunImport(Wizard):
    ...
    info_unposted = StateView('banking.run_import.info_unposted',
        'account_banking_import.run_import_info_unposted_view_form', [
            Button('Ok', 'open_lines', 'tryton-ok'),
            ])
    ...
    def transition_next_(self):
        ...
        def next_line():
            ...
            if line_balance != self.start.running_balance:
                self.raise_user_warning('Banking Import: Running balance check',
                    'wrong_line_balance', (line.rec_name, line_balance,
                        self.start.running_balance,
                        line_balance - self.start.running_balance))
            # invoice matching
        ...
        if getattr(self.show, 'lines', None) is None:
            ...
            # write the first date found -1 as future processing_start_date
            ...
            if line_balance != start_balance:
                self.raise_user_warning('Banking Import: Initial balance check',
                    'wrong_initial_balance', (first_line.rec_name, line_balance,
                        start_balance, line_balance - start_balance))
