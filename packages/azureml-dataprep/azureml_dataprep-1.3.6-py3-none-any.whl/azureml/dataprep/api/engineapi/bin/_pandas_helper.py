...
_have_numpy = True
_have_pyarrow = True
_pyarrow_and_pandas_compatible = True
try:
    import pandas
except:
    _have_pandas = False
    _pyarrow_and_pandas_compatible = False
try:
    import numpy
except:
    _have_numpy = False
try:
    ...
    if pyarrow_major <= 0:
        if pyarrow_minor < 11:
            _have_pyarrow = False
            _pyarrow_and_pandas_compatible = False
        elif pyarrow_minor == 11 and _pyarrow_and_pandas_compatible:
            pandas_version_components = pandas.__version__.split('.')
            pandas_major = int(pandas_version_components[0])
            pandas_minor = int(pandas_version_components[1])
            if pandas_major >= 1:
                _pyarrow_and_pandas_compatible = False
        elif pyarrow_minor == 14:
...
def pyarrow_and_pandas_compatible() -> bool:
    global _pyarrow_and_pandas_compatible
    return _have_pyarrow and _have_pandas and _pyarrow_and_pandas_compatible
