...
def img_cr(ctx, arg1, **kwargs):
    ...
    if kwargs['opts']:
        ...
        try:
            ...
            results = cst.pool_call(img.img_2_stats, arg1,
                                    dict(res=x, omegas=omegas, cos=cos))
        ...
        try:
            ...
            for val in ['srcvalues', 'values']:
                if len(kwargs[val]) > 0:
                    ...
                    print('\t'.join([f'{i: <15}' for i in headers[val] +
                                     kwargs[val]]), file=f)
                    for j in range(len(arg1)):
                        if val == 'srcvalues':
                            for k, src in enumerate(results[j]['srcs']):
                                print('\t'.join([f'{i: <15}' for i in [imgkeys[j], k] +
                                                 [src[m] for m in 
                                                  kwargs['srcvalues']]]), file=f)
                        else:
                            print('\t'.join([f'{i: <15}' for i in [imgkeys[j]] +
                                             [results[j][m] for m in 
                                              kwargs['values']]]), file=f)
                    if out is not None:
                        f.close()
                    else:
                        print()
...
@click.option('-srng', default=None, callback=clk.split_float)
@clk.shared_decs(clk.command_decs(hs.__version__))
        ...
        try:
            ...
            if kwargs['baselinei'] is not None:
                baseline, scale, select = get_baseline(aS, kwargs['baselinei']
                                                       - kwargs['sidx'])
            for i in range(aS.shape[-1] + kwargs['dototal']):
                ...
                if kwargs['legacy']:
                    if kwargs['resample'] is None:
                        out = np.stack((labels.reshape(-1, 1), out[:,0:-1],
                                        rdif.reshape(-1, 1),
                                        adif.reshape(-1, 1)), axis=1)
                    else:
