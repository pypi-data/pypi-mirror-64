...
class BatchLine(ModelSQL, ModelView):
    ...
    def view_header_get(cls, value, view_type='form'):
        ...
        name = super(BatchLine, cls).view_header_get(value,
            view_type=view_type)
        ...
        batch_journal = BatchJournal(Transaction().context['journal'])
        ...
        if Transaction().context.get('batch'):
            ...
            batch = Batch(Transaction().context['batch'])
            name += ' | %s: %s | %s: %s' % (trans_batch,
                    batch.name, trans_journal, batch_journal.rec_name)
        else:
            name += ' | %s: %s' % (trans_journal, batch_journal.rec_name)
        ...
        if Transaction().context.get('period'):
            ...
            name += ' | %s: %s' % (trans_period, period.name)
            ...
            fiscalyear = Fiscalyear(Transaction().context['fiscalyear'])
            name += ' | %s: %s' % (trans_fiscalyear, fiscalyear.name)

        if Transaction().context.get('header_suffix'):
            name += ' %s' % (Transaction().context['header_suffix'],)
        return name
    ...
    def default_get(cls, fields, with_rec_name=True):
        ...
        if Transaction().context.get('journal'):
            journal = BatchJournal(Transaction().context['journal'])
            account_journal = journal.account_journal
            side = cls._choose_side(_ZERO, account_journal)
            if 'journal' in fields:
                values['journal'] = Transaction().context.get('journal')
        ...
        if self.tax:
            ...
            for tax in taxes:
                ...
                if ((tax_group_kind is None
                    ...
                    #abs_tax_amount *= tax.invoice_tax_sign
                    #tax_code = tax.invoice_tax_code
                    #base_tax_amount = abs_base_amount * tax.invoice_base_sign
                    base_tax_amount = abs_base_amount
                    #base_tax_code = tax.invoice_base_code
                    line_type = 'base'

                else:
                    #print('reverse', tax_group_kind)
                    #abs_tax_amount *= tax.credit_note_tax_sign
                    abs_tax_amount *= -1
                    #tax_code = tax.credit_note_tax_code
                    #base_tax_amount = (abs_base_amount
                    #    * tax.credit_note_base_sign)
                    base_tax_amount = abs_base_amount
                    base_tax_amount *= -1
                    #base_tax_code = tax.credit_note_base_code
                    line_type = 'base'

                #tax_line = self._get_tax_line(abs_tax_amount, tax_code, tax)
                tax_line = self._get_tax_line(abs_tax_amount, 'tax', tax)
                #print('tax_line', abs_tax_amount, 'tax', tax)
                tax_move_line.tax_lines += (tax_line,)
                # Create the tax line(s) for the base
                #if base_tax_code:
                base_tax_line = self._get_tax_line(base_tax_amount,
                    line_type, tax)
                #print('base_tax_line', base_tax_amount, line_type, tax)
