...
class Strategy(RefMixin):
    ...
    !!! example
        ...
        if __name__ == '__main__':
            ...
            candle_set = CandleSet(epic=epic, timeframe='15minutes', indicators=[mm50])
    ...
    def _is_strategy_paused(self, current_datetime: arrow.Arrow) -> bool:
        ...
        if self.paused_until is None:
            self.set_pause(current_datetime)
    ...
    def on_new_tick(self, tick: Tick) -> None:
        ...
        if self.stopped is False:
            self.set_stop(tick.datetime)
        if self.stopped is True:
            logger.warning('Strategy %s is stopped, it must not be called' % self.ref)
            return

        if self._is_strategy_paused(tick.datetime):
            logger.debug('Strategy %s is paused, does not apply tick.')
            return
    ...
    #     raise NotImplementedError('On new candle not implemented on Strategy')

    ##################################################
    # Strategy apply method to be implemented
    ##################################################
    def set_pause(self, current_datetime: arrow.Arrow) -> None:
        """
        This method is called before every opening and closing strategy methods
        execution. Its purpose is to update the `self.paused_until` parameter
        with a datetime (as arrow.Arrow) representing the target date until no
        opening/closing strategy method will be called.

        When a tick with a datetime superior to the `self.paused_until` value will
        be received from provider, the strategy will be un-paused automatically (the
        `self.paused_until` value will be reset to `None`)

        Arguments:
            current_datetime: datetime of the last received tick

        !!! note
            Setting the `self.paused_until` does not close trades automatically,
            if required you have to explicitly close trades.
