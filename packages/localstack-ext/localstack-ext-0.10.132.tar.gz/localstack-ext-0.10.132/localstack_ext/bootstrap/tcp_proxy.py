...
class AttrDict(OmJXr):
 def __init__(self,*args,**kwargs):
  OmJXM(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and OmJXR(socket,'IPV6_V6ONLY'):
...
class Half(OmJXQ):
 def __init__(self,opt,sock,addr,OmJXl):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.OmJXl=OmJXl
  self.name='peer' if self.OmJXl else 'client'
  self.queue=[]
  self.dest=OmJXN
  self.err=OmJXN
  self.ready=OmJXP
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':OmJXP,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
