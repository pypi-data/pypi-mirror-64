...
def import_auth():
 try:
  ...
  raise prIis('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=prIiQ)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=prIiM)
def get_params(args,names=[],mandatory=prIiQ):
 result=()
 for name in names:
  value=prIio
  if name in args:
  ...
  if not value and mandatory:
   raise prIis('Please provide %s=... parameter'%name)
  result+=(value)
 if prIid(result)==1:
  return result[0]
 return result
