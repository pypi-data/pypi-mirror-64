...
def import_auth():
 try:
  ...
  raise TzdhE('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=Tzdhj)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=Tzdhy)
def get_params(args,names=[],mandatory=Tzdhj):
 result=()
 for name in names:
  value=TzdhC
  if name in args:
  ...
  if not value and mandatory:
   raise TzdhE('Please provide %s=... parameter'%name)
  result+=(value)
 if TzdhK(result)==1:
  return result[0]
 return result
