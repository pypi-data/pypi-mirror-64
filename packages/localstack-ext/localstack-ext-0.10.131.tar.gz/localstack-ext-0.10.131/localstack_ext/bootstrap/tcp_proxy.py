...
class AttrDict(tYFHS):
 def __init__(self,*args,**kwargs):
  tYFHJ(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and tYFHn(socket,'IPV6_V6ONLY'):
...
class Half(tYFHR):
 def __init__(self,opt,sock,addr,tYFHs):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.tYFHs=tYFHs
  self.name='peer' if self.tYFHs else 'client'
  self.queue=[]
  self.dest=tYFHc
  self.err=tYFHc
  self.ready=tYFHC
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':tYFHC,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
