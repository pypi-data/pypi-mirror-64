...
class AttrDict(gfFlO):
 def __init__(self,*args,**kwargs):
  gfFlk(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and gfFls(socket,'IPV6_V6ONLY'):
...
class Half(gfFlq):
 def __init__(self,opt,sock,addr,gfFlK):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.gfFlK=gfFlK
  self.name='peer' if self.gfFlK else 'client'
  self.queue=[]
  self.dest=gfFlh
  self.err=gfFlh
  self.ready=gfFld
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':gfFld,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
