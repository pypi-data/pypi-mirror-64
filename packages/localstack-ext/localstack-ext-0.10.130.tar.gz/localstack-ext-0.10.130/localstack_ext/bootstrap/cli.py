...
def import_auth():
 try:
  ...
  raise iskVG('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=iskVa)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=iskVf)
def get_params(args,names=[],mandatory=iskVa):
 result=()
 for name in names:
  value=iskVg
  if name in args:
  ...
  if not value and mandatory:
   raise iskVG('Please provide %s=... parameter'%name)
  result+=(value)
 if iskVu(result)==1:
  return result[0]
 return result
