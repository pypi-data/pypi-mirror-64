...
class AttrDict(JvjiQ):
 def __init__(self,*args,**kwargs):
  JvjiR(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and JvjiB(socket,'IPV6_V6ONLY'):
...
class Half(JvjiO):
 def __init__(self,opt,sock,addr,Jvjit):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.Jvjit=Jvjit
  self.name='peer' if self.Jvjit else 'client'
  self.queue=[]
  self.dest=Jvjic
  self.err=Jvjic
  self.ready=JvjiA
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':JvjiA,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
