...
def import_auth():
 try:
  ...
  raise clnLM('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=clnLV)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=clnLo)
def get_params(args,names=[],mandatory=clnLV):
 result=()
 for name in names:
  value=clnLC
  if name in args:
  ...
  if not value and mandatory:
   raise clnLM('Please provide %s=... parameter'%name)
  result+=(value)
 if clnLi(result)==1:
  return result[0]
 return result
