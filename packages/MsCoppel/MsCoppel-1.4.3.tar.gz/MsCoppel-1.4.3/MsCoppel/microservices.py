...
class Microservices(BaseClass, ABC):
    ...
    def _message(self, msg):
        ...
        try:
            ...
            self.__response(msg, error.errorCode, span=span,
                            nats_route=msg.get('uuid', None))  # Enviar el error
        except Exception as e:
            span.set_tag(ext_tags.ERROR, True)
            ...
            self.logs.error(e)
            # Enviar el error
            self.__response(msg, -99, span=span,
                            nats_route=msg.get('uuid', None))

        span.finish()

    def __response(self, data, errorCode, isSmokeTest=False, nats_route=None, span=None):
        """
