...
def adaptive_binarization(threshold_type='otsu' ):
    def img_op(image: np.ndarray):
        ...
        try:
            ...
            image =(blur > local_thresh).astype(np.float32)*255.0
...
def random_blur():
    def img_op(image: np.ndarray):
        ...
        if n==0:
            ...
            image =  gaussian(image.astype(np.float32), sigma=np.random.choice(np.arange(0.1,0.6,0.05)),multichannel=True)
        return image
    return img_op
