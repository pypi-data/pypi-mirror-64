...
def l1_reg(model:nn.Module,reg_weight=1e-6):
    ...
    loss =torch.tensor(0,dtype=torch.float,requires_grad=True).to(_device)
...
def total_variation_norm_reg(output:torch.Tensor,reg_weight=1):
    ...
    loss = reg_weight * (torch.mean(torch.abs(output[:, :, :, :-1] - output[:, :, :, 1:])) + torch.mean(torch.abs(output[:, :, :-1, :] - output[:, :, 1:, :])))/2.0
    return loss
