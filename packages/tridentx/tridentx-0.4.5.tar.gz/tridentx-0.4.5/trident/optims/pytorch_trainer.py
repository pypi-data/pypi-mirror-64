...
class Model(ModelBase):
    ...
    @property
    def outputs(self):
        if self._model is None or not self._model.built:
            return None
            ...
            dummay_input = to_tensor(np.random.standard_normal((1, *self._model.input_shape)).astype(np.float32))
    ...
    def with_loss(self, loss, loss_weight=1, output_idx=0, start_epoch=0,name='', **kwargs):
        ...
        if outputs is not None and len(outputs) ==1 :
            keys=argnames.key_list.copy()
            if keys[0].lower()=='input':
    ...
    def do_on_data_received(self, train_data, test_data):
        ...
        if 'data_feed' not in self.training_context or len(self.training_context['data_feed']) == 0:
            try:
                ...
                if train_data is not None:
                    ...
                    if len(self.signature.key_list)==1and data_feed[self.signature.key_list[0]] !=None:
        ...
        try:
            ...
            for item in train_data.key_list:
                ...
                elif item in self.targets.key_list:
    ...
    def save_model(self, save_path=None):
        ...
        if save_path is None or len(save_path) == 0:
            ...
            if save_path is None or len(save_path) == 0:
                save_path = 'Models/model_{0}_epoch{1}.pth'.format(self._model.name, self.training_context['current_epoch'])
        torch.save({'state_dict': self._model.state_dict()}, save_path + '.tar_')
        #save safety
        shutil.copyfile(save_path + '.tar_',save_path + '.tar')
        self._model.train()
