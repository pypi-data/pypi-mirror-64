...
class _ConvNd(Conv):
  ...
  def __init__(self, rank,
    ...
    super(_ConvNd, self).__init__(rank=rank, filters=num_filters, kernel_size=kernel_size, strides=strides, padding='same' if auto_pad==True else 'valid', data_format='channels_last',
        dilation_rate=dilation, activation=get_activation(activation), use_bias=use_bias, kernel_initializer='glorot_uniform', bias_initializer='zeros',
        kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None, kernel_constraint=None,
        bias_constraint=None, trainable=True, name=name)
    self.filter_rate=filter_rate
  ...
  def build(self, input_shape):
    ...
    kernel_shape = self.kernel_size + (input_filters, self.num_filters//self.groups)
    ...
    channel_axis = self._get_channel_axis()
    self.input_spec = InputSpec(ndim=self.rank + 2,
                                axes={channel_axis: input_filters})
  ...
  def compute_output_shape(self, input_shape):
    ...
    if self.data_format == 'channels_last':
      ...
      raise ValueError('only channel_last is supported in tensorflow')
