...
__all__ = ['get_uid','to_numpy','to_tensor','print_network','plot_tensor_grid','summary','calculate_flops', 'Layer', 'Sequential', 'Input', 'get_device', 'load','Combine','ReplayBuffer','check_keys','try_map_args_and_call','get_signature']
...
class Sequential(Layer):
    ...
    def add_module(self, name, module):
        ...
        if not isinstance(module, Layer) and module is not None:
            ...
            raise KeyError("module name can't be empty string \"\"")
        if len(self._modules) > 0 and self._modules[list(self._modules.keys())[0]]._input_shape is not None and \
                self._modules[list(self._modules.keys())[-1]].built and self._modules[
            list(self._modules.keys())[-1]]._output_shape is not None:
            module.input_shape = self._modules[list(self._modules.keys())[-1]]._output_shape
        self._modules[name] = module
