...
class TileImageCallback(VisualizationCallbackBase):
    ...
    def plot_tile_image(self, training_context):
        ...
        for data_key in data.key_list:
            ...
            elif 'output' in data_key and not 'target' in data_key and data_key not in data_feed.value_list:
                    output = data[data_key]
            elif ('target' in data_key or 'label' in data_key or 'mask' in data_key) and not 'output' in data_key and data_key in data_feed.value_list:
...
class SegTileImageCallback(VisualizationCallbackBase):
    ...
    def plot_tile_image(self, training_context):
        ...
        # if self.tile_image_include_mask:
        #     tile_images_list.append(input*127.5+127.5)
        tile_rgb_images(*tile_images_list, save_path=os.path.join(self.save_path, self.tile_image_name_prefix),
                        imshow=True)
...
class PlotLossMetricsCallback(VisualizationCallbackBase):
    ...
    def on_overall_batch_end(self, training_context):
        if self.batch_inteval > 0 and (
                self.training_items.value_list[0].training_context['current_batch'] + 1) % self.batch_inteval == 0:
