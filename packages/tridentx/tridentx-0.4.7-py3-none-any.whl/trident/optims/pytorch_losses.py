...
__all__ = ['MSELoss', 'CrossEntropyLoss','NullLoss','BCELoss', 'F1ScoreLoss', 'L1Loss', 'L2Loss', 'make_onehot', 'MS_SSIM',
           'CrossEntropyLabelSmooth', 'mixup_criterion', 'DiceLoss','IouLoss', 'focal_loss_with_logits', 'FocalLoss',
...
class CrossEntropyLoss(_Loss):
    ...
    def forward(self, output, target):
        ...
        return F.cross_entropy(output, target,weight=self.weight,ignore_index=self.ignore_index,reduction=self.reduction)

class NullLoss(_Loss):
    def __init__(self, weight=None, with_logits=True, ignore_index=-100, reduction='mean', name='CrossEntropyLoss'):
        super().__init__(reduction=reduction)
        self.name = name
        self.weight = weight
    ...
    def forward(self, output, target):
        ...
        if len(output.size()) == len(target.size()):
            target = argmax(target, 1)
        return F.nll_loss(output, target, weight=self.weight, ignore_index=self.ignore_index,
                               reduction=self.reduction)

    #     if len(output.shape)==2 and len(target.shape)==1:
    #         return F.cross_entropy(output, target.long(),ignore_index=self.ignore_index,reduction=self.reduction)
    #     elif len(output.shape)==2 and len(target.shape)==2:
    #         return super().forward(output, argmax(target,1))
    #     elif len(output.shape)>2 and len(target.shape)==len(output.shape)-1:
    #         return super().forward(output.view(-1,output.size(1)), target.long().view(-1))
    #     elif len(output.shape)>2 and len(target.shape)==len(output.shape):
    #         return super().forward(output.view(-1,output.size(1)), argmax(target,1).view(-1))
...
class FocalLoss(_Loss):
    ...
    def forward(self, output, target):
        ...
        for cls in range(num_classes):
            ...
            loss+=focal_loss_with_logits(cls_input, cls_target, gamma=self.gamma, alpha=self.alpha, reduction=self.reduction)
