...
class NjinnWorker():
    ...
    def __init__(self, registration_token=None, njinn_url=None):
        ...
        if not config_valid and not registration_token:
            if not config_valid:
                print(
                    "Config is invalid. Please try to register the worker again.", file=sys.stderr)
            else:
    ...
    def register(self, registration_token, njinn_url):
        ...
        api_base = njinn_url or os.environ.get(
            'NJINN_URL', 'https://api.njinn.io')
        ...
        if response.status_code != 200:
            ...
            api_url = os.getenv(
                "NJINN_WORKER_API",
                scheme + '://' + response.json()['domain_url']
            )
...
def main(api=None, token=None):
    ...
    njinn_url = sys.argv[-2] if len(
        sys.argv) > 2 else os.getenv("NJINN_URL")
    registration_token = token or os.getenv("NJINN_WORKER_TOKEN")
    print("Config file:", get_config_path())
    worker = NjinnWorker(
        registration_token=registration_token, njinn_url=njinn_url)
