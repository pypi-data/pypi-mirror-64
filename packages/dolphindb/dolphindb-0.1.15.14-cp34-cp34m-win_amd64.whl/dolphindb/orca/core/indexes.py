...
from .operator import (ArithOpsMixin, IndexLike, IOOpsMixin, LogicalOpsMixin,
                       SeriesLike, StatOpsMixin)
from .utils import (_infer_level, _to_freq, _to_numpy_dtype,
...
class IndexOpsMixin(ArithOpsMixin, LogicalOpsMixin, metaclass=abc.ABCMeta):
    ...
    @property
    def _is_free_index(self):
        # is an index not dependent to a Series or DataFrame
        return isinstance(self._internal, _ConstantSP)

    @property
    def _ddb_dtype(self):
        if self._is_free_index:
            return self._type
        else:
...
class Index(IndexLike, _InternalAccessor, IndexOpsMixin, IOOpsMixin):
    ...
    @classmethod
    def _get_from_script(cls, session, script, name):
        var = _ConstantSP.run_script(session, script)
        return cls(var, name=name, session=session)
    ...
    @property
    def nlevels(self):
        return len(self._index_columns)
    ...
    def get_level_values(self, level):
        ...
        index_map = [self._index_map[level_idx]]
        data_columns = self._data_columns
        odf = _InternalFrame(self._session, self._var, index_map, data_columns,
                             self._column_index, self._column_index_names)
        return Index(odf, session=self._session)
