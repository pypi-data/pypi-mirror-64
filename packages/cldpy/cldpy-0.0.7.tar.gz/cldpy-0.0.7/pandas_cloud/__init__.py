...
monkey_patch('pandas', 'pandas_cloud', SERVER_URL, proxy_classes)
del sys.modules['pandas_cloud']
__import__('pandas_cloud')
...
proxy_read_csv = sys.modules['pandas_cloud'].read_csv
    ...
    with ServerProxy(SERVER_URL, allow_none=True) as server:
        ...
        if not has_file:
            ...
            resp = requests.put(url, headers={'Content-Type': content_type},
                                data=data)
        req = Request('', '', '', [], kwargs)
        resp = Response.deserialize(server.from_file(file_hash, content_type, req.serialize()))
        return resp.unpack(get_factory_singleton())
sys.modules['pandas_cloud'].read_csv = read_csv
