...
@syspools.add(IdiomaticPool)
def np_applies(dim, additional=(), const_input=True, ref_out=True, typedefs: Dict[str] = (), *, swim):
    def dim_params(pref):
...
def numpy_arrays(numpy_dir='', additionals=(), typedefs: Dict[str, str] = None, allow_char_arrays=False,
    ...
    additional = list(additionals)
    if allow_char_arrays:
        np_char(swim=swim)
        additional.append('char')
    if typedefs:
        additionals.extend(typedefs.values())

    additional = tuple(additional)

    for dim in range(1, max_dim + 1):
        swim(np_applies(dim=dim, const_input=const_input, ref_out=ref_out, additional=additional, typedefs=typedefs))
