    ...
    def __init__(self,
        ...
        for attr in kwargs:
            if not (re.match(r'\S*__\w+__$', attr) or attr in self.ignored_attributes or type(kwargs[attr]) == Filter):
                # TODO make the pattern above match "__eq__"
    ...
    def __eq__(self, other):
        ...
        for pattern in patterns:
            if type(self.__getattribute__(pattern)) != Filter:
                ...
                try:
                    attributes = [x for x in pattern.split("__")[0:-2] if x]
                    ...
                    print(f"{repr(vo)}.{magic_method}({repr(self.__getattribute__(pattern))}) is {r}")
                    checks.append(r)
