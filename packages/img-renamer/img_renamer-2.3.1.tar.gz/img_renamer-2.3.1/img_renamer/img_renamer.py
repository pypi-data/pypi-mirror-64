...
def rename_images():
    ...
    if not yes:
        ...
        print('_'*20)
        print('Renaming {nro} of {total}'.format(nro=rename_total, total=nro_total))
    ...
    for i in rename:
        n += 1
        osrename('{folder}{r0}'.format(folder=folder, r0=rename[n][0]), '{folder}xxx{r1}'.format(folder=folder, r1=rename[n][1]))
    # Rename images to correct name
    n = -1
    for i in rename:
        n += 1
        osrename('{folder}xxx{r1}'.format(folder=folder, r1=rename[n][1]), '{folder}{r1}'.format(folder=folder, r1=rename[n][1]))
...
def create_log(rename_list):
    ...
    for i in rename_list:
        n += 1
        text += '{r0} -> {rl1}\n'.format(r0=rename_list[n][0], rl1=rename_list[n][1])
