...
def safe_port(from_internet=False, which_ports='static'):
    """
    ...
    """
    ports = get_ports(scrape=from_internet, which_ports=which_ports)
    while True:
...
def get_ports(scrape=False, which_ports='static'):
    """
...
def _format_ports_options_str():
    ...
    """
    all_but_last = ', '.join(PORTS_OPTIONS.keys()[:-1])
    last = PORTS_OPTIONS.keys()[-1]
...
def main():
    ...
    try:
        port = safe_port(args.internet, args.which)
