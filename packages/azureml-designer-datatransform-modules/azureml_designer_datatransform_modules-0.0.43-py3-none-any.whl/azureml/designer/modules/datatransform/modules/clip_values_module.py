...
class ClipValuesModule(ModuleBase):
    ...
    def clip_function(
        ...
        if indicator is True:
            return column.apply(
                lambda x: True if (
                    up_threshold is not None and x > up_threshold) or (
                    bottom_threshold is not None and x < bottom_threshold) else False)
        else:
            return column.apply(
                lambda x: up_replace_value
                if up_threshold is not None and x > up_threshold
                else bottom_replace_value
                if bottom_threshold is not None and x < bottom_threshold else x)
