...
from azureml.studio.core.utils.column_selection import ColumnSelectionBuilder, ColumnType, \
    ColumnSelectionColumnNotFoundError
...
from azureml.studio.internal.error import ErrorMapping, BadNumberOfSelectedColumnsError, ParameterParsingError, \
    InvalidColumnTypeError, NotInRangeValueError, ColumnNotFoundError, CouldNotConvertColumnError, InfinityError
...
class ApplyMathModule(ModuleBase):
    _OPS_FUNC_MAPPING = {
        ...
        BasicFunc.Log: lambda x, y: math.log(x, y) if y > 1 else np.NaN,
        ...
        else raise_exception(CouldNotConvertColumnError(type1=np.NaN, type2=int)),
    ...
    _MPMATH_SUPPORTED = (
        ...
        TrigonometricFunc.Acsc,
        TrigonometricFunc.AcscDegrees,
        TrigonometricFunc.Acsch,
        TrigonometricFunc.Arg,
        TrigonometricFunc.Asec,
        TrigonometricFunc.AsecDegrees,
        TrigonometricFunc.Asech,
        ...
        TrigonometricFunc.Atanh,
        TrigonometricFunc.Cos,
        TrigonometricFunc.CosDegrees,
        TrigonometricFunc.Cosh,
        TrigonometricFunc.Cot,
        TrigonometricFunc.CotDegrees,
        TrigonometricFunc.Coth,
        TrigonometricFunc.Csc,
        TrigonometricFunc.CscDegrees,
        TrigonometricFunc.Csch,
        TrigonometricFunc.Sec,
        TrigonometricFunc.SecDegrees,
        TrigonometricFunc.Sech,
        TrigonometricFunc.Sign,
        TrigonometricFunc.Sin,
        TrigonometricFunc.Sinc,
    ...
    def _call_mpmath_by_name(func_name: str, x):
        if func_name.endswith("Degrees"):
            ...
            return ApplyMathModule.convert_mpmath_result(
                method_to_call(math.radians(x)))
        ...
        for category in MathCategory:
            ...
            parameter_list.extend([
                ...
                ValueModuleParameter(
                    ...
                    data_type=float if category != MathCategory.Rounding else int, default_value=0),
            ])
    ...
    def run(self):
        ...
        try:
            ...
            ErrorMapping.rethrow(
                valErr, NotInRangeValueError)
        except OverflowError as overflowErr:
            ErrorMapping.rethrow(
                overflowErr, InfinityError)
        except FloatingPointError as floatingPointErr:
            ErrorMapping.rethrow(
                floatingPointErr, NotInRangeValueError)
    ...
    def _column_select_arg_op(
        ...
        if len(
            data_df.columns) != 1 and len(
            arg_df.columns) != 1 and len(
            data_df.columns) != len(
                arg_df.columns):
            ErrorMapping.throw(
