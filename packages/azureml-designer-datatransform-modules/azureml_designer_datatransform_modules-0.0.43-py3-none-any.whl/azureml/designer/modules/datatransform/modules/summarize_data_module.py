...
class Series_Summarize():
    ...
    def _is_numeric(self) -> bool:
        if self._is_timedelta or self._is_datetime or np.issubdtype(
                self._data, np.number):
            return True
        return False
    ...
    def get_summarize(self) -> pd.DataFrame:
        functions = {
            ...
            "Unique Value Count": self._data.nunique() if type(self._data.iloc[0]) != np.ndarray
            else pd.Series(self._data).apply(lambda x: tuple(sorted(x))).nunique(),
            ...
            if type(self._data.iloc[0]) != np.ndarray else None,
            "Range": self._range(),
