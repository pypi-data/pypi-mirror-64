...
from pyalink.alink.py4j_util import get_java_gateway, get_java_class, set_java_gateway
from .config import g_config
from .converters import py_list_to_j_array
from .utils import get_alink_lib_path, get_pyflink_path
...
def useRemoteEnv(host: str, port: Union[int, str], parallelism: int, flinkHome: str = None,
                 localIp: str = "localhost", shipAlinkAlgoJar: bool = True, config: object = None) -> MLEnv:
    """
...
def usePyFlinkEnv(parallelism: int = None, flinkHome: str = None) -> MLEnv:
    ...
    if g_config["flink_home"] is None:
        ...
        # Let PyFlink to launch gateway, and warn users to add jars to flink lib path
        print("Warning: it seems you're running the script using 'flink run'. "
              "In this case, you have to manually add alink jars to flink lib path.")
        import pyflink
...
def getMLEnv() -> MLEnv:
    ...
    if _mlenv is None:
        if has_pyflink():
            ...
            raise Exception("Call useLocalEnv/useRemoteEnv to initialize MLEnv first, or update to a newer version of PyAlink which depends on PyFlink-1.10 to use PyFlink env.")
    return _mlenv
