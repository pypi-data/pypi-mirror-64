...
class BaseOperator(WithParams):
    ...
    def __init__(self, params, name, cls_name, j_op=None, model=None):
        ...
        if j_op is not None:
            ...
            if model is None:
                self._j_op = get_java_gateway().jvm.__getattr__(cls_name)()
            else:
                self._j_op = get_java_gateway().jvm.__getattr__(cls_name)(model.get_j_obj())
    ...
    def udf(self, func, selectedCols, outputCol, resultType, reservedCols=None):
    ...
    def udtf(self, func, selectedCols, outputCols, resultTypes, reservedCols=None):
