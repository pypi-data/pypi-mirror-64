...
class Simulation:
    ...
    def add_file(self, name, group=None, clean=False, forced_name=None):
        n = forced_name if forced_name is not None else path.basename(name)
        self.files[n] = {'type': 'file', 'path': name, 'clean': clean}
        self.response['files'].append({'path': n, 'group': group, 'type': guess_type(name)[0]})

    def add_plot_file(self, name, plt, group=None, dpi=150, transparent=False):
        full_name = path.join(mkdtemp(), name)
        plt.savefig(full_name, dpi=dpi, transparent=transparent)
        self.add_file(full_name, group, True, name)

    def add_file_with(self, name, content, mime_type, group=None):
        n = path.basename(name)
        self.files[n] = {'type': 'inline', 'content': content, 'clean': False}
