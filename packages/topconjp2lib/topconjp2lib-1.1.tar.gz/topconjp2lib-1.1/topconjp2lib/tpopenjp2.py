...
libname2 = 'topconopenjp2.dll'
path = os.path.join(os.path.dirname(__file__),libname2)

# path = str(os.getcwd()) + '/lib/openjp2.dll'
# path = './lib/openjp2.dll'
sys.stdout.write("DLL path: %s" %(path))
sys.stdout.write("libname: %s" %(libname2))
# loader = ctypes.cdll
loader = ctypes.windll.LoadLibrary if os.name == 'nt' else ctypes.CDLL
try:
    sys.stdout.write("before load library executed")
    OPENJP2 = loader(path)
    # OPENJP2 = ctypes.cdll.LoadLibrary(path)
    sys.stdout.write("Call to load library executed")
    if OPENJP2 is None:
        name = "topconopenjp2.dll"
        OPENJP2 = loader(name)
    sys.stdout.write("Call2 to load library executed")
except TypeError:
    # This can happen on Windows.  Apparently ctypes.windll.LoadLibrary
    # is no longer taking a WindowsPath
    # path = str(path)
    # OPENJP2 = loader(path)
    sys.stdout.write("inside TypeError branch")
    name = str(path)
    OPENJP2 = loader(name)
    sys.stdout.write("After call to loader in TypeError branch")
except OSError:
    msg = f'The {libname2} library at {path} could not be loaded.'
    # warnings.warn(msg, UserWarning)
    OPENJP2 = None
except:
    sys.stdout.write('The %s library at %s could not be loaded.' %(libname, path))
    traceback.print_exc()
    #return opj_lib

# OPENJP2 = cfg.glymur_config()
# sys.stdout.write("glymur version: %s" %(glymur.version.openjpeg_version))
# sys.stdout.write("glymur version: %s" %(glymur.version.version))
...
sys.stdout.write(" Major: %d... Minor: %d... Path: %d..." % (_MAJOR, _MINOR, _PATCH))
...
class MemoryStreamType(ctypes.Structure):
    """Memory stream type.

    Corresponds to opj_memory_stream type in openjp2 headers.
    """
    # Resolution num start, Component num start, given by POC
    _fields_ = [
        ("pData",     ctypes.POINTER(ctypes.c_uint8)), # noqa
        ("dataSize",  ctypes.c_size_t),  # noqa
        ("offset",    ctypes.c_uint8),  # noqa
    ]
...
def stream_create_default_memory_stream(mem_stream, isa_read_stream):
    """Wraps openjp2 library function opj_stream_create_default_memory_stream.

    Sets the stream to be a memory stream.  This function is only valid for the
    2.1 version of the openjp2 library.

    Parameters
    ----------
    mem_stream : structure
        Specifies a memory stream.
    ...
    ARGTYPES = [ctypes.POINTER(MemoryStreamType), ctypes.c_int32]
    OPENJP2.opj_stream_create_default_memory_stream.argtypes = ARGTYPES
    OPENJP2.opj_stream_create_default_memory_stream.restype = STREAM_TYPE_P
    read_stream = 1 if isa_read_stream else 0
    if (read_stream == 1):
        stream = OPENJP2.opj_stream_create_default_memory_stream(mem_stream, read_stream)
    else:
        stream = OPENJP2.opj_stream_default_create(read_stream)
    return stream
...
def stream_create_default_file_stream(fname, isa_read_stream):
    ...
    if (read_stream == 1):
        stream = OPENJP2.opj_stream_create_default_file_stream(file_argument, read_stream)
    else:
        stream = OPENJP2.opj_stream_default_create(read_stream)
    return stream
