...
def generate_refline(bound_l: np.ndarray,
    ...
    both_tb = np.row_stack((
        bound_l[0, :] - np.diff(bound_l[:2, :] * 0.5, axis=0),
        bound_l,
        bound_l[-1, :] + np.diff(bound_l[-2:, :] * 0.5, axis=0),
        bound_r[-1, :] + np.diff(bound_r[-2:, :] * 0.5, axis=0),
        np.flipud(bound_r),
        bound_r[0, :] - np.diff(bound_r[:2, :] * 0.5, axis=0)
    ))
