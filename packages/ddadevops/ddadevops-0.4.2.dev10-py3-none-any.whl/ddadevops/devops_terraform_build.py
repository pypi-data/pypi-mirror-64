...
class DevopsTerraformBuild(DevopsBuild):
    ...
    def init_client(self):
        ...
        self.print_terraform_command(tf, 'init')
        tf.init()
        if self.use_workspace:
            try:
                self.print_terraform_command(tf, 'select', self.stage)
                tf.workspace('select', self.stage)
            except:
                self.print_terraform_command(tf, 'new', self.stage)
                tf.workspace('new', self.stage)
        return tf

    def write_output(self, tf):
        self.print_terraform_command(tf, 'output', json=IsFlagged)
    ...
    def plan(self):
        tf = self.init_client()
        self.print_terraform_command(tf, 'plan', var=self.project_vars())
        tf.plan(capture_output=False, raise_on_error=True, var=self.project_vars())

    def apply(self, auto_approve=False):
        tf = self.init_client()
        self.print_terraform_command(tf, 'apply', skip_plan=auto_approve,
    ...
    def destroy(self, auto_approve=False):
        tf = self.init_client()
        self.print_terraform_command(tf, 'destroy', force=force, 
            var=self.project_vars())
        if auto_approve:
            force=IsFlagged
        else:
    ...
    def tf_import(self, tf_import_name, tf_import_resource,):
        tf = self.init_client()
        self.print_terraform_command(tf, 'import', tf_import_name, tf_import_resource,
                      var=self.project_vars())
        tf.import_cmd(tf_import_name, tf_import_resource,
                      capture_output=False, raise_on_error=True, var=self.project_vars())

    def print_terraform_command(self, tf, cmd, *args, **kwargs):
        if self.debug_print_terraform_command:
            output = 'cd ' + self.build_path() + tf.generate_cmd_string(cmd, *args, **kwargs)
            print(output)
