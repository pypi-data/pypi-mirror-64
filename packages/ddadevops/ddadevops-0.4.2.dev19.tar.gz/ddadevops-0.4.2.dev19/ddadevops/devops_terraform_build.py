...
class WorkaroundTerraform(Terraform):
    def super().__init__(working_dir, targets, state, variables, parallelism,
    ...
    def generate_cmd_string(self, cmd, *args, **kwargs):
        result = super().generate_cmd_string(self, cmd, *args, **kwargs)
        self.latest_cmd = ' '.join(result)
        print(self.latest_cmd)
        return result
...
class DevopsTerraformBuild(DevopsBuild):
    ...
    def print_terraform_command(self, tf):
        if self.debug_print_terraform_command:
            output = 'cd ' + self.build_path() + ' ' + tf.get_latest_cmd()
            print(output)
