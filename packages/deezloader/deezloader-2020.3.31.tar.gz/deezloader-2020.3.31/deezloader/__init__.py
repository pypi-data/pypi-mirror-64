...
from deezloader.utils import (
	...
	check_dir, get_ids, not_found,
	qualities, generate_token, artist_sort
)
...
class Login:
	...
	def download(
		...
		if "track" in link:
			...
			try:
				...
				try:
					ids = not_found(song, datas['music'], name)
				except IndexError:
		...
		for a in t:
			...
			try:
				...
				try:
					ids = not_found(song, detas['music'])
	...
	def download_trackdee(
		...
		url1 = request(
			api_album % str(url['album']['id']), True
		).json()
	...
	def download_albumspo(
		...
		try:
			...
			while upc[0] == "0":
				...
				try:
					upc = "upc:%s" % upc
					url = request(api_album % upc, True).json()
