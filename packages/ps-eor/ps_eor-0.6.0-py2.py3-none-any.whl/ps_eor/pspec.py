...
class PowerSpectraBuilder(object):
    ...
    def __init__(self, ps_config, eor_bin_list=None):
        self.ps_config = PowerSpectraConfig.load(ps_config)
        if eor_bin_list is not None:
            self.eor_bin_list = EorBinList.load(eor_bin_list)
        else:
            self.eor_bin_list = None
...
class SpatialPowerSpectra(PowerSpectraMath):
    ...
    def plot_kper(self, ax=None, nsigma=0, fill_std=False, normalize=False, mkelvin=True,
        ...
        if weighted and isinstance(self.w, np.ndarray):
            y = np.average(self.data, weights=self.w, axis=0)
            y_err = np.sqrt(np.sum((self.err * self.w) ** 2, axis=0)) / self.w.sum(axis=0)
        else:
            y = np.mean(self.data, axis=0)
            y_err = np.sqrt(np.sum(self.err ** 2, axis=0)) / len(self.freqs)
