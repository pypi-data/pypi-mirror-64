...
from .releases import Types, Commands
...
class _Versioning:
    ...
    def discover(release_: int, build_: int) -> Tuple[int, int]:
        ...
        all_builds = sorted({int(dir_entry.name[1:5].rstrip('tcu')) for dir_entry in os.scandir(BUILDS.__path__[0])
    ...
    def load(self, release: int, build: int, request):
        self.types = importlib.import_module(f'archicad.releases.ac{release}.b{build}types')
