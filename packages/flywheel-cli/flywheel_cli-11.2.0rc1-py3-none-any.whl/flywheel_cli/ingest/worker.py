...
class Worker:  # pylint: disable=too-few-public-methods
    ...
    def run(self):
        """Run the worker"""
        orig_sigint_handler = signal.signal(signal.SIGINT, self.exit_gracefully)
        orig_sigterm_handler = signal.signal(signal.SIGTERM, self.exit_gracefully)
    ...
    def exit_gracefully(self, signum, *_):
        ...
        """
        log.info(f"{self.name} received {signum}, trying to shut down gracefully...")
        self.shutdown.set()
        signal.alarm(15)
...
def alarm_handler(signum, frame):
    ...
    """
    raise ShutdownTimeout("Worker shutdown grace period exceeded")
