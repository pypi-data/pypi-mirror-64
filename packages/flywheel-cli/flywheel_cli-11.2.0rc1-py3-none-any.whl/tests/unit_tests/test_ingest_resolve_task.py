...
def test_resolve_container_visited(attr_dict, resolve_task):
    ...
    assert container == resolve_task.db.get_container.return_value
    resolve_task.db.get_container.assert_called_once_with(path='path')
...
def fw_resolve_mock(mocker, attr_dict):
    fw_mock = mocker.patch('flywheel_cli.ingest.utils.flywheel.Flywheel')
