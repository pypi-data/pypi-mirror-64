...
class ForecasterParalleLauncher():
    def __init__(self,drop_token='', dropbox_backup=True, local_root_directory='../data',user='napoleon',  db_path_suffix = '_run.sqlite', ss=[10], ns=[21], s_evals = [5], signal_idiosyncratic_paramaters=[None]):
        self.args = []
        self.counter = 1
        self.seed = 0
        for s in ss:
            for n in ns:
                for s_eval in s_evals:
                    for signal_type, params  in signal_idiosyncratic_paramaters.items():
                        for param in params:
                            self.args.append((self,self.seed,n,s,s_eval,signal_type,json.dumps(param, sort_keys=True)))
