...
class AbstractRunner(ABC):
    def __init__(self, starting_date = None, running_date = None, drop_token=None, dropbox_backup = True, X_y_pkl_file_name='X_y.pkl', local_root_directory='../data/', user = 'napoleon'):
        super().__init__()
        self.X_y_pkl_file_name=X_y_pkl_file_name
    ...
    @abstractmethod
    def runTrial(self,saver,  seed,  n, s, s_eval, signal_type, idios_string):
        pass
...
class ForecasterRunner(AbstractRunner):
    def runTrial(self, saver, seed, n, s, s_eval, signal_type, idios_string):
        ...
        if check_run_existence:
            return
        raw_data = pd.read_pickle(self.local_root_directory + self.X_y_pkl_file_name)
        print('size before filtering')
        print(raw_data.shape)
        raw_data=raw_data[raw_data.index >= self.starting_date]
        ...
        model = forecasting_model.ForecasterWrapper()
