...
def plot_ave_with_interval(x_arr,
    ...
    *Plot average profile with or without confidence interval*
    ...
    axe.set_ylabel(y_label)#, fontsize=15, fontweight='bold')
    axe.set_xlabel(x_label)#, fontsize=15, fontweight='bold')

    if "average" in profile:
        axe.plot(average, x_arr, c='k', lw=0.5)

    #if "interval" in profile or (upper is None) or (lower is None):
    #    pass
    #else:
